...
public class AdGroupMutationServiceUpdateTextGroupSaveDraftTest {
    ...
    private GdUpdateTextAdGroup gdUpdateTextAdGroup;
    ...
    @Before
    public void before() {
        ClientInfo clientInfo = steps.clientSteps().createClient(defaultClient().withCountryRegionId(Region.RUSSIA_REGION_ID));
        ...
        gdUpdateTextAdGroup = new GdUpdateTextAdGroup().withUpdateItems(singletonList(
                new GdUpdateTextAdGroupItem()
                        .withAdGroupName("test")
    ...
    @Test
    public void updateTextAdGroup_saveDraftTrue() {
        GdUpdateAdGroupPayload payload = adGroupMutationService.updateTextAdGroup(UidAndClientId.of(uid, clientId), uid,
                gdUpdateTextAdGroup.withSaveDraft(true));
        checkStatusModerate(payload, StatusModerate.NEW);
    }

    @Test
    public void updateTextAdGroup_saveDraftFalse() {
        GdUpdateAdGroupPayload payload = adGroupMutationService.updateTextAdGroup(UidAndClientId.of(uid, clientId), uid,
                gdUpdateTextAdGroup.withSaveDraft(false));
        checkStatusModerate(payload, StatusModerate.READY);
    }

    @Test
    public void updateTextAdGroup_withoutSaveDraft() {
        GdUpdateAdGroupPayload payload = adGroupMutationService.updateTextAdGroup(UidAndClientId.of(uid, clientId),
                uid, gdUpdateTextAdGroup);
        checkStatusModerate(payload, StatusModerate.READY);
    }

    private void checkStatusModerate(GdUpdateAdGroupPayload payload, StatusModerate expectedStatusModerate) {
