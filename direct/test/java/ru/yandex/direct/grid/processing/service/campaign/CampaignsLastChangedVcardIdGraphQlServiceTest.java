...
import static ru.yandex.direct.grid.processing.service.campaign.CampaignsGraphQlService.LAST_CHANGED_VCARD_ID_RESOLVER_NAME;
...
public class CampaignsLastChangedVcardIdGraphQlServiceTest {
    ...
    public void initTestData() {
        ...
        vcard = steps.vcardSteps().createVcard(defaultCampaign);
        vcard.getVcardId();
    ...
    @Test
    public void testLastChangedVcardId() {
        String query = String.format(QUERY_TEMPLATE, context.getOperator().getLogin(),
                graphQlSerialize(campaignsContainer), LAST_CHANGED_VCARD_ID_RESOLVER_NAME);
                ...
                "client", ImmutableMap.of(
                        "campaigns", Map.of(
                                "rowset", Collections.singletonList(
                                        Map.of(LAST_CHANGED_VCARD_ID_RESOLVER_NAME, vcard.getVcardId())
                                )
                        )
                )
        );
