...
public abstract class ReachOutdoorBaseTest {

    protected static final CompareStrategy REACH_OUTDOOR_RESULT_STRATEGY = DefaultCompareStrategies.allFields()
    ...
    public void before() {
        ...
        OutdoorPlacement outdoorPlacement1 = outdoorPlacementWithBlocks(PAGE_ID_1,
                singletonList(
                        outdoorBlockWithOneSize(PAGE_ID_1, PAGE_1_BLOCK_ID_1)));
        OutdoorPlacement outdoorPlacement2 = outdoorPlacementWithBlocks(PAGE_ID_2,
                asList(
                        outdoorBlockWithOneSize(PAGE_ID_2, PAGE_2_BLOCK_ID_1),
                        outdoorBlockWithOneSize(PAGE_ID_2, PAGE_2_BLOCK_ID_2)));
        steps.placementSteps().clearPlacements();
        steps.placementSteps().addPlacements(outdoorPlacement1, outdoorPlacement2);

        inventoriWebService = new InventoriWebService(shardHelper, inventoriClient, cryptaService,
                authenticationSource, userService, campaignInfoCollector, inventoriService, campaignRepository,
                pricePackageService, featureService, retargetingConditionRepository, campaignWithBrandSafetyService, adGroupRepository, cryptaSegmentRepository);
    ...
    private CreativeInfo createCreative(double duration, Pair<Integer, Integer>... sizes) {
        ...
        return steps.creativeSteps().createCreative(new CreativeInfo()
                ...
                .withCreative(defaultCpmOutdoorVideoAddition(null, null)
    ...
    protected void inventoriSuccessResponse() {
        when(inventoriClient.getOutdoorPrediction(any(), any(), any(), any(), any(), any()))
                .thenReturn(new OutdoorPredictionResponse()
                        .withReach(1000L)
    ...
    protected ReachOutdoorRequest defaultRequest() {
