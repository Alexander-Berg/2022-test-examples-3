...
class CampaignWithNewIosVersionUpdateValidationTypeSupportTest {

    @InjectMocks
    lateinit var typeSupport: CampaignWithNewIosVersionUpdateValidationTypeSupport
    ...
    fun shouldValidate(featureEnabled: Boolean, campaignHasChanges: Boolean, matcher: Matcher<Any>) {
        ...
        var mc = ModelChanges(campaignId, CampaignWithNewIosVersion::class.java)
        if (campaignHasChanges) {
            mc = mc.process(true, MobileContentCampaign.IS_NEW_IOS_VERSION_ENABLED)
        }
