...
public class ReceiveBannerButtonModerationResponseTest extends AbstractModerationResponseTest
        <BannerAssetModerationMeta, Verdict, BannerAssetModerationResponse> {

    private static final long DEFAULT_VERSION = 50000L;
    ...
    public void before() {
        ...
        CpmBannerInfo cpmBannerInfo = steps.bannerSteps()
                .createActiveCpmVideoBanner(activeCpmVideoBanner(null, null, creativeId)
                        .withButtonHref("New href")
                        .withButtonAction(ButtonAction.BUY)
                        .withButtonCaption("New caption")
                        .withButtonStatusModerate(OldBannerButtonStatusModerate.SENT)
                        .withStatusModerate(OldBannerStatusModerate.YES), campaignInfo);

        this.banner = cpmBannerInfo.getBanner();
        testModerationRepository.createBannerButtonsVersion(shard, this.banner.getId(), DEFAULT_VERSION);
    }

    private BannerButtonsRecord getExpectedBannerButtons(BannerAssetModerationResponse response) {
        BannerButtonsRecord record = new BannerButtonsRecord();

        record.setBid(response.getMeta().getBannerId());
        record.setHref("New href");
        record.setCaption("New caption");
        record.setStatusmoderate(ModerationDecisionAdapter.toBannerButtonsStatusmoderate(response.getResult().getVerdict()));
    ...
    protected void checkInDbForId(long bid, BannerAssetModerationResponse response) {
        List<BannerButtonsRecord> dbBannerButtons = testModerationRepository.getBannerButtons(shard,
                Collections.singleton(bid));

        assumeThat(dbBannerButtons, not(empty()));

        BannerButtonsRecord dbRecord = dbBannerButtons.get(0);
        BannerButtonsRecord expectedBannerButtons = getExpectedBannerButtons(response);

        assertEquals(dbRecord.getStatusmoderate(), expectedBannerButtons.getStatusmoderate());
    }

    @Override
    protected ModerationReceivingService<BannerAssetModerationResponse> getReceivingService() {
        return bannerButtonModerationReceivingService;
    }
    ...
    protected long createObjectInDb(long version) {
        ...
        testModerationRepository.createBannerButtonsVersion(shard, cpmBannerInfo.getBannerId(), version);
    ...
    protected BannerAssetModerationResponse createResponse(long bid, ModerationDecision status,
        ...
        response.setType(BANNER_BUTTONS);
    ...
    protected void deleteDefaultObjectVersion() {
        testModerationRepository.deleteBannerButtonVersion(getShard(), getDefaultObjectId());
    }
}
