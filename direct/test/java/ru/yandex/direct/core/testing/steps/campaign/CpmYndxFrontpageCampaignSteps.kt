...
@Component
class CpmYndxFrontpageCampaignSteps(
...
) : CampaignSteps<CpmYndxFrontpageCampaign, CpmYndxFrontpageCampaignInfo>(dslContextProvider, campaignModifyRepository,
    campaignAddOperationSupportFacade, clientSteps, userSteps, metrikaClient) {

    override fun getCampaignInfo(): CpmYndxFrontpageCampaignInfo {
        return CpmYndxFrontpageCampaignInfo()
    }

    override fun getCampaignInfoClass(): KClass<CpmYndxFrontpageCampaignInfo> {
        return CpmYndxFrontpageCampaignInfo::class
    }

    override fun createRelations(campaignInfo: CpmYndxFrontpageCampaignInfo) {
        //костыль для обратной совместимости
        if (campaignInfo.campaign == null) {
            campaignInfo.withCampaign(TestCampaigns.activeCpmYndxFrontpageCampaign(null, null))
        }
    }
