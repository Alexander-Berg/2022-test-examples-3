...
) : CampaignSteps<CpmBannerCampaign, CpmBannerCampaignInfo>(dslContextProvider, campaignModifyRepository,
    campaignAddOperationSupportFacade, clientSteps, userSteps, metrikaClient) {

    override fun getCampaignInfo(): CpmBannerCampaignInfo {
        return CpmBannerCampaignInfo()
    }

    override fun getCampaignInfoClass(): KClass<CpmBannerCampaignInfo> {
        return CpmBannerCampaignInfo::class
    }

    override fun createRelations(campaignInfo: CpmBannerCampaignInfo) {
        //костыль для обратной совместимости
        if (campaignInfo.campaign == null) {
            campaignInfo.withCampaign(TestCampaigns.activeCpmBannerCampaign(null, null))
        }
    }
