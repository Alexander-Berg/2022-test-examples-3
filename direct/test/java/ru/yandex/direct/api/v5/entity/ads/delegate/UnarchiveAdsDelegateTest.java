...
public class UnarchiveAdsDelegateTest {
    ...
    @Autowired
    private AdsArchiveUnarchiveRequestValidator adsArchiveUnarchiveRequestValidator;

    @Autowired
    private BannerTypedRepository bannerRepository;

    @Autowired
    private BannerArchiveUnarchiveService bannerService;

    @Autowired
    private AdsArchiveUnarchiveRequestConverter requestConverter;
    ...
    private UnarchiveAdsDelegate delegate;
    ...
    public void before() {
        ...
        delegate = new UnarchiveAdsDelegate(
                bannerService,
                auth,
                adsArchiveUnarchiveRequestValidator,
                requestConverter,
                resultConverter,
    ...
    @Test
    public void unarchiveCmpAd_success() {
        ...
        checkSuccessfulUnarchive(adGroupInfo.getShard(), bannerId);
    }

    @Test
    public void unarchivePerformanceAd_success() {
        ...
        PerformanceBanner performanceBanner =
                fullPerformanceBanner(campaignId, adGroupId, creativeId).withStatusActive(true);
    ...
    private ValidationResult<List<ModelChanges<BannerWithSystemFields>>, DefectType> validate(Long bannerId) {
        ...
        var unarchiveRequest = new UnarchiveRequest().withSelectionCriteria(idsCriteria);
        List<ModelChanges<BannerWithSystemFields>> modelChanges = delegate.convertRequest(unarchiveRequest);
        return delegate.validateInternalRequest(modelChanges);
    }

    private void checkSuccessfulUnarchive(int shard, Long bannerId) {
        BannerWithSystemFields actualBanner = getBanner(shard, bannerId);
        assertThat(actualBanner.getStatusArchived()).isFalse();
    }

    private BannerWithSystemFields getBanner(int shard, Long bannerId) {
