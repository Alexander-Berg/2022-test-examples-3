...
public class ArchiveAdsDelegateTest {
    ...
    private ArchiveAdsDelegate delegate;
    ...
    public void before() {
        ...
        delegate = new ArchiveAdsDelegate(
                bannerService,
                auth,
    ...
    @Test
    public void archiveCmpAd_success() {
        ...
        checkSuccessfulArchive(adGroupInfo.getShard(), bannerId);
    }

    @Test
    public void archivePerformanceAd_success() {
        ...
        PerformanceBanner performanceBanner =
                fullPerformanceBanner(campaignId, adGroupId, creativeId).withStatusShow(false);
    ...
    private ValidationResult<List<ModelChanges<BannerWithSystemFields>>, DefectType> validate(Long bannerId) {
        ...
        var archiveRequest = new ArchiveRequest().withSelectionCriteria(idsCriteria);
        List<ModelChanges<BannerWithSystemFields>> modelChanges = delegate.convertRequest(archiveRequest);
        return delegate.validateInternalRequest(modelChanges);
    }

    private void checkSuccessfulArchive(int shard, Long bannerId) {
        List<BannerWithSystemFields> banners =
                bannerRepository.getSafely(shard, List.of(bannerId), BannerWithSystemFields.class);
        var actualBanner = banners.get(0);
        assertThat(actualBanner.getStatusArchived()).isTrue();
    }
}
