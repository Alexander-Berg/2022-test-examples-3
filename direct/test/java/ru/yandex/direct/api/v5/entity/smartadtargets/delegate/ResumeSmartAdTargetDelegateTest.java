...
public class ResumeSmartAdTargetDelegateTest {
    @Autowired
    private Steps steps;
    @Autowired
    ...
    private ResumeSmartAdTargetDelegate delegate;

    private Integer shard;
    private Long filterId;
    private PerformanceFiltersQueryFilter queryFilter;

    private static ResumeRequest createRequest(Long filterId) {
    ...
    public void before() {
        ...
        steps.performanceFilterSteps().setPerformanceFilterProperty(filterInfo, PerformanceFilter.IS_SUSPENDED, true);
        ...
        delegate = new ResumeSmartAdTargetDelegate(auth, validationService, performanceFilterService,
    ...
    public void suspend_success() {
        ...
        ResumeRequest request = createRequest(filterId);
        ResumeResponse response = genericApiService.doAction(delegate, request);
        List<ExceptionNotification> errors = response.getResumeResults().get(0).getErrors();
    ...
    public void suspend_whenFilterNotExist_failure() {
        ...
        ResumeRequest request = createRequest(notExistFilterId);
        ResumeResponse response = genericApiService.doAction(delegate, request);
        List<ExceptionNotification> errors = response.getResumeResults().get(0).getErrors();
