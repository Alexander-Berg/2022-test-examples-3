...
public class IntapiClientServiceClientExistsInBalanceTest {
    ...
    @Test
    public void addExistingClient_InvalidCurrency_Error() {
        String login = "invalidCurrencyIntapiClientTestLogin";
        Long uid = 1111122L;
        ClientId clientId = ClientId.fromLong(200000020L);
        ...
        DefectInfo expectedMessage = Result.broken(ValidationResult.failed(loginOrUid.get(), invalidValue()))
                .getErrors()
                .get(0);
        assertThatThrownBy(() -> testForUser(clientId, login, uid, RUB, RUSSIA_REGION_ID))
                .hasMessageStartingWith(expectedMessage.toString());
        verifySomeBalanceCallsNeverDone();
    }

    @Test
    public void addExistingClient_ValidCurrency_Success() {
        String login = "validCurrencyIntapiClientTestLogin";
        Long uid = 1111121L;
        ClientId clientId = ClientId.fromLong(200000019L);
        ClientsAddOrGetResponse response = testForUser(clientId, login, uid, TRY, TURKEY_REGION_ID);
    ...
    private ClientsAddOrGetResponse testForUser(ClientId clientId, String login, Long uid,
                                                CurrencyCode balanceCurrency, Long balanceRegionId) {
        mockInstancesForTest(clientId, login, uid, balanceCurrency, balanceRegionId);
    ...
    private void mockInstancesForTest(ClientId clientId, String login, Long uid,
                                      CurrencyCode balanceCurrency, Long balanceRegionId) {
        when(balanceClient.createClient(anyLong(), any())).thenReturn(clientId.asLong());
        FindClientResponseItem findClientMockedAnswer = new FindClientResponseItem()
                .withClientId(clientId.asLong());
        when(balanceClient.findClient(any())).thenReturn(List.of(findClientMockedAnswer));
        FirmCountryCurrencyItem firmCountryCurrencyMockedAnswer = new FirmCountryCurrencyItem()
                .withCurrency(balanceCurrency.name())
                .withRegionId(balanceRegionId.intValue());
        when(balanceClient.getFirmCountryCurrency(any())).thenReturn(List.of(firmCountryCurrencyMockedAnswer));
    ...
    private void verifySomeBalanceCallsNeverDone() {
        verify(balanceClient, never()).editPassport(any(), any(), any());
        verify(balanceClient, never()).createUserClientAssociation(any(), any(), any());
        verify(balanceClient, never()).getClientRepresentativePassports(any(), any());
    }
}
