...
public class MetrikaPerformanceFiltersValidationTest {
    ...
    public List<MetrikaPerformanceFiltersParam> params;
    ...
    public static Collection params() {
        return Arrays.asList(new Object[][]{
                ...
                {Collections.singletonList(new MetrikaPerformanceFiltersParam()
                        .withOrderId(CORRECT_ID)), "params[0].perf_filter_id cannot be null"},
                {Collections.singletonList(new MetrikaPerformanceFiltersParam()
                        .withPerfFilterId(CORRECT_ID)), "params[0].order_id cannot be null"},
                {Collections.singletonList(new MetrikaPerformanceFiltersParam()
                        .withPerfFilterId(INCORRECT_ID)
                        .withOrderId(CORRECT_ID)), "params[0].perf_filter_id must be greater than 0"},
                {Collections.singletonList(new MetrikaPerformanceFiltersParam()
                        .withPerfFilterId(CORRECT_ID)
                        .withOrderId(INCORRECT_ID)), "params[0].order_id must be greater than 0"},
                {Arrays.asList(new MetrikaPerformanceFiltersParam()
                        .withPerfFilterId(CORRECT_ID)
    ...
    public void setUp() throws Exception {
        metrikaPerformanceFiltersService = new MetrikaPerformanceFiltersService();
    }

    @Test
    public void expectException() {
        String validationError =
                getErrorText(metrikaPerformanceFiltersService.validate(params), path(PathHelper.field("params")));
