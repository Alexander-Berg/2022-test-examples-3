...
@Aqua.Test
@Description("Сброс статуса statusBsSynced баннера при удалении параметра для учета корректировок в URL")
...
public class DeleteCoefGoalContextIdTest extends CoefGoalContextIdBaseTest {

    public DeleteCoefGoalContextIdTest(CampaignTypeEnum campaignType, String href) {
        this.campaignType = campaignType;
        this.href = href;
        bannerRule = BannersRuleFactory.getBannersRuleBuilderByCampType(campaignType)
                .overrideBannerTemplate(new Banner().withHref(href + PARAM_WITH_COEF_GOAL_CONTEXT))
                .withUlogin(CLIENT);
    ...
    @Test
    @Description("Сброс bsSynced баннера при удалении параметра для учета корректировок в URL")
    @ru.yandex.qatools.allure.annotations.TestCaseId("9341")
    public void checkBsSyncedAfterCoefGoalContextIdDeleted() {
        super.test();
    }

    public void check() {
        BsSyncedHelper.checkGroupBsSynced(bannerRule.getCurrentGroup(), StatusBsSynced.YES);
