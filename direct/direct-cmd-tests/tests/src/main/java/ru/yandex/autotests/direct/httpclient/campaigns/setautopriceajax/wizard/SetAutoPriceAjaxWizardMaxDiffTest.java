 ...
 */
@Aqua.Test
@Description("Изменение в мастере цен в процентах от цены первого места(SetAutoPriceAjax)")
@Stories(TestFeatures.Campaigns.SET_AUTO_PRICE_AJAX)
@Features(TestFeatures.CAMPAIGNS)
@Tag(TrunkTag.YES)
@Tag(OldTag.YES)
public class SetAutoPriceAjaxWizardMaxDiffTest extends SetAutoPriceAjaxTestBase {

    public SetAutoPriceAjaxWizardMaxDiffTest() {
        super(new PropertyLoader<>(SetAutoPriceAjaxRequestParameters.class).getHttpBean("wizardAutoPrice"));
    }

    @Test
    @Description("Изменение в мастере цен в процентах от цены первого места")
    @ru.yandex.qatools.allure.annotations.TestCaseId("10395")
    public void changeWizardMaxPriceDiffTest() {
        requestParams.setWizardSearchPriceBase(PriceBase.MAX);
