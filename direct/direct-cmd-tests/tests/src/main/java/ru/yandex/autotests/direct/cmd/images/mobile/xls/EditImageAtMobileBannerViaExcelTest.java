...
@Aqua.Test
@Description("Изменение картинки в мобильном баннере через excel")
...
public class EditImageAtMobileBannerViaExcelTest extends UploadMobileBannerWithImageViaExcelBase {

    private ImageUploadHelper imageUploader2 = (ImageUploadHelper) new ImageUploadHelper().
            ...
            withClient(getClientToUploadImage());

    @Before
    public void before() {
        // загружаем вторую картинку в Директ
        imageUploader2.withBannerImageSteps(cmdRule.cmdSteps().bannerImagesSteps());
        imageUploader2.forCampaign(mobileBannersRule.getCampaignId());
        imageUploader2.uploadAndResize();
        super.before();
    }

    @Test
    @Description("Изменение картинки в мобильном баннере через excel")
    @ru.yandex.qatools.allure.annotations.TestCaseId("9880")
    public void editImageAtMobileCampViaExcel() {
        super.test();
    }
    ...
    protected Matcher<Banner> getBannerMatcher() {
        String sourceImageName = imageUploader2.getResizeResponse().getName();
    ...
    /*
        Загружаем данные из excel-файла в существующую кампанию
     */
    @Override
    ...
    protected String getUploadedImageToSendInXls() {
        return imageUploader2.getResizeResponse().getImage();
    }

    @Override
    protected String getUploadedImageGroupToSendInXls() {
        return imageUploader2.getResizeResponse().getMdsGroupId();
    }
}
