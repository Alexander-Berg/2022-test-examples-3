...
@Aqua.Test
@Description("Проверка кодов ответов БК при запросе статистики через мастер отчетов, часть 3")
@Features(TestFeatures.SHOW_STAT_RESPONSE_CHECKER)
@RunWith(Parameterized.class)
@Tag(TrunkTag.YES)
@Tag(OldTag.YES)
public class MasterOfReportsResponseChecker3Test extends MasterOfReportsBaseTest {
    @Parameterized.Parameters(name = "url = {0}")
    public static List testData() {
        try {
            List<String> it = IOUtils.readLines(MasterOfReportsBaseTest.class.getClassLoader().getResourceAsStream("url_part3.list"));
    ...
    @Test
    @Override
    @ru.yandex.qatools.allure.annotations.TestCaseId("10575")
