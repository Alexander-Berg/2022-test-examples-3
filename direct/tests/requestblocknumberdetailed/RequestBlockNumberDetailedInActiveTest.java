...
@Description("Проверка корректного подсчёта объектов в кампании с неактивным объявлением.")
public class RequestBlockNumberDetailedInActiveTest {
    protected static LogSteps log = LogSteps.getLogger(RequestBlockNumberDetailedInActiveTest.class);
    ...
    public static void prepareTestData() {
        ...
        log.info("Остановим объявления.");
        api.userSteps.bannersFakeSteps().makeBannerActive(firstAdId);
        api.userSteps.bannersFakeSteps().makeBannerActive(secondAdId);
        api.userSteps.adsSteps().adsSuspend(firstAdId);
        api.userSteps.adsSteps().adsSuspend(secondAdId);
        response =
