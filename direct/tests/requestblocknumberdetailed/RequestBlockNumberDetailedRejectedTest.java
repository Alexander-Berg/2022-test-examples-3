...
@Description("Проверка корректного подсчёта объектов в кампании с отклоненным объявлением.")
public class RequestBlockNumberDetailedRejectedTest {
    protected static LogSteps log = LogSteps.getLogger(RequestBlockNumberDetailedRejectedTest.class);
    ...
    public static void prepareTestData() {
        ...
        log.info("Создадим первую группу с 3мя фразами и 2мя объявлениями.");
        Long adGroupID = api.userSteps.adGroupsSteps().addDefaultGroup(campaignId);
        api.userSteps.keywordsSteps().addDefaultKeywords(adGroupID, 3);
        Long firstAdId = api.userSteps.adsSteps().addDefaultTextAd(adGroupID);
        Long secondAdId = api.userSteps.adsSteps().addDefaultTextAd(adGroupID);
        log.info("Отклоним оба.");
        api.userSteps.bannersFakeSteps().makeBannersDeclined(firstAdId);
        api.userSteps.bannersFakeSteps().makeBannersDeclined(secondAdId);
        response =
    ...
    public void checkDetails() {
        ...
        expectedResponse.setRejectedGroup("1");
        expectedResponse.setRejectedBlock("2");
        expectedResponse.setRejectedRequest("3");
        expectedResponse.setActiveBlock("0");
        expectedResponse.setArchiveBlock("0");
        expectedResponse.setDraftBlock("0");
        expectedResponse.setInactiveGroup("1");
        expectedResponse.setInactiveBlock("2");
        expectedResponse.setInactiveRequest("3");
        expectedResponse.setModerateBlock("0");
        expectedResponse.setActiveRequest("0");
        expectedResponse.setArchiveRequest("0");
        expectedResponse.setDraftRequest("0");
        expectedResponse.setModerateRequest("0");
        expectedResponse.setActiveGroup("0");
        expectedResponse.setArchiveGroup("0");
        expectedResponse.setDraftGroup("0");
