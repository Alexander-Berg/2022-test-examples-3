...
@Aqua.Test(title = "DostupAddRole - добавление ролей тимлидера")
@Tag(TagDictionary.RELEASE)
@Tag(TagDictionary.TRUNK)
@Features(FeatureNames.DOSTUP_ADD_ROLE)
@RunWith(Parameterized.class)
public class DostupAddRoleTeamleadersTest {
    ...
    public void dostupAddRoleAddTeamleadersTest() {
        darkSideSteps.getDostupSteps().clearRoles(DostupSteps.STAFF_LOGIN);
        String passportLogin = Logins.getRoleLogin(role);

        log.info("Сначала добавляем роль менеджера - без неё роль тимлидера не добавится");
        ...
        assertThat("В ответе должна была быть две роли(менеджера и тимлидера)",
                getUserRolesResponse.getRoles().size(), equalTo(2));
