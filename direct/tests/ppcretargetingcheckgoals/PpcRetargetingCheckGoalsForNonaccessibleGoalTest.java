...
public class PpcRetargetingCheckGoalsForNonaccessibleGoalTest {
    ...
    public void runScriptAndCheckResult() {
        api.userSteps.getDBSteps().getRetargetingGoalsSteps().setIsAccessible(
                retargetingConditionId, NONACCESSIBLE_GOAL_ID, 1, shard);

        api.userSteps.getDarkSideSteps().getRunScriptSteps().runPpcRetargetingCheckGoals(shard, clientId);

        RetargetingGoals goal = api.userSteps.getDBSteps().getRetargetingGoalsSteps().getRetargetingGoals(
                retargetingConditionId, NONACCESSIBLE_GOAL_ID, shard);
