 ...
 */
@Aqua.Test(title = "AgencyCheckForMetrica: не представитель агентства")
@Tag(TagDictionary.RELEASE)
@Tag(TagDictionary.TRUNK)
@Features(FeatureNames.METRICA_AGENCY_CHECK_FOR_METRICA)
public class AgencyCheckForMetricaNotAgentTest {
    DarkSideSteps darkSideSteps = new DarkSideSteps();

    @Test
    public void agencyCheckForMetricaNotAgencyTest() {
        ...
        expected.setMessage(
                MetricaOldTransportSteps.NON_AGENCY_CLIENT_UID + " is not agency in direct"
        );
    ...
    @Test
    public void agencyCheckForMetricaAnotherAgencyTest() {
        AgencyCheckForMetrikaResponse response =
                darkSideSteps.getMetricaOldTransportSteps().checkAgencyRightsForMetrikaNoErrors(
                        MetricaOldTransportSteps.ANOTHER_AGENCY_UID,
                        MetricaOldTransportSteps.AGENCY_UID,
                        MetricaOldTransportSteps.NON_AGENCY_CLIENT_UID
                );
        AgencyCheckForMetrikaResponse.Data expected =
        ...
        expected.setResult(0);
        expected.setMessage(
                MetricaOldTransportSteps.AGENCY_UID + " is not chief agency for " +
                        MetricaOldTransportSteps.ANOTHER_AGENCY_UID
        );
        assertThat("Данные в ответе не совпадают с ожидаемыми", response.data,
                beanEquals(expected));
