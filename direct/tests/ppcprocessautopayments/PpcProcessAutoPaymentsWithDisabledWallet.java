...
@Description("ОС отключен")
@Issue("https://st.yandex-team.ru/DIRECT-52054")
public class PpcProcessAutoPaymentsWithDisabledWallet {
    private static final String LOGIN = Logins.AUTOPAY_LOGIN13;
    ...
    public void before() {
        ...
        for (Long campaignId : campaignIds) {
            dbSteps.campaignsSteps().updateCampaigns(
                    dbSteps.campaignsSteps().getCampaignById(campaignId).setWalletCid(0L)
            );
        }
    }
    ...
    public void clear() {
        for (Long campaignId : campaignIds) {
            dbSteps.campaignsSteps().updateCampaigns(
                    dbSteps.campaignsSteps().getCampaignById(campaignId).setWalletCid(walletCid)
            );
        }
