...
@Aqua.Test
@Title("Тест статистики по всем кампаниям на логин по неделям")
@Features(TestFeatures.ALL_CAMPAIGNS_REPORT)
public class AllCampaignsLoginReportGroupByWeekTest extends AllCampaignsLoginReportTestBase {

    @Test
    @Title("Тест статистики по всем кампаниям на логин по неделям")
    public void allCampaignsLoginReportGroupByWeekXlsTest() {
        allCampaignsLoginReportXlsTest();
    }

    @Override
    protected ShowCampStatRequest getRequest() {
        return new ShowCampStatRequest().withGroup(StatGroupEnum.WEEK).withTargetAll(ONE);
    }
}
