...
@Aqua.Test
@Title("Тест статистики по всем кампаниям на логин на поиске")
@Features(TestFeatures.ALL_CAMPAIGNS_REPORT)
public class AllCampaignsLoginReportSearchTest extends AllCampaignsLoginReportTestBase {

    @Test
    @Title("Тест статистики по всем кампаниям на логин на поиске")
    public void allCampaignsLoginReportSearchTest() {
        allCampaignsLoginReportXlsTest();
    }

    @Override
    protected ShowCampStatRequest getRequest() {
        return new ShowCampStatRequest().withTarget0(ONE);
    }
}
