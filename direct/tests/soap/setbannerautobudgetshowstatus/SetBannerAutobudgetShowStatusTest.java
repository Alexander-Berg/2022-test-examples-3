...
@Aqua.Test(title = "setBannerAutobudgetShowStatus - отключение показов по автобюджету")
@Tag(TagDictionary.RELEASE)
@Tag(TagDictionary.TRUNK)
@Features(FeatureNames.SOAP_SET_BANNER_AUTOBUDGET_SHOW_STATUS)
@RunWith(Parameterized.class)
public class SetBannerAutobudgetShowStatusTest {
        ...
        Object[][] data = new Object[][]{
                {
                        ...
                        "кампания с растянутым дневным бюджетом"
                },
                {
    ...
    @Test
    public void setBannerAutobudgetShowStatusToYesTest() {
        log.info("Вызываем метод " + currentMethodName);
        darkSideSteps.getSoapClientSteps().checkSetBannerAutobudgetShowStatusNoErrors(
                currentMethodName, new SetBannerAutobudgetShowStatusRequestData().withBanner(pid, false)
        );
        log.info("Снова вызываем метод " + currentMethodName + "- меняем значение обратно");
        darkSideSteps.getSoapClientSteps().checkSetBannerAutobudgetShowStatusNoErrors(
                currentMethodName, new SetBannerAutobudgetShowStatusRequestData().withBanner(pid, true)
        );
        log.info("Проверяем, что значение autobudgetShowStatus поменялось");
        GroupFakeInfo groupFakeInfo = api.userSteps.groupFakeSteps().getGroupParams(pid);
        assertThat("Не удалось изменить значение autobudgetShowStatus", groupFakeInfo.getStatusAutobudgetShow(),
