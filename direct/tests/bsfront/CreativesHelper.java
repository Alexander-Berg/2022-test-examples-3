...
public class CreativesHelper {
    ...
    public CreativesHelper(ApiSteps apiSteps) {
        ...
        this.dbSteps = new DirectJooqDbSteps(
                DirectTestRunProperties.getInstance().getDirectHost().replace("https://", "")
        );
    }
    ...
    public PerfCreativesRecord getCreativeInBsFromDb(CreativeGroupResponse creativeGroup, String client) {
        PerfCreativesRecord perfCreatives = getApiSteps().userSteps.getDirectJooqDbSteps().useShardForLogin(client)
                .perfCreativesSteps().getPerfCreatives(creativeGroup.getFirstCreativeId());
        if (perfCreatives == null) {
            Integer uid = Integer.parseInt(getApiSteps().userSteps.clientFakeSteps().getClientData(client).getPassportID());
            getApiSteps().userSteps.getDarkSideSteps().getBsFrontSteps()
                    .changeNotifyGroupWithAssumption(uid, client, creativeGroup.getCreativeGroupId());
            perfCreatives = getApiSteps().userSteps.getDirectJooqDbSteps().useShardForLogin(client)
                    .perfCreativesSteps().getPerfCreatives(creativeGroup.getFirstCreativeId());
