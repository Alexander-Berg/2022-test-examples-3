...
@Aqua.Test(title = "NotifyOrder2 - старый таймстамп")
@Tag(TagDictionary.RELEASE)
@Tag(TagDictionary.TRUNK)
@Features(FeatureNames.JSON_BALANCE_NOTIFY_ORDER2)
public class NotifyOrder2OldTimestampTest {

    private static DarkSideSteps darkSideSteps = new DarkSideSteps();
    protected static LogSteps log = LogSteps.getLogger(NotifyOrder2OldTimestampTest.class);
    ...
    @Test
    public void notifyOrder2WithOldTimestampTest() {
        long campaignId = api.userSteps.campaignSteps().addDefaultTextCampaign();
        Money qty = Money.valueOf(100.0f, Currency.RUB);
        log.info("Вызываем метод NotifyOrder2, чтобы установить таймстамп");
        ...
        log.info("Вызываем метод NotifyOrder2 со старым таймстампом");
        darkSideSteps.getBalanceClientNotifyOrderJsonSteps().notifyOrderNoErrors(new NotifyOrder2JSONRequest()
                .withServiceId(NotifyOrder2JSONRequest.DIRECT_SERVICE_ID)
                .withServiceOrderId(campaignId)
                .withTid("1")
                .withConsumeQty(qty.addVAT().floatValue())
                .withProductCurrency(Currency.RUB.value())
        );
