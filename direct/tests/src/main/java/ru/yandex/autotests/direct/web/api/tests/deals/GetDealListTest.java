...
@Aqua.Test
@Description("Получение сделок")
@Stories(TestFeatures.Deal.GET_DEAL_LIST)
@Features(TestFeatures.DEAL)
@Tag(TrunkTag.YES)
@Tag(Tags.DEAL)
public class GetDealListTest {
    ...
    public void getDealList() {
        GetDealsListResponse response =
                dealRule.getDirectWebApiSteps().dealSteps().getDealsList(CLIENT_LOGIN);
        assertThat("в ответе флаг success должен быть равен true", response.getSuccess(), is(true));
    }

    @Test
    public void getDealListWithSort() {
        GetDealsListResponse response =
                dealRule.getDirectWebApiSteps().dealSteps().getDealsList(CLIENT_LOGIN, "name", "desc");
        assertThat("в ответе флаг success должен быть равен true", response.getSuccess(), is(true));
    }
}
