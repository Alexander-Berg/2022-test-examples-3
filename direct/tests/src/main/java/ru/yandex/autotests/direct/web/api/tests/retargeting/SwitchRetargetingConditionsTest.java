...
public class SwitchRetargetingConditionsTest {
    ...
    public void canSuspendRetargetingConditions() {
        List<SwitchRetargetingWeb> request = Collections.singletonList(new SwitchRetargetingWeb()
                .withIsSuspended(true)
                .withRetCondId(retargetingConditionRule.getRetCondId())
        );
        boolean result =
                directRule.webApiSteps().retargetingSteps()
                        .switchRetargetingConditions(request, Logins.DEFAULT_CLIENT);
