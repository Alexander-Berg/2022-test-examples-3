    describe('b-banner-preview2_view_search-desktop', function() {
    var clock,
        model,
        block,
        constStub;
    ...
    beforeEach(function() {
        ...
        block = u.getInitedBlock({
            ...
            mods: { type: 'text', view: 'search-desktop' },
            data: model.toJSON(),
            modelsParams: {
    ...
    describe('Переопределение элементов', function() {
        ['flag-age', 'flags'].forEach(function(elemName) {
            describe('b-banner-preview2__' + elemName, function() {
                describe('в поле flagsSettings установлены разные параметры addRemove и edit и в блоки предупреждений они НЕ пробрасываются (т.к. доступны только на просмотр)', function() {
                    [
                        {
                            addRemove: true,
                            edit: true
                        ...
                        it(JSON.stringify(settings), function() {
                            ...
                            expect(adWarningsBlock.params.can).to.eql(undefined);
                        });
                    });
                });
            });
