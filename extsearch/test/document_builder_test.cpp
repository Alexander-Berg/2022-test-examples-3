...
#include <extsearch/images/robot/parsers/html_parser/imagelib/document_builder.h>
#include <extsearch/images/robot/parsers/html_parser/imagelib/custom/factory.h>
...
namespace NImageLib {
    static void ProcessDocumentBuilder(const TString& url, const TString& filePath, ECharset encode, TString& result) {
        ...
        NImageLib::Document doc(writeLog);
        TNlpInputDecoder decoder(encode);
        NImageLib::TDocumentBuilder documentBuilder(url, doc, writeLog, &HtProcessor, &decoder);
        ...
        TImgHtmlParserPtr customHtmlParser = CreateImgLinkExtractor(url);

        if (customHtmlParser) {
            customHtmlParser->SetPageEncoding(encode);
            events.Numerate(*customHtmlParser, zoneImg.Data(), zoneImg.Size());
            imageRes = customHtmlParser->InsertKiwiResults(url2props);
        }
    ...
    void CompareTestResult(const TString& samplePath, const TString& result, bool isCanonize) {
        ...
        for (size_t i = 0; i != sample.size(); ++i) {
            ...
            UNIT_ASSERT_VALUES_EQUAL(sample[i], current[i]);
        }
    }
