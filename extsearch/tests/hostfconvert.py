...
def test_export(yt_stuff):
    ...
    return yt_utils.yt_test(
        ...
        input_tables=[
            TableSpec('hostfactors', table_name='hostfactors', mapreduce_io_flags=['-format', 'yamr'], sort_on_load=True)
        ],
        output_tables=[
            TableSpec('hostfactors.index', table_name='hostfactors.index', mapreduce_io_flags=['-format', 'yamr'], sort_on_read=True),
        ],
        yt_stuff=yt_stuff
    )
