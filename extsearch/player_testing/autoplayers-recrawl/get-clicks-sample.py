...
def get_recent_table(clicks_directory):
    days_ago = 1
    while not yt.exists(yt.ypath_join(clicks_directory, datetime.strftime(datetime.now() - timedelta(days_ago), '%Y%m%d'))):
        days_ago += 1
...
def read_urls_sample(table_path, sample_max_size):
    ...
    if sampling_rate < MIN_SAMPLING_RATE:
        ...
        logging.info(
            'Sampling rate is too low, setting to {}'.format(MIN_SAMPLING_RATE))
    else:
...
if __name__ == '__main__':
    ...
    url_sample = read_urls_sample(get_recent_table(
        args.clicks_directory), int(args.sample_max_size))
