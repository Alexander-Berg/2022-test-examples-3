...
from crm.agency_cabinet.documents.proto import contracts_pb2, request_pb2
...
def procedure(mocker):
    ...
    mocker.patch(
        "crm.agency_cabinet.documents.server.src.procedures.GetContractUrl",
        return_value=mock,
    )
...
async def test_returns_serialized_operation_result(handler):
    input_pb = request_pb2.RpcRequest(
        get_contract_url=contracts_pb2.GetContractUrlInput(
            agency_id=22, contract_id=42
        )
    )
    result = await handler(input_pb.SerializeToString())

    assert contracts_pb2.GetContractUrlOutput.FromString(result) == contracts_pb2.GetContractUrlOutput(
