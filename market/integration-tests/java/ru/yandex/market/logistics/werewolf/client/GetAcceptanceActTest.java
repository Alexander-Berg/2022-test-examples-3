...
@DisplayName("Генерация списка принятых заказов от партнера")
class GetAcceptanceActTest extends AbstractGetActTest {

    private static final String PATH = "document/acceptanceAct/generate";

    @Override
    protected byte[] executeQuery(RtaOrdersData data, DocumentFormat format) {
        return wwClient.generateAcceptanceAct(data, format);
    }
