...
public class TransferFitToSurplusTest extends AbstractWMSTest {
    private final Logger log = LoggerFactory.getLogger(TransferFitToSurplusTest.class);

    private final FFWfApiSteps ffWfApiSteps = new FFWfApiSteps();

    private final Item wmsItem = Item.builder().sku("TRANSFER-AUTOTEST4").vendorId(10345562)
            .article("TRANSFER-AUTOTEST4").quantity(1).build();
    ...
    public void setUp() {
        ...
        inboundId = ffWfApiSteps.createInbound("ffwfapi/tests/transfer/TransferFitToSurplusTest/inbound.json");
    ...
    @Disabled("Падает приемка поставки в WMS")
    @Test
    @DisplayName("Проверка флоу трансфера с годного на излишек")
    public void transferFitToSurplusFlowTest() {
        ...
        ApiSteps
                .Stocks()
                .verifyItemTransferedFromFitToSurplus(stockBefore, stockAfter);
    }
}
