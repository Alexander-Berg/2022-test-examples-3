...
public class PvzDeliveryLoadUnloadReopenOneOrderTest {
    ...
    @Test
    @Feature("Доставка")
    @DisplayName(value = "Тест загрузки, разгрузки заказа из-за габаритов, переоткрытия задания и успешной загрузки")
    void test() {
        apiFacade.createLockerOrder();
        apiFacade.pickupOrders();

        publicApiFacade.arriveToRoutePoint();
        var task = publicApiFacade.loadToLocker();
        String externalOrderId = task.getOrders().iterator().next().getExternalOrderId();
        publicApiFacade.unloadFromLocker(externalOrderId, PickupPointReturnReason.DIMENSIONS_EXCEEDS);
        checkOneFailedTask();
        reopen();
        publicApiFacade.arriveToRoutePoint();
    ...
    @Step("Возобновляем задание задание")
    private void reopen() {
