package ru.yandex.market.delivery.deliveryintegrationtests.wms.tests.android;
...
@DisplayName("Android: Single Order")
@Epic("Android Tests")
@Slf4j
public class SingleOrderTest extends AbstractUiTest {
    ...
    @RetryableTest
    @DisplayName("Тест отбора синглового заказа на Android")
    @ResourceLock("Тест отбора синглового заказа на Android")
    public void orderTestWithManualWave() {
        ...
        switchFromSeleniumToAppium();
        androidSteps.Login().PerformLogin();
        List<String> itemSerials = androidSteps.Order().pickMultiAssignment(areaKey, containerLabel);
        switchFromAppiumToSelenium();
