...
class ObjectInstanceAdapter @Inject constructor(
    ...
    ) = processorLogic.generateFactoryCallForObject(type.getTypeElement())

    override fun isTypeSupported(type: TypeMirror): Boolean {
        val classElement = type.getTypeElementOrNull()
        return classElement != null && processorLogic.isObject(classElement)
    }
}