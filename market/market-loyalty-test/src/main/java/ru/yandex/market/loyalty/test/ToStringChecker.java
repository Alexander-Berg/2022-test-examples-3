...
public class ToStringChecker {
    ...
    public static void checkToStringInSameModule(
            BiPredicate<ResolvedReferenceTypeDeclaration, ResolvedFieldDeclaration>... exclusions) throws IOException {
        assertThat(
    ...
    @SafeVarargs
    static Set<String> missedFields(BiPredicate<ResolvedReferenceTypeDeclaration, ResolvedFieldDeclaration>... exclusions) throws IOException {
                    ...
                    return classDeclaration
                            ...
                            .map(toString -> {
                                ...
                                return fieldsToCheck
                                        ...
                                        .filter(field -> Arrays.stream(exclusions).noneMatch(filter -> filter.test(resolved, field)))
                                        .filter(field -> stringLiterals.stream().noneMatch(literal ->
                                                //whole words only with java variable specific grammar
                                                literal.matches("(.*[^a-zA-Z_$]|^)+" + field.getName() + "([^a-zA" +
                                                        "-Z_$0-9]+.*|$)")
                                        ))
    ...
    public static BiPredicate<ResolvedReferenceTypeDeclaration, ResolvedFieldDeclaration> excludeByClasses(Class<?>... classes) {
    ...
    @SafeVarargs
    public static BiPredicate<ResolvedReferenceTypeDeclaration, ResolvedFieldDeclaration> excludeByField(Pair<Class<?>, String>... exclusions) {
        ...
        return (owner, field) -> exclusionsAsMap.getOrDefault(owner.getQualifiedName(), Collections.emptySet()).contains(field.getName());
    }
