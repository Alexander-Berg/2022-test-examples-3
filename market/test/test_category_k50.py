...
def test_shop_categories_exists(output_xml):
    ...
    assert len(actual) == len(CATEGORIES)  # вернули категори в начало, как сделать переключение на фильтрованные категории? len(actual) == len(CATEGORIES_NEED)


def test_shop_categories_ids(output_xml):
    actual = _get_tag_attr(output_xml, '/yml_catalog/shop/categories/category/@id')
    expected = [str(cat['hyper_id']) for cat in CATEGORIES]  # вернули категори в начало,...? expected = [str(cat['hyper_id']) for cat in CATEGORIES_NEED]
    assert actual == expected


def test_shop_categories_name(output_xml):
    actual = _get_tag_text(output_xml, '/yml_catalog/shop/categories/category')
    expected = [str(cat['name']) for cat in CATEGORIES]  # вернули категори в начало, как сделать переключение на фильтрованные категории? expected = [str(cat['name']) for cat in CATEGORIES_NEED]
    assert actual == expected
