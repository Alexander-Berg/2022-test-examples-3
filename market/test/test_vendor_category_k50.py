...
from market.idx.export.awaps.yatf.resources.white_vendor_category_k50 import WhiteVendorCategoryK50
...
DATA = [
    {
        ...
        'model_picture_url': "http://model_picture.ru",
    },
]
...
def white_vendor_category_k50(yt_server):
    tablepath = ypath_join(get_yt_prefix(), 'out', 'banner', 'white_vendor_category_k50')
    return WhiteVendorCategoryK50(yt_server, tablepath, DATA)
...
@pytest.fixture(scope='module')
def banner_upload_wf(yt_server, white_vendor_category_k50, categories_table):
    resources = {
        'white_category_k50': white_vendor_category_k50,
        'categories_table': categories_table
    }
    bin_flags = [
        '--input', white_vendor_category_k50.get_path(),
        '--categories-tree', categories_table.get_path(),
        '--feed', 'vendor-category-k50',
...
def test_shop_offers_ids(output_xml):
    ...
    expected = [str(cantor_pairing(row['hyper_id'], row['vendor_id'])) for row in DATA]
    assert actual == expected
...
def test_shop_offers_urls(output_xml):
    ...
    expected = ['https://market.yandex.ru/catalog--{slug}/{nid}/list?glfilter=7893318%3A{vendor_id}&hid={hid}&utm_term={utm_term}'.format(
        nid=row['nid'],
        hid=row['hyper_id'],
        slug=slug.translit(row['category_name'] + ' ' + row['vendor_name']),
        vendor_id=row['vendor_id'],
        utm_term=quote_plus(str(row['hyper_id']) + '|' + str(cantor_pairing(row['hyper_id'], row['vendor_id']))),
    ) for row in DATA]
    assert actual == expected
...
def test_shop_offers_names(output_xml):
    ...
    expected = [str(row['category_name']) + ' ' + str(row['vendor_name']) for row in DATA]
    assert actual == expected


def test_shop_offers_vendors(output_xml):
    actual = _get_tag_text(output_xml, '/yml_catalog/shop/offers/offer/vendor')
    expected = [str(offer['vendor_name']) for offer in DATA]
    assert actual == expected
