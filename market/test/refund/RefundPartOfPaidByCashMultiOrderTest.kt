...
import ru.yandex.market.tpl.courier.presentation.feature.screen.checkOrderLineView
...
class RefundPartOfPaidByCashMultiOrderTest: BaseTest() {
    ...
    private fun prepareDeliveryTasks(): Exceptional<RoutePointId> {
        ...
        firstOrderId = tasks.first().orderId.unwrap()
    ...
    @TmsLink("courier-app-251")
    @Story("Возврат заказов")
    @DisplayName("Частичный возврат после выдачи оплаченного в смене налом мульта")
    @Description("Частичный возврат после выдачи мульта оплаченного налом")
    fun refundPartOfPaidByCashMultiOrderTest() {
        ...
        Screen.onScreen<RefundConfirmationDrawerScreen> {
            ...
            Allure.step("Снимаем выделение с первого заказа") {
                ordersLineRecyclerView.checkOrderLineView(firstOrderId) {
                    click()
                }
