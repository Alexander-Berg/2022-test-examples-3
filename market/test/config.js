...
describe.skip('Configuration', function () {
    ...
    describe('auth', function () {
        ...
        it('client_secret is not enumerable', function (done) {
            var config = {
                auth: {
                    getAuthTicket: true,
                    client_id: 31337,
            ...
            initAuth(config)
                .then(function (user) {
                    var auth = user.config.config.auth;
                    var enumProps = Object.keys(auth);
                    assert.notInclude(enumProps, 'client_secret', 'client_secret must not be enumerable');
                    done();
                })
                .done();
        });
