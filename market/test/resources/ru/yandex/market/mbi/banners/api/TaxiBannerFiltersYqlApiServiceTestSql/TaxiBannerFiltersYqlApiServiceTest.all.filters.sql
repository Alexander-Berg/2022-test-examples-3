...
FROM $partner_ids AS all_partners
    ...
    INNER JOIN $state_b2b_partner AS partners_with_state
        ON all_partners.id = partners_with_state.id
    ...
    INNER JOIN $express AS express_partners
        ON all_partners.id = express_partners.id

    WHERE all_partners.id IN (1094857,1982410983,1938471982374)

UNION ALL SELECT id AS partner_id FROM $business_id;
