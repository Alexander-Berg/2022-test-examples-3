...
class BookingControllerFreeSlotsTest(
    ...
    @Test
    fun doNotShowSlotsInThePastTest() {
        ...
        val jsonResponseNoFile = getJsonResponseNoFile("/slots-in-past/response.json")
        JSONAssert.assertEquals(jsonResponseNoFile, mvcResult.response.contentAsString, JSONCompareMode.NON_EXTENSIBLE)
        verifyBasicFfwfCommunication(1, 0)
    }
    ...
    fun doNotShowSlotsInForDayAfterTomorrowForOutbound() {
        ...
        val jsonResponseNoFile =
            FileContentUtils.getFileContent("fixtures/controller/booking/free-slots/outbound-today/fulfillment/response.json")
        JSONAssert.assertEquals(jsonResponseNoFile, mvcResult.response.contentAsString, JSONCompareMode.NON_EXTENSIBLE)
        verifyBasicFfwfCommunication(1, 0)
    }
    ...
    fun doNotShowSlotsForTheNextDayOfQuotaFromForOutboundFF() {
        ...
        val jsonResponseNoFile =
            FileContentUtils.getFileContent("fixtures/controller/booking/free-slots/quota-from/fulfillment/response.json")
    ...
    fun showSlotsForTheSameDayOfQuotaFromForOutboundSC() {
        ...
        val jsonResponseNoFile =
            FileContentUtils.getFileContent("fixtures/controller/booking/free-slots/quota-from/sorting-center/response.json")
    ...
    fun showSlotsForTodayForOutboundSC() {
        ...
        val jsonResponseNoFile =
            FileContentUtils.getFileContent("fixtures/controller/booking/free-slots/outbound-today/sorting-center/response.json")
    ...
    @Test
    @DatabaseSetup(value = ["classpath:fixtures/controller/booking/free-slots/few-warehouses/before.xml"])
    fun getFewWarehousesSuccessfully() {
        setupLmsGateSchedule(
        ...
        val jsonResponseNoFile = getJsonResponseNoFile("/few-warehouses/response.json")
        JSONAssert.assertEquals(jsonResponseNoFile, mvcResult.response.contentAsString, JSONCompareMode.NON_EXTENSIBLE)
    ...
    fun emptySlotsForOutboundDueQuotaExhausted() {
        ...
        val jsonResponseNoFile = getJsonResponseNoFile("/empty-slots/response.json")
        JSONAssert.assertEquals(jsonResponseNoFile, mvcResult.response.contentAsString, JSONCompareMode.NON_EXTENSIBLE)
    ...
    @Test
    fun whenValidateSlotThenDoNotCheckQuotaTest() {
        setupLmsGateSchedule(
            warehouseIds = listOf(1, 2, 3),
            from = LocalTime.of(10, 0),
            to = LocalTime.of(14, 0),
        ...
        val mvcResult: MvcResult = performValidateSlots(params)

        val jsonResponseNoFile = getJsonResponseNoFile("/validate-slot/response.json")
        JSONAssert.assertEquals(jsonResponseNoFile, mvcResult.response.contentAsString, JSONCompareMode.NON_EXTENSIBLE)

        verifyBasicFfwfCommunication(0, 0)
    }
    ...
    @Test
    @DatabaseSetup(value = ["classpath:fixtures/controller/booking/free-slots/round-up/before.xml"])
    fun roundUpPeriodAndStepTest() {
        setupLmsGateSchedule(
            from = LocalTime.of(9, 0),
            to = LocalTime.of(18, 0),
            workingDays = setOf(LocalDate.of(2021, 5, 17))
        ...
        params.add("bookingType", BookingType.SUPPLY.toString())
        params.add("slotDurationMinutes", "60")
        params.add("calendaringStep", "21")
        params.add("from", "2021-05-17T09:00")
        params.add("to", "2021-05-17T14:51")
        params.add("supplierType", SupplierType.THIRD_PARTY.toString())
        params.add("supplierId", "101")
        params.add("takenItems", "50")
        params.add("takenPallets", "2")
        ...
        val jsonResponseNoFile = getJsonResponseNoFile("/round-up/response.json")
        JSONAssert.assertEquals(jsonResponseNoFile, mvcResult.response.contentAsString, JSONCompareMode.NON_EXTENSIBLE)
    ...
    /**
     * Свободный слот с начала периода до начала занятого слота не подойдет по длине.
     * Начало периода с 09:00 слот забронирован на 09:30 - 10:30
     */
    @Test
    @DatabaseSetup(value = ["classpath:fixtures/controller/booking/free-slots/slot-duration/before.xml"])
    fun slotDurationRuleTest() {
        setupLmsGateSchedule(
            from = LocalTime.of(9, 0),
            to = LocalTime.of(14, 0),
            workingDays = setOf(LocalDate.of(2021, 5, 17))
        ...
        params.add("calendaringStep", "30")
        params.add("from", "2021-05-17T09:00")
        params.add("to", "2021-05-17T13:00")
        params.add("supplierType", SupplierType.THIRD_PARTY.toString())
        params.add("supplierId", "101")
        params.add("takenItems", "50")
        params.add("takenPallets", "2")
        ...
        val jsonResponseNoFile = getJsonResponseNoFile("/slot-duration/response.json")
        JSONAssert.assertEquals(jsonResponseNoFile, mvcResult.response.contentAsString, JSONCompareMode.NON_EXTENSIBLE)
    ...
    fun errorIfTryingToGetSlotsWithIncorrectDuration() {
        ...
        params.add("slotDurationMinutes", "60")
        params.add("calendaringStep", "30")
        params.add("from", "2021-05-17T22:00")
        params.add("to", "2021-05-18T14:00")
        params.add("supplierType", SupplierType.THIRD_PARTY.toString())
        params.add("takenItems", "50")
        params.add("takenPallets", "2")
    ...
    @Test
    @DatabaseSetup("classpath:fixtures/controller/booking/slot-duration-settings/slot-duration-settings.xml")
    fun tryingToGetSlotsWithIncorrectDurationWithSkipValidatedSlotDuration() {
        ...
        params.add("takenItems", "50")
        params.add("takenPallets", "2")
        params.add("skipValidatedSlotDuration", "true")
    ...
    fun getFreeSlotsWithDestinationServiceId() {
        ...
        val params: MultiValueMap<String, String> = LinkedMultiValueMap()
        params.add("warehouseIds", "1")
        params.add("destinationWarehouseId", "147")
        params.add("bookingType", BookingType.SUPPLY.toString())
        params.add("slotDurationMinutes", "60")
        params.add("calendaringStep", "30")
        params.add("from", "2021-05-17T07:00")
        ...
        val response = GetQuotaResponseDto(listOf(AvailableQuotasForServiceDto(1, listOf())))
        Mockito.`when`(ffwfClientApi!!.getQuota(argThat { param -> param.destinationWarehouseId == 147L }))
            .thenReturn(response)

        performGetSlots(params)
        Mockito.verify(ffwfClientApi!!).getQuota(argThat { param -> param.destinationWarehouseId == 147L })
    }
    ...
    private fun performValidateSlots(params: MultiValueMap<String, String>): MvcResult {
        return mockMvc!!.perform(
            MockMvcRequestBuilders.get("/booking/slots-without-quota-check")
                .contentType(MediaType.APPLICATION_JSON)
                .params(params)
        ).andDo(MockMvcResultHandlers.print())
            .andExpect(MockMvcResultMatchers.status().isOk)
