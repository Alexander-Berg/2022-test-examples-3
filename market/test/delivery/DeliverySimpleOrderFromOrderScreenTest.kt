...
class DeliverySimpleOrderFromOrderScreenTest : BaseTest() {
    private lateinit var orderIdToDelivery: OrderId

    private fun prepareDeliveryTask(maxAttempts: Int = MAX_RETRY_ATTEMPTS): Exceptional<Pair<RoutePointId, OrderId>> {
        ...
        if (task.totalPrice.amount > MAX_MULTI_ORDER_TOTAL_PRICE.toBigDecimal()) {
            if (maxAttempts > 0) {
                e("DeliverySimpleOrderFromOrderScreenTest: создан заказ дороже 50000руб, пробуем еще раз")
                runBlocking {
                    delay(RETRY_DELAY.millis)
                }
                return prepareDeliveryTask(maxAttempts = maxAttempts - 1)
            }
            return failure(IllegalStateException("DeliverySimpleOrderFromOrderScreenTest: не удалось получить заказ с ценой меньше 50000 рублей"))
        }

        d("DeliverySimpleOrderFromOrderScreenTest: получили заказ стоимостью: ${task.totalPrice.amount}")

        val multiOrderId = checkNotNull(task.orderId)
        ...
        return success(Pair(routePoint.id, multiOrderId))
    }

    override fun prepareData() {
        val (routePointId, orderId) = prepareDeliveryTask().orThrow()
    ...
    @Test
    @Issue("MARKETTPLAPP-759")
    @TmsLink("courier-app-121")
    @Story("Выдача заказов")
    @DisplayName("Выдача простого оплаченного заказа из карточки в дровере")
    @Description("Выдача простого заказа из карточки в дровере")
    fun deliverySimpleOrderFromOrderScreenTest() {
