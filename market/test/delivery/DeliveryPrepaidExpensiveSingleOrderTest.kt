...
class DeliveryPrepaidExpensiveSingleOrderTest : BaseTest() {
    ...
    @TmsLink("courier-app-232")
    @Story("Выдача заказов")
    @DisplayName("Обязательность фото к одиночному предоплаченному заказу со стоимостью более 50к")
    @Description("Выдача одиночного предоплаченного заказа на сумму более 50 000р")
    fun deliveryPrepaidExpensiveSingleOrderTest() {
        Screen.onScreen<MainScreen> {
            ...
            Allure.step("Проверить наличие тега 'Проверить паспорт' на превьюшке в дровере") {
                wait(10.seconds) until {
                    orderView {
                        hasDescendant {
                            containsText(CHECK_DOCUMENT_TAG)
                        }
                    }
                }
            }
        ...
        Screen.onScreen<MultiOrderScreen> {
            ...
            Allure.step("Проверить наличие тега 'Проверить паспорт' в карточке") {
                rootSwipeView {
                    hasDescendant {
                        containsText(CHECK_DOCUMENT_TAG)
                    }
    ...
    companion object {
        private const val CHECK_DOCUMENT_TAG = "Проверьте паспорт"
    }
}