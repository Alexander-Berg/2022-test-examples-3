...
class DeliverySimpleOrderFromOrderScreenCashPaymentTest : BaseTest() {
    private lateinit var orderIdToDelivery: OrderId

    private fun prepareDeliveryTask(): Pair<RoutePointId, OrderId> {
        ...
        return Pair(routePoint.id, task.orderId)
    }

    override fun prepareData() {
        val (routePointId, orderId) = prepareDeliveryTask()
    ...
    @Test
    @Issue("MARKETTPLAPP-762")
    @TmsLink("courier-app-128")
    @Story("Выдача заказов")
    @DisplayName("Выдача простого заказа из карточки в дровере - оплата наличными")
    @Description("Выдача простого заказа из карточки в дровере с оплатой наличными")
    fun deliverySimpleOrderFromOrderScreenCashPaymentTest() {
