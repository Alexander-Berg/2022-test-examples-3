...
class ConcreteCapacityUnitAvailabilityRestrictionTest : SoftAssertionSupport() {
    private var restriction: ConcreteCapacityUnitAvailabilityRestriction? = null
    ...
    fun init() {
        ...
        restriction = ConcreteCapacityUnitAvailabilityRestriction(capacityUnitFacade!!, clientFacade!!, edgeFacade!!)
    }
    ...
    fun testIsApplicable() {
        ...
        val actual = restriction!!.isApplicable(
            clientId,
            RestrictionEntity(1, edgeId, RestrictionType.CONCRETE_CAPACITY_UNIT_AVAILABILITY_RESTRICTION)
        )
    ...
    fun testIsApplicableFalse() {
        ...
        Mockito.`when`(capacityUnitFacade!!.findCapacityUnitIdByStateAndClientId(1, clientId))
            .thenReturn(null)
        Mockito.`when`(edgeFacade!!.getByIdOrThrow(edgeId)).thenReturn(EdgeEntity(stateFromId = 1))
        Mockito.`when`(clientFacade!!.getByIdOrThrow(clientId)).thenReturn(YardClient(id = 1))
