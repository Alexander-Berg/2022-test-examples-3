...
const status = require('../../../../middleware/pp/pp-status');
 ...
 * @returns {Object}
 */
const request = (userAgent, query, settings, cookies = {}, searchData = {}, offerAccepted = false) => ({
    query,
    settings,
    searchData,
    headers: {
    ...
    cookies: {
        'svt-partner': crypto.encrypt(JSON.stringify({ offerAccepted })),
        ...cookies,
    },
});

describe('pp middleware, status', () => {
    describe('OK', () => {
        test('should be returned if Sovetnik is not installed, device is unrecognized (desktop) and name of browser is known', () => {
            const userAgents = [
                /**
    ...
    describe('UNSUPPORTED_MOBILE', () => {
        test('should be returned if Sovetnik is not installed, device is mobile and OS is not Android', () => {
            ...
            userAgents.forEach((userAgent) => {
                const req = request(userAgent, {});
                const expected = 'UNSUPPORTED_MOBILE';
                const actual = status(req);
    ...
    describe('UNSUPPORTED_DESKTOP', () => {
        test('should be returned if Sovetnik is not installed, device is not mobile and name of browser is unrecognized', () => {
            const userAgents = [
            ...
            userAgents.forEach((userAgent) => {
                const req = request(userAgent, {});
                const expected = 'UNSUPPORTED_DESKTOP';
                const actual = status(req);
    ...
    describe('ALREADY_EXISTS', () => {
        test('should be returned if Sovetnik is installed and has button', () => {
            ...
            userAgents.forEach((userAgent) => {
                const req = request(userAgent, undefined, {}, { 'svt-button': true });
                const expected = 'ALREADY_EXISTS';
                const actual = status(req);
        ...
        test('should be returned is Sovetnik is installed and has mobile view', () => {
            ...
            userAgents.forEach((userAgent) => {
                const req = request(userAgent, {}, {}, {}, { isMobile: true });
                const expected = 'ALREADY_EXISTS';
                const actual = status(req);
    ...
    describe('WITHOUT_BUTTON', () => {
        test('should be returned if Sovetnik is installed but does not have button', () => {
            ...
            userAgents.forEach((userAgent) => {
                const req = request(userAgent, undefined, {}, {});
                const expected = 'WITHOUT_BUTTON';
                const actual = status(req);
