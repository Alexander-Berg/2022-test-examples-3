...
@DisplayName("Тесты, проверяющие создание план-факта перехода статусов OUT для СЦ в статус IN для ffwf на возвратном сегменте")
class ScOutToFulfillmentInPlanFactProcessorTest : AbstractLrmReturnPlanFactTest() {

    lateinit var processor: ScOutToFulfillmentInPlanFactProcessor
    ...
    @BeforeEach
    fun setUp() {
        processor = ScOutToFulfillmentInPlanFactProcessor(
        ...
        val segment = LrmReturnSegmentEntity(
                ...
                shipment = LrmShipmentFields(
                        destination = LrmShipmentFields.Destination(
                                ShipmentDestinationType.FULFILLMENT,
                                partnerId = 2L,
                                returnSegmentId = 15L))
        )
        val nextSegment = LrmReturnSegmentEntity(
                id = 1L, lrmReturnId = 1L, lrmSegmentId = 15L, status = nextStatus,
                logisticPoint = LrmLogisticPointFields(
                        partnerId = 10L,
                        logisticPointExternalId = "13",
                        logisticPointId = 200L,
                        type = LogisticPointType.FULFILLMENT
                ),
                shipment = LrmShipmentFields(
                        destination = LrmShipmentFields.Destination(
    ...
    fun planFactCreationSuccessWithDefaultTimeout() {
        ...
        assertSoftly {
            ...
            planFact.expectedStatusDatetime shouldBe Instant.parse("2021-12-22T16:00:00Z")
            planFact.producerName shouldBe "ScOutToFulfillmentInPlanFactProcessor"
            planFact.planFactStatus shouldBe PlanFactStatus.CREATED
            planFact.processingStatus shouldBe ProcessingStatus.ENQUEUED
            planFact.scheduleTime shouldBe Instant.parse("2021-12-22T16:00:00Z")
        }
    }
    ...
    fun planFactCreationSuccessWith1HourEverydaySchedule() {
        ...
        assertSoftly {
            ...
            planFact.entityId shouldBe 1L
            planFact.expectedStatus shouldBe "IN"
            planFact.expectedStatusDatetime shouldBe Instant.parse("2021-12-16T09:00:00Z")
            planFact.producerName shouldBe "ScOutToFulfillmentInPlanFactProcessor"
            planFact.planFactStatus shouldBe PlanFactStatus.CREATED
            planFact.processingStatus shouldBe ProcessingStatus.ENQUEUED
            planFact.scheduleTime shouldBe Instant.parse("2021-12-16T09:00:00Z")
        }
    }

    @DisplayName("Не создавать план-факт, если лог.точка не fulfillment")
    @Test
    ...
    fun doNotCreatePlanFactIfExistsActive() {
        val existingPlanFact = PlanFact(
            producerName = ScOutToFulfillmentInPlanFactProcessor::class.simpleName
    ...
    @DisplayName("Проверка создания план-факта, для сегмента с статусом больше нужного")
    @Test
    fun planFactCreationNotSkipped() {
        val planFactsCaptor = argumentCaptor<List<PlanFact>>()
        val context = mockReturnSegmentEventContext(
            segmentStatus = ReturnSegmentStatus.IN,
            nextStatus = null
