...
class OnDemandCourierReceivedPlanFactProcessorTest {

    private val settingSegment = TestableSettingsService()
    private val processor = OnDemandCourierReceivedPlanFactProcessor(settingSegment)

    @Test
    @DisplayName("Расчет ожидаемого времени после получения 32 чекпоинта")
    fun calculateExpectedDatetime() {
        val expectedDeadline = FIXED_TIME.plus(OnDemandCourierReceivedPlanFactProcessor.TIMEOUT)
    ...
    @Test
    @DisplayName("Процессор неприменим если нет 32 чекпоинта в истории")
    ...
    private fun createSegment(
        ...
        segmentStatus: SegmentStatus = SegmentStatus.TRANSIT_COURIER_FOUND,
