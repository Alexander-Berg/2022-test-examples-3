...
internal class DropshipDsIntakePlanFactProcessorTest: BaseSelfSufficientPlanFactTest() {
    private val settingsService = TestableSettingsService()

    lateinit var processor: DropshipDsIntakePlanFactProcessor

    @BeforeEach
    fun setUp() {
        processor = DropshipDsIntakePlanFactProcessor(clock, planFactService, settingsService)
    }
    ...
    @DisplayName("ПФ не создается, если второй сегмент не DELIVERY")
    @ParameterizedTest
    @EnumSource(
        value = PartnerType::class,
        mode = EnumSource.Mode.EXCLUDE,
    ...
    private fun verifyPlanFact(
        ...
        assertSoftly {
            ...
            planFact.producerName shouldBe DropshipDsIntakePlanFactProcessor::class.simpleName
    ...
    private fun createOrder(): LomOrder {
        return createMkOrder(firstSegment = createDropshipSegment(), secondSegment = createMkSegment())
    }
    ...
    private fun createPlanFact(
        ...
        return PlanFact(
            ...
            producerName = DropshipDsIntakePlanFactProcessor::class.simpleName,
        ).apply { entity = waybillSegment }
    }

    companion object {
        private val EXPECTED_PLAN_TIME = Instant.parse("2022-03-17T20:59:00.00Z")
    }
}
