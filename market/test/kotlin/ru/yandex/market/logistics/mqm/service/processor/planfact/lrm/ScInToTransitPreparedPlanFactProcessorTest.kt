...
@DisplayName("Тесты, проверяющие создание план-факта перехода статусов IN в PROCESSED для СЦ на возвратном сегменте")
class ScInToTransitPreparedPlanFactProcessorTest {

    lateinit var processor: ScInToTransitPreparedPlanFactProcessor
    ...
    @BeforeEach
    fun setUp() {
        processor = ScInToTransitPreparedPlanFactProcessor(
    ...
    private fun mockReturnSegmentEventContext(
        ...
        shipmentDestinationType: ShipmentDestinationType = ShipmentDestinationType.SORTING_CENTER
    ...
    @DisplayName("Проверка создания план-факта из SC в SC")
    @Test
    fun planFactScToScCreationSuccess() {
        ...
        assertSoftly {
            ...
            planFact.expectedStatusDatetime shouldBe Instant.parse("2021-12-15T16:00:00Z")
            planFact.producerName shouldBe "ScInToTransitPreparedPlanFactProcessor"
            planFact.planFactStatus shouldBe PlanFactStatus.CREATED
            planFact.processingStatus shouldBe ProcessingStatus.ENQUEUED
            planFact.scheduleTime shouldBe Instant.parse("2021-12-15T16:00:00Z")
        }
    }

    @DisplayName("Проверка создания план-факта из SC в SHOP")
    @Test
    fun planFactScToShopCreationSuccess() {
        whenever(clock.instant()).thenReturn(DEFAULT_TIME)
        val planFactsCaptor = argumentCaptor<List<PlanFact>>()
        val context = mockReturnSegmentEventContext(shipmentDestinationType = ShipmentDestinationType.SHOP)
        processor.returnSegmentStatusWasChanged(context)
        verify(planFactService).save(planFactsCaptor.capture())
        val planFact = planFactsCaptor.firstValue.single()
        assertSoftly {
            ...
            planFact.entityId shouldBe 0L
            planFact.expectedStatus shouldBe "OUT"
            planFact.expectedStatusDatetime shouldBe Instant.parse("2021-12-14T17:00:00.00Z")
            planFact.producerName shouldBe "ScInToTransitPreparedPlanFactProcessor"
            planFact.planFactStatus shouldBe PlanFactStatus.CREATED
            planFact.processingStatus shouldBe ProcessingStatus.ENQUEUED
            planFact.scheduleTime shouldBe Instant.parse("2021-12-14T17:00:00.00Z")
        }
    }

    @DisplayName("Проверка создания план-факта из SC в DROPOFF")
    @Test
    fun planFactScToDropoffCreationSuccess() {
        whenever(clock.instant()).thenReturn(DEFAULT_TIME)
        val planFactsCaptor = argumentCaptor<List<PlanFact>>()
        val context = mockReturnSegmentEventContext(shipmentDestinationType = ShipmentDestinationType.DROPOFF)
        processor.returnSegmentStatusWasChanged(context)
        verify(planFactService).save(planFactsCaptor.capture())
        val planFact = planFactsCaptor.firstValue.single()
        assertSoftly {
    ...
    fun doNotCreatePlanFactIfLogisticPointNotPickup() {
        val context = mockReturnSegmentEventContext(
            ...
            shipmentDestinationType = ShipmentDestinationType.SORTING_CENTER)
        processor.returnSegmentStatusWasChanged(context)
    ...
    fun doNotCreatePlanFactIfExistsActive() {
        val existingPlanFact = PlanFact(
            producerName = ScInToTransitPreparedPlanFactProcessor::class.simpleName,
    ...
    companion object {
        private val DEFAULT_TIME = Instant.parse("2021-12-14T16:00:00Z")
    }
}
