...
@DisplayName("Тесты, проверяющие создание план-факта перехода статусов IN в PROCESSED для магазина на возвратном сегменте")
class ShopInToTransitPreparedPlanFactProcessorTest {

    lateinit var processor: ShopInToTransitPreparedPlanFactProcessor
    ...
    @BeforeEach
    fun setUp() {
        processor = ShopInToTransitPreparedPlanFactProcessor(
    ...
    private fun mockReturnSegmentEventContext(
        logisticPointType: LogisticPointType = LogisticPointType.SHOP,
        planFacts: List<PlanFact> = listOf(),
        segmentStatus: ReturnSegmentStatus = ReturnSegmentStatus.IN,
        shipmentDestinationType: ShipmentDestinationType = ShipmentDestinationType.SHOP
    ...
    fun planFactScToShopCreationSuccess() {
        ...
        assertSoftly {
            ...
            planFact.producerName shouldBe ShopInToTransitPreparedPlanFactProcessor::class.simpleName
    ...
    @DisplayName("Не создавать план-факт, если лог.точка не SHOP")
    @Test
    ...
    fun doNotCreatePlanFactIfExistsActive() {
        val existingPlanFact = PlanFact(
            producerName = ShopInToTransitPreparedPlanFactProcessor::class.simpleName,
