...
class OnDemandCourierFoundPlanFactProcessorTest {

    private val settingSegment = TestableSettingsService()
    private val processor = OnDemandCourierFoundPlanFactProcessor(settingSegment)

    @Test
    @DisplayName("Расчет ожидаемого времени после получения 31 чекпоинта")
    fun calculateExpectedDatetime() {
        val expectedDeadline = FIXED_TIME.plus(OnDemandCourierFoundPlanFactProcessor.TIMEOUT)
    ...
    @Test
    @DisplayName("Процессор неприменим если нет 31 чекпоинта в истории")
    ...
    private fun createSegment(
        ...
        segmentStatus: SegmentStatus = SegmentStatus.TRANSIT_COURIER_SEARCH,
