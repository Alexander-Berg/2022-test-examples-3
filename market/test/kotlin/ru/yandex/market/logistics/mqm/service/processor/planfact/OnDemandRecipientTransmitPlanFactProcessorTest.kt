...
class OnDemandRecipientTransmitPlanFactProcessorTest {

    private val settingSegment = TestableSettingsService()
    private val processor = OnDemandRecipientTransmitPlanFactProcessor(settingSegment)

    @Test
    @DisplayName("Расчет ожидаемого времени после получения 35 чекпоинта")
    fun calculateExpectedDatetime() {
        val expectedDeadline = FIXED_TIME.plus(OnDemandRecipientTransmitPlanFactProcessor.TIMEOUT)
    ...
    @Test
    @DisplayName("Процессор неприменим если нет 35 чекпоинта в истории")
    ...
    private fun createSegment(
        ...
        segmentStatus: SegmentStatus = SegmentStatus.TRANSIT_COURIER_RECEIVED,
