...
import ru.yandex.market.wms.inventory_management.dao.InventoryManagementConfigPropertyDao
...
@SpringBootApplication(scanBasePackages = ["ru.yandex.market.wms.inventory_management"])
class DatabaseConfiguration {
    @Bean
    @Primary
    fun dataSource(): DataSource {
        val dataSource = dataSourceProperties().initializeDataSourceBuilder().build()
        val initScriptContent = FileContentUtils.getFileContent("inventory-management-schema.sql")
    ...
    @Bean
    fun inventoryManagementConnection(): DatabaseDataSourceConnection {
        return IntegrationTestConfig.dbUnitDatabaseConnection("INVENTORY_MANAGEMENT", dataSource())
    }
    ...
    fun globalConfigurationDao(
        ...
        clock: Clock
    ): GlobalConfigurationDao = InventoryManagementConfigPropertyDao(securityDataProvider, jdbcTemplate, clock)
