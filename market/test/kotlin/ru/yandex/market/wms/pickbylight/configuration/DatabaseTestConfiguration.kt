...
import ru.yandex.market.wms.pickbylight.dao.PickByLightConfigPropertyDao
...
@SpringBootApplication(scanBasePackages = ["ru.yandex.market.wms.pickbylight"])
class DatabaseTestConfiguration {
    @Bean
    @Primary
    fun dataSource(): DataSource {
        val dataSource = dataSourceProperties().initializeDataSourceBuilder().build();
        val initScriptContent = FileContentUtils.getFileContent("pick-by-light-schema.sql");
    ...
    @Bean
    fun pickByLightConnection(): DatabaseDataSourceConnection {
        return IntegrationTestConfig.dbUnitDatabaseConnection("PICK_BY_LIGHT", dataSource())
    }

    @Bean
    fun globalConfigurationDao(
        securityDataProvider: ru.yandex.market.wms.shared.libs.authorization.SecurityDataProvider,
        jdbcTemplate: NamedParameterJdbcTemplate,
        clock: Clock
    ): GlobalConfigurationDao = PickByLightConfigPropertyDao(securityDataProvider, jdbcTemplate, clock)
