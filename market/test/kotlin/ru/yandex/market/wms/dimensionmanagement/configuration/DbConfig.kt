...
import ru.yandex.market.wms.dimensionmanagement.dao.implementation.DimensionManagementConfigPropertyDao
...
@SpringBootApplication(scanBasePackages = ["ru.yandex.market.wms.dimensionmanagement"])
class DbConfig {
    @Bean
    @Primary
    fun dataSource(): DataSource {
        val dataSource = dataSourceProperties().initializeDataSourceBuilder().build();
        val initScriptContent = FileContentUtils.getFileContent("dimension-management-schema.sql");
    ...
    @Bean
    fun dimensionManagementConnection(): DatabaseDataSourceConnection {
        return IntegrationTestConfig.dbUnitDatabaseConnection("DIMENSION_MANAGEMENT", dataSource())
    }
    ...
    fun globalConfigurationDao(
        ...
        clock: Clock
    ): GlobalConfigurationDao = DimensionManagementConfigPropertyDao(securityDataProvider, jdbcTemplate, clock)
