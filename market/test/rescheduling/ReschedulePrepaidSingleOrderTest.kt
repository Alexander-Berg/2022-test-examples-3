...
class ReschedulePrepaidSingleOrderTest : BaseTest() {
    ...
    @Test
    @Issue("MARKETTPLAPP-1382")
    @TmsLink("courier-app-245")
    @Story("Перенос заказов")
    @DisplayName("Перенос и восстановление одиночного предоплаченного заказа")
    @Description("Перенос простого предоплаченного заказа")
    fun reschedulePrepaidSingleOrderTest() {
        ...
        Screen.onScreen<RescheduleScreen> {
            ...
            Allure.step("Выбираем 'Нет паспорта/Нет 18 лет' и идем дальше") {
                reasonList.checkRadioButton("Нет паспорта/Нет 18 лет") {
                    isCompletelyDisplayed()

                    val button = radioButton("Нет паспорта/Нет 18 лет")
