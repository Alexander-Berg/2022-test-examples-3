...
class TerminateSignPickupTest : BaseTest() {
    ...
    private val signCancelButtonText = resources.getString(R.string.delivery_scan_discard_cancel)
    private val signConfirmButtonText = resources.getString(R.string.delivery_scan_discard_confirm)
    ...
    @TmsLink("courier-app-222")
    @DisplayName("Апп. Прерывание согласования")
    @Description("Прерывание согласования")
    fun terminateSignPickup() {
        ...
        Screen.onScreen<FinishPickupScanScreen> {
            ...
            step("Кликаем по \"$mainButtonText\"") {
                mainButton { click() }
            }
        }
        ...
        Screen.onScreen<SignPickupScreen> {
            ...
            step("Появляется всплывающее окно с кнопками: ") {
                wait until {
                    cancelButton { isDisplayed() }
                    confirmButton { isDisplayed() }
                }

                step(signCancelButtonText) {
                    cancelButton { hasText(signCancelButtonText) }
                }

                step(signConfirmButtonText) {
                    confirmButton { hasText(signConfirmButtonText) }
                }
            }

            runBlocking { delay(30000) }

            step("Нажать \"$signCancelButtonText\"") {
                cancelButton { click() }
            }
            ...
            step("Нажать \"$signConfirmButtonText\"") {
                wait until {
                    cancelButton { isDisplayed() }
                    confirmButton { isDisplayed() }
                }
