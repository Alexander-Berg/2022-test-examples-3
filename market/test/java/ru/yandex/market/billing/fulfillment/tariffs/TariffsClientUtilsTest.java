...
class TariffsClientUtilsTest {
    ...
    private static final Set<ServiceTypeEnum> TYPES_TO_IGNORE = Set.of(
    ...
    /**
     * Проверяет, что для всех тарифов из тарифницы есть соответствующая конвертация.
     * Если тарифы отсутствуют в конвертации, то либо их необходимо туда добавить, либо в случае отсутствия нужды
     * конвертировать надо их добавить в {@param TYPES_TO_IGNORE}
     */
    @Test
    void testBillingServiceTypeExistsForAllTariffs() {
        try (ExceptionCollector exceptionCollector = new ExceptionCollector()) {
            Arrays.stream(ServiceTypeEnum.values())
                    .filter(type -> !TYPES_TO_IGNORE.contains(type))
