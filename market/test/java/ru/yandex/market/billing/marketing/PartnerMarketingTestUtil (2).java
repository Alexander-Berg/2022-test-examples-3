...
import ru.yandex.market.mbi.exception.CommonRuntimeException;
...
import static ru.yandex.market.mbi.yt.YtTestUtils.readMultipleYtTables;
import static ru.yandex.market.mbi.yt.YtTestUtils.treeMapNode;
...
public class PartnerMarketingTestUtil {
    public static Yt mockYt(@Nullable InputStream dataSource) {
        ...
        if (Objects.isNull(dataSource)) {
            doThrow(new CommonRuntimeException("yt.tables().read() error"))
                    .when(tables).read(any(), any(), any(Consumer.class));

            when(cypress.exists(any(YPath.class))).thenThrow(new CommonRuntimeException("yt.cypress().exists() error"));
        } else {
