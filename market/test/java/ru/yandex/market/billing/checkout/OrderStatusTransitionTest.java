...
class OrderStatusTransitionTest extends FunctionalTest {
    ...
    private static final long ORDER_BLUE_FF_CPA_AUCTION = 1103L;
    ...
    private static final ImmutableList<Long> ALL_ORDER_IDS = ImmutableList.of(
            ORDER_ZDRAV_CITY, ORDER_BLUE_FF, ORDER_BLUE_FF_CPA_AUCTION, ORDER_BLUE_NO_FF);

    private static final ImmutableList<OrderStatus> ALL_ORDER_STATUSES = ImmutableList.copyOf(OrderStatus.values());

    private static final Pair<Long, OrderStatusTransition> ZDRAV_CITY_PROCESSING_DELIVERY =
            createParams(ORDER_ZDRAV_CITY, OrderStatus.PROCESSING, OrderStatus.DELIVERY);
    private static final Pair<Long, OrderStatusTransition> ZDRAV_CITY_FEE_CANCELLATION =
            createParams(ORDER_ZDRAV_CITY, OrderStatus.DELIVERED, OrderStatus.CANCELLED);
    private static final Pair<Long, OrderStatusTransition> ZDRAV_CITY_PROCESSING_DELIVERY_DROPSHIP =
            createParams(ORDER_ZDRAV_CITY_DROPSHIP, OrderStatus.PROCESSING, OrderStatus.DELIVERY);
    private static final Pair<Long, OrderStatusTransition> BLUE_NO_FF_DROPSHIP_PROCESSING_DELIVERY =
            createParams(ORDER_BLUE_NO_FF_SERVICE_CENTER, OrderStatus.PROCESSING, OrderStatus.DELIVERY);
    private static final Pair<Long, OrderStatusTransition>
            BLUE_NO_FF_DROPSHIP_PROCESSING_DELIVERY_DIFFERENT_SERVICE_TYPE =
            createParams(ORDER_BLUE_NO_FF_SERVICE_CENTER_DIFFERENT_SERVICE_TYPE, OrderStatus.PROCESSING,
                    OrderStatus.DELIVERY);
    private static final Pair<Long, OrderStatusTransition> BLUE_FF_PROCESSING_DELIVERY = createParams(ORDER_BLUE_FF,
            OrderStatus.PROCESSING, OrderStatus.DELIVERY);
    private static final Pair<Long, OrderStatusTransition> BLUE_FF_DELIVERY_DELIVERED = createParams(ORDER_BLUE_FF,
            OrderStatus.DELIVERY, OrderStatus.DELIVERED);
    private static final Pair<Long, OrderStatusTransition> BLUE_FF_PICKUP_DELIVERED = createParams(ORDER_BLUE_FF,
            OrderStatus.PICKUP, OrderStatus.DELIVERED);
    private static final Pair<Long, OrderStatusTransition> BLUE_FF_FEE_CANCELLATION = createParams(ORDER_BLUE_FF,
            OrderStatus.DELIVERED, OrderStatus.CANCELLED);

    private static final Pair<Long, OrderStatusTransition> CPA_AUCTION_PROCESSING_DELIVERY =
            createParams(ORDER_BLUE_FF_CPA_AUCTION, OrderStatus.PROCESSING, OrderStatus.DELIVERY);
    private static final Pair<Long, OrderStatusTransition> CPA_AUCTION_DELIVERY_DELIVERED =
            createParams(ORDER_BLUE_FF_CPA_AUCTION, OrderStatus.DELIVERY, OrderStatus.DELIVERED);
    private static final Pair<Long, OrderStatusTransition> CPA_AUCTION_PICKUP_DELIVERED =
            createParams(ORDER_BLUE_FF_CPA_AUCTION, OrderStatus.PICKUP, OrderStatus.DELIVERED);

    private static final Pair<Long, OrderStatusTransition> CPA_AUCTION_FEE_CANCELLATION =
            createParams(ORDER_BLUE_FF_CPA_AUCTION, OrderStatus.DELIVERED, OrderStatus.CANCELLED);
    private static final Pair<Long, OrderStatusTransition> BLUE_NO_FF_FEE_CANCELLATION =
            createParams(ORDER_BLUE_NO_FF, OrderStatus.DELIVERED, OrderStatus.CANCELLED);
    private static final Pair<Long, OrderStatusTransition> BLUE_NO_FF_PROCESSING_DELIVERY =
            createParams(ORDER_BLUE_NO_FF, OrderStatus.PROCESSING, OrderStatus.DELIVERY);
    private static final Pair<Long, OrderStatusTransition> BLUE_NO_FF_DELIVERY_DELIVERED =
            createParams(ORDER_BLUE_NO_FF, OrderStatus.DELIVERY, OrderStatus.DELIVERED);
    private static final Pair<Long, OrderStatusTransition> BLUE_NO_FF_PICKUP_DELIVERED =
            createParams(ORDER_BLUE_NO_FF, OrderStatus.PICKUP, OrderStatus.DELIVERED);
    private static final List<Pair<Long, OrderStatusTransition>> WITH_CHANGE_PARAMS = Stream.of(
                    ...
                    cpaAuctionDeliveredParams(),
                    cpaAuctionFeeCancellationParams(),
                    cpaAuctionDeliveryParams(),
    ...
    private static Pair<Long, OrderStatusTransition> createParams(long orderId, OrderStatus statusFrom,
                                                                  OrderStatus statusTo) {
        return Pair.of(orderId, createTransition(statusFrom, statusTo, true));
    }

    private static OrderStatusTransition createTransition(OrderStatus statusFrom, OrderStatus statusTo,
                                                          boolean mandatory) {
    ...
    private static Stream<Pair<Long, OrderStatusTransition>> cpaAuctionDeliveredParams() {
        return Stream.of(CPA_AUCTION_DELIVERY_DELIVERED, CPA_AUCTION_PICKUP_DELIVERED);
    }

    private static Stream<Pair<Long, OrderStatusTransition>> cpaAuctionFeeCancellationParams() {
        return Stream.of(CPA_AUCTION_FEE_CANCELLATION);
    }

    private static Stream<Pair<Long, OrderStatusTransition>> cpaAuctionDeliveryParams() {
        return Stream.of(CPA_AUCTION_PROCESSING_DELIVERY);
    }
    ...
    void setUp() {
        ...
        doReturn(dbOrderDao).when(eventProcessorSupport).getDbOrderDao();
    ...
    private void doUpdateStatus(Pair<Long, OrderStatusTransition> params) {
        ...
        MbiOrder order = dbOrderDao.getOrderWithItems(orderId);
    ...
    @ParameterizedTest
    @MethodSource("cpaAuctionDeliveredParams")
    @DbUnitDataSet(after = "OrderStatusTransitionCpaAuctionTest.after.csv")
    void cpaAuctionDelivered(Pair<Long, OrderStatusTransition> params) {
        doUpdateStatus(params);
    }
    ...
    @DbUnitDataSet(before = "OrderStatusTransitionZdravCityTest.after.csv", after =
            "OrderStatusTransitionZdravCityTest.after.csv")
