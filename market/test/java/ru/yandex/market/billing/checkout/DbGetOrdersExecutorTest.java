...
class DbGetOrdersExecutorTest extends FunctionalTest
    ...
    /**
     * Проверяем, что при обработке событий относящихся к чекам, наличие записи о чеке для рефанда приводит к ошибке.
     */
    ...
    @Test
    @DisplayName("Проверка обновления информации о доставке в заказе при получении события ORDER_DELIVERY_UPDATED")
    @DbUnitDataSet(
    ...
    @Test
    @DisplayName("Проверка обновления, shopOrderId=null не перезаписывает значение в order_num")
    @DbUnitDataSet(
    ...
    @Test
    @DisplayName("Проверка обновления, shopOrderId записывается в order_num")
    @DbUnitDataSet(
    ...
    void handleErrorEvent() {
        ...
        Mockito.when(supportSpy.getDbOrderDao())
    ...
    void handleIgnoredEvent() {
        ...
        GetOrderEventsStrategy goe = new GetOrderEventsStrategy(supportSpy);
        goe.process(events, DATE_2017_01_01);
        goe.process(events, DATE_2017_01_01);
    }
