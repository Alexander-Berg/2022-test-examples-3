...
public class GetOrderEventsStrategyUpdateItemsEventTest extends FunctionalTest implements ResourceUtilitiesMixin {
    ...
    @DbUnitDataSet(
            before = "db/" +
                    "GetOrderEventsStrategy_updateItems_WhenNotBilled_ChangeFee_ChangeTotal_PaidWithCashOnDelivery" +
                    ".before.csv",
            after = "db/" +
                    "GetOrderEventsStrategy_updateItems_WhenNotBilled_ChangeFee_ChangeTotal_PaidWithCashOnDelivery" +
                    ".after.csv"
    )
    void testProcessUpdateOrderItemsShouldUpdateOrderFeeValuesWhenOrderNotInBilledStatusAndPaidWithCashOnDelivery()
            throws IOException {
        List<OrderHistoryEvent> events = events(
                "processingToProcessing-with-itemsCountDecrementWithDelete-paymentCashOnDelivery.json"
        );
     ...
     * его содержимое не используется логике - items before поднимаются из базы.
     * Не все поля из таблиц используются в тесте, но инициализация в json и представление в таблицах максимально
     * синхронизировано для удобства восприятия.
     * <p>
     * До обработки события:
     * - состав заказа имеет непустое множество позиций
     * <p>
     * После обработки события:
     * - состав заказов в cpa_order_item меняется(удаляются позиции,изменяются счетчики и ПЕРЕСЧИТЫВАЮТСЯ значения комиссии в полях FEE_UE,FEE_NET_UE)
     * - в таблице cpa_order ПЕРЕСЧИТЫВАЕТСЯ общая сумма заказа (поля ITEMS_TOTAL).
     * - применяется скидка, так как тип оплаты {@link PaymentMethod#YANDEX}.
     */
    @Test
    @DbUnitDataSet(
            before = "db/GetOrderEventsStrategy_updateItems_WhenNotBilled_ChangeFee_ChangeTotal_PaidWithYandex" +
                    ".before.csv",
            after = "db/GetOrderEventsStrategy_updateItems_WhenNotBilled_ChangeFee_ChangeTotal_PaidWithYandex" +
                    ".after.csv"
    )
    void test_process_UpdateOrderItems_should_updateOrderFeeValues_when_orderNotInBilledStatusAndPaidWithYandex()
            throws IOException {
        List<OrderHistoryEvent> events = events(
    ...
    void testProcessUpdateOrderItemsShouldUpdateAnyOrderValuesWhenItemsHasNoChangeButOrderChangedAndOrderNotBilled()
            throws IOException {
        List<OrderHistoryEvent> events = events(
