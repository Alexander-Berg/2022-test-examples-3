...
/**
 * Тесты {@link GetOrderEventsStrategy}, затаргивающие логику обновления полей транзакции при событи {@link HistoryEventType#ITEMS_UPDATED}.
 */
@DbUnitDataSet(before = {"db/datasource.csv", "db/environmentZeroDelay.csv"})
class DbGetOrdersExecutor_Transactions_ItemsUpdate_Test extends FunctionalTest
     ...
     * <p>
     * Непосредственно само изменение составаа заказа, тестируется отдельно в {@link GetOrderEventsStrategy_UpdateItemsEvent_Test}.
     * <p>
     * Случай, ктогда удалось получить информацию о платеже.
     */
    @Test
    @DbUnitDataSet(
            before = {
    ...
    /**
     * Тоже что и {@link DbGetOrdersExecutor_Transactions_ItemsUpdate_Test#test_executor_when_eventIsItemsUpdated_should_updateTransactionInfo},
    ...
    void test_executor_when_prepaidOrderWithoutPayment_should_updateTransactionWithActualTotalItemsFromOrder() throws IOException {
        runTest("processingToProcessing-with-itemsCountDecrementWithDelete-prepaid-withoutPaymentId.json");
    }

    /**
     * Тест, проверяеющий, что для типа оплаты отличного от  {@link ru.yandex.market.checkout.checkouter.pay.PaymentType#PREPAID}, не происходит модификация полей транзакции.
     */
    @Test
    @DbUnitDataSet(
            before = {
    ...
    /**
     * Предоплата счетом {@link PaymentMethod#B2B_ACCOUNT_PREPAYMENT}, {@link PaymentGoal#ORDER_ACCOUNT_PAYMENT}
     */
    @Test
    @DbUnitDataSet(
            before = {
    ...
    void test_executor_when_B2BPayment_should_updateTransactionInfo() throws IOException {
        runTest("processingToProcessing-with-itemsCountDecrementWithDelete-b2b-withPaymentId.json");
    }
