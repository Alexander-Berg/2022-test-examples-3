...
import ru.yandex.market.mbisfintegration.converters.impl.yt.ShopPartnerRatingYtConverter;
...
public class ShopPartnerRatingYtImportTest extends AbstractYtImportTest {
    ...
    protected void setUp() throws Exception {
        ...
        entityType = SHOP;
        converter = new ShopPartnerRatingYtConverter();
        ...
        ytMockHelper.mockSelectSingleRow(
                ...
                "partner_id", new YTreeIntegerNodeImpl(true, IMPORTED_SHOP_ID, Map.of()),
                "rating", new YTreeDoubleNodeImpl(67.890, Map.of())
        );

        ytImporter = new ShopPartnerRatingYtImport(ytClientMock, configurationService, ytAccountPreparationService);
    }

    @Test
    void readAndConvertOneRow() {
        entityService.add(
                new Entity(IMPORTED_SHOP_ID,
                        entityType,
                        null,
                        new Account().withShopIDC((double) IMPORTED_SHOP_ID)
                )
        );
        ...
        Assertions.assertThat(this.<Account>findEntityData(IMPORTED_SHOP_ID))
                .isEqualTo(
                        new Account()
                                .withShopIDC((double) IMPORTED_SHOP_ID)
