...
public class MessageDispenserTest {
    ...
    public void testEmptyQueueGivesOutNoTasks() throws InterruptedException {
        ...
        assertFalse("Something is issued", taskIssuedLatch.await(500, TimeUnit.MILLISECONDS));
    }
    ...
    public void testIgnoreUnregisteredPartitionMessages() throws Exception {
        ...
        assertFalse("Queue has messages", latch.await(250, TimeUnit.MILLISECONDS));
    }
    ...
    public void testIfMessageProcessingFailedItWillBeRetried() throws InterruptedException {
        ...
        assertNull("Same task issued immediately", tasks.poll(250, TimeUnit.MILLISECONDS));

        FlushTask task = tasks.poll(500, TimeUnit.MILLISECONDS);
        assertNotNull("Task not issued repeatedly", task);
    ...
    public void testOnlyOneConsumerAtATime() throws InterruptedException, BrokenBarrierException {
        ...
        assertFalse("Parallel processing for consumer", latch2.await(500, TimeUnit.MILLISECONDS));
    ...
    public void testConsumerDataIsAllowedForQueueingWhileProcessing() throws Exception {
        ...
        assertNotNull("No task is issued", task);
    ...
    public void testBlocksOnInsertWhenFull() throws InterruptedException {
        ...
        assertFalse("Queue did not block on insert", latch.await(500, TimeUnit.MILLISECONDS));
    ...
    public void testQueueDoesNotBlockWhenNotEnoughDataParsed() throws InterruptedException {
        ...
        assertTrue("Query insertion is blocked", latch1.await(250, TimeUnit.MILLISECONDS));
        ...
        assertFalse("Query insertion is not blocked", latch2.await(250, TimeUnit.MILLISECONDS));
    }
    ...
    public void testClearPartitionBlocksUntilCurrentProcessingIsOver() throws Exception {
        ...
        assertFalse("Method did not block", latch2.await(250, TimeUnit.MILLISECONDS));

        latch1.countDown();

        assertTrue("Method did not unblock", latch2.await(250, TimeUnit.MILLISECONDS));

        CountDownLatch latch3 = new CountDownLatch(1);

        pollAsync(queue, task -> latch3.countDown());

        assertFalse("Queue still has messages of partition", latch3.await(250, TimeUnit.MILLISECONDS));
    }
}
