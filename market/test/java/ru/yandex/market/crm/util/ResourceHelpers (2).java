    ...
    private static Optional<Method> tryFindMethodWithTestAnnotation(Class aClass, String methodName) {
        return Arrays.stream(aClass.getMethods())
                .filter(m -> {
                    ...
                    try {
                        return null != m.getAnnotation(org.junit.jupiter.api.Test.class)
                                || null != m.getAnnotation(org.junit.jupiter.api.BeforeEach.class)
                                || null != m.getAnnotation(Test.class)
                                || null != m.getAnnotation(Before.class);
