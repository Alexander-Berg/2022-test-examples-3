...
/**
 * Вспомогательные методы для генерации случайных значений для тестов.
 */
public class Randoms {

    public static final String HEX_DICTIONARY = "0123456789abcdef";

    private static ThreadLocalRandom random() {
        return ThreadLocalRandom.current();
    }

    public static <T extends Enum<T>> T enumValue(Class<T> cls) {
    ...
    public static BigDecimal positiveBigDecimal(long maxValue) {
        return BigDecimal.valueOf(unsignedLongValue(maxValue), 2);
    }
    ...
    public static long unsignedLongValue(long maxValue) {
    ...
    public static String hex(int length) {
        ...
        for (int i = 0; i < length; ++i) {
            sb.append(HEX_DICTIONARY.charAt(random().nextInt(HEX_DICTIONARY.length())));
        }
        return sb.toString();
    }
    ...
    public static double moneyAsDouble() {
        return moneyAsDouble(5_000_000);
    }

    public static double moneyAsDouble(double bound) {
        return BigDecimal.valueOf(random().nextDouble(bound))
                .setScale(2, RoundingMode.HALF_UP)
                .doubleValue();
    }

    public static BigDecimal moneyAsBigDecimal() {
        return moneyAsBigDecimal(5_000_000);
    }

    public static BigDecimal moneyAsBigDecimal(double bound) {
        return BigDecimal.valueOf(random().nextDouble(bound))
                .setScale(2, RoundingMode.HALF_UP);
    }

    public static BigDecimal moneyAsBigDecimal(double origin, double bound) {
