...
/**
 * Tests of {@link ToPendingApproveStrategy}.
 */
public class ToPendingApproveStrategyTest extends DeepmindBaseEnhancedTrackerApproverExecutorTestClass {
    private SskuMskuStatusService sskuMskuStatusService;

    private ToPendingApproveStrategy strategySpy;
    private TrackerApproverFacade<ServiceOfferKey, ToPendingMeta, ?> facade;
    ...
    public void setUp() {
        ...
        strategySpy = Mockito.spy(new ToPendingApproveStrategy(
            session,
    ...
    public void startStepTest() {
        ...
        var meta = new ToPendingMeta().setAuthor("author1").setDescription("description1");
        ...
        Assertions.assertThat(session.issues().get(ticketKey).getComponents())
            ...
            .containsExactlyInAnyOrder("Ввод ассортимента", "1P");
        Assertions
            .assertThat(session.issues().get(ticketKey).getSummary())
            .contains("Заявка на ввод нового ассортимента");
        var description = session.issues().get(ticketKey).getDescription();
        Assertions
            .assertThat(description.get())
            .contains(meta.getDescription(), "Прошу согласовать ввод нового ассортимента",
    ...
    public void simpleRun() {
        ...
        var ticket = facade.start(List.of(new ServiceOfferKey(1, "a")), new ToPendingMeta());
    ...
    public void checkEconomicMetrics() throws IOException {
        ...
        Assertions.assertThat(metrics)
            ...
            .containsOnly(businessProcessMetric(ticket, ToPendingApproveStrategy.TYPE));
    ...
    public void simpleRunWithReopenStep() {
        ...
        Mockito.doAnswer(invok -> {
            // first run is failed
            if (!toReopenCall.get()) {
                ProcessRequest<ServiceOfferKey, ToPendingMeta, ?> request = invok.getArgument(0);
    ...
    public void checkStepTest() {
        ...
        var ticket = facade.start(shopSkuKeys, new ToPendingMeta());
    ...
    public void processStepTest() {
        ...
        SessionUtils.addExcelAttachment(session, ticket, "excel2.xlsx", secondAttachCreatedAt,
            createCorrectExcelFile(List.of(new ServiceOfferKey(222, "shop-sku-222")),
                EnrichApproveToPendingExcelComposer.HEADERS), user);
        executor.run();
        Assertions.assertThat(facade.findTicketStatus(ticket).getState()).isEqualTo(TicketState.CLOSED);
    ...
    public void processStepHeadersNotMatchTest() {
        ...
        var ticket = facade.start(keys, new ToPendingMeta());
    ...
    public void processStepStringInSupplierColumn() {
        ...
        var excelFile = createCorrectExcelFile(keys,
            ...
            .setValue(3, Headers.SHOP_SKU_KEY, "b")
            .setValue(3, Headers.SUPPLIER_ID_KEY, "aa ")
            .build();
        SessionUtils.close(session, ticket, TicketResolution.FIXED);
        SessionUtils.addExcelAttachment(session, ticket, "excel.xlsx", Instant.now().plusSeconds(1),
            excelFile, user);
        ...
        Assertions.assertThat(SessionUtils.getLastComment(session, ticket)).contains(
            MbocErrors.get().excelRowParseError(4,
                "Значение 'aa' в колонке 'ID поставщика' не является числом").toString()
        );
        Assertions.assertThat(facade.findTicketStatus(ticket).getState()).isEqualTo(TicketState.REOPENED);
    }
    ...
    public void processStepWithOnlyHeadersExcel() {
        ...
        Assertions.assertThat(SessionUtils.getLastComment(session, ticket)).contains(
            "Последний excel файл пустой: робот ожидает," +
                " что офферы будут расположены, начиная с 3-й строки (так как первые 2 занимает шапка)." +
                " Если так и должно быть то, либо отмените заявку, либо не согласовывайте ее."
        );
        Assertions.assertThat(facade.findTicketStatus(ticket).getState()).isEqualTo(TicketState.REOPENED);
    }
    ...
    public void closeWithWontFixBeforeEnrichWillChangeToInactiveTmp() {
        ...
        sskuMskuStatusService.saveSskuStatuses(
            ...
            sskuStatus(333, "shop-sku-333", OfferAvailability.INACTIVE_TMP, "comment3", null),
            sskuStatus(444, "shop-sku-444", OfferAvailability.INACTIVE_TMP, "comment4", null),
            sskuStatus(555, "shop-sku-555", OfferAvailability.INACTIVE_TMP, "comment5", null)
        );
    ...
    public void closeWithWontFixAfterReopenAndEnrich() {
        ...
        Mockito.doAnswer(invoke -> {
            // first run is failed
            if (!toReopenCall.get()) {
                ProcessRequest<ServiceOfferKey, ToPendingMeta, ?> request = invoke.getArgument(0);
                // переводим статус в решен (чтобы сэмулировать готовность тикета)
        ...
        var ticket = facade.start(shopSkuList, new ToPendingMeta());
        SessionUtils.close(session, ticket, TicketResolution.FIXED);
        // запускаем в первый раз
        executor.run();
        Mockito.verify(strategySpy, Mockito.times(1)).reopen(Mockito.any());
        ...
        Assertions.assertThat(sskuStatusRepository.find(shopSkuList).stream()
            .filter(sskuStatus -> (sskuStatus.getAvailability() == OfferAvailability.INACTIVE_TMP
                && sskuStatus.getComment().equals(UNDER_CONSIDERATION.getLiteral())))).isEmpty();
