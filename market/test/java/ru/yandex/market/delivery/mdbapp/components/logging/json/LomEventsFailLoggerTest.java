...
public class LomEventsFailLoggerTest {
    private static final Instant CREATED =
        LocalDate.of(2020, 10, 14).atStartOfDay().toInstant(ZoneOffset.ofHours(3));
    private static final Instant ENTITY_CREATED =
        LocalDate.of(2020, 10, 13).atStartOfDay().toInstant(ZoneOffset.ofHours(3));
    ...
    private LomEventsFailLogger logger;
    private Logger dataLoggerMock;


    @Before
    public void setUp() throws Exception {
        logger = spy(new LomEventsFailLogger());
        ...
        public boolean matches(String argument) {
            try {
                ...
                return Objects.nonNull(data.get("date"))
                    ...
                    && Objects.equals(
                    data.get("created"),
                    sdf.format(Date.from(CREATED))
                )
                    && Objects.equals(
                    data.get("entityCreated"),
                    sdf.format(Date.from(ENTITY_CREATED))
                )
                    ...
                    && Objects.equals(data.get("message"), MESSAGE);
