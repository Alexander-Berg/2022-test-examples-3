...
import static ru.yandex.market.fulfillment.stockstorage.client.StockStoragePreOrderRestClient.GET_AVAILABLE;
...
public class StockStoragePreOrderClientTest extends BaseIntegrationTest {

    public static final String FIXTURE_PREORDER_DIR = "fixture/preorder/";
    ...
    @Autowired
    private StockStoragePreOrderClient client;
    ...
    public void getAvailable() throws Exception {
        mockServer.expect(requestTo(buildPath(PREORDER, GET_AVAILABLE)))
                .andExpect(method(HttpMethod.POST))
                .andExpect(header(SERVICE_TICKET_HEADER, SERVICE_TICKET))
                .andExpect(checkBody(extractFileContent(FIXTURE_PREORDER_DIR + "available_request.json")))
                .andRespond(withStatus(OK)
                        .contentType(APPLICATION_JSON)
                        .body(extractFileContent(FIXTURE_PREORDER_DIR + "available_response.json")));
    ...
    public void freezeSuccessful() throws Exception {
        mockServer.expect(requestTo(buildPath(PREORDER)))
                .andExpect(method(HttpMethod.POST))
                .andExpect(header(SERVICE_TICKET_HEADER, SERVICE_TICKET))
                .andExpect(checkBody(extractFileContent(FIXTURE_PREORDER_DIR + "freeze_request.json")))
                .andRespond(withStatus(OK));
    ...
    public void unfreeze() throws StockStorageFreezeNotFoundException {
        mockServer.expect(requestTo(buildPath(PREORDER, orderId).concat("?cancel=false")))
    ...
    public void unfreezeCancel() throws StockStorageFreezeNotFoundException {
        mockServer.expect(requestTo(buildPath(PREORDER, orderId).concat("?cancel=true")))
                .andExpect(method(HttpMethod.DELETE))
                .andExpect(header(SERVICE_TICKET_HEADER, SERVICE_TICKET))
                .andRespond(withStatus(OK));
