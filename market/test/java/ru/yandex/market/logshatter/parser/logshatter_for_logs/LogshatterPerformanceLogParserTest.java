...
public class LogshatterPerformanceLogParserTest {
    private static final LogParserChecker CHECKER = new LogParserChecker(new LogshatterPerformanceLogParser());
    private static final SimpleDateFormat SIMPLE_DATE_FORMAT = new SimpleDateFormat("yyyy-MM-dd'T'hh:mm:ss.SSS");

    @Test
    public void parse() throws Exception {
        URL resource = getClass().getClassLoader().getResource("logshatter-performance.log");
        String line = FileUtils.readLines(new File(resource.toURI())).get(0).toString();
        resource = getClass().getClassLoader().getResource("logshatter-performance-rest.json");
        ...
        CHECKER.check(
            ...
            "", // trace_id
            "", // span_id
            "ride_tech_logparser RideTechLogParser", // component
            UUID.fromString("ceac2278-7256-3cc6-b7dc-b31a1bf5bb60"), // record_id
            "", // validation_err
            rest // rest
        );
    }
