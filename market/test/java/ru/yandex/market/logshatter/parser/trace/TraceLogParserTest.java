...
import ru.yandex.market.health.configs.logshatter.url.Page;
...
public class TraceLogParserTest {
    ...
    private static final Date LOG_LINE_DATE_ALTERNATIVE = new Date(1542621750230L);
    private static final String CHECKER_HOST = "hostname.test";
    private static final Object[] LOG_LINE_FIELDS = {
        1471611275470L,
    ...
    @BeforeEach
    public void setup() {
        checker = new LogParserChecker(new TraceLogParser());
        checker.setHost(CHECKER_HOST);
    ...
    public void testParse(String expectedQueryParams) throws Exception {
        String line = LOG_LINE +
            ...
            "\tprotocol=http" +
            "\thttp_method=GET" +
            "\tquery_params=/buker/GetCards?collection=facts&ids=12518941&secret=" +
        ...
        expectedFields[QUERY_PARAMS] = expectedQueryParams;
    ...
    @Test
    public void testParseWithoutMask() throws Exception {
        checker.setParam("sanitizer", "false");
        testParse("/buker/GetCards?collection=facts&ids=12518941&secret=" +
            SnooperSanitizerTest.STRING_WITH_RAW_SECRET);
    }

    @Test
    public void testParseWithMask() throws Exception {
        testParse("/buker/GetCards?collection=facts&ids=12518941&secret=" +
    ...
    public void testParseAlternative() throws Exception {
        String line =
            "tskv" +
                ...
                "\tretry_num=3" +
                "\tduration_ms=17" +
                "\terror_code=DeliveryCalc response error (ext. type=PROTOCOL_ERROR code=501): " +
                "market/report/src/external_requester.cpp:688: HTTP request to full://localhost:37716/feedOffers " +
                "failed. Error:request failed(HTTP/1.1 501 Not Implemented) 17 ms elapsed" +
                "\tprotocol=http" +
                "\thttp_method=GET" +
                "\tkv.in.weight=4.5" +
                "\tkv.in.feed_id=-1" +
                "\tkv.in.program_type=4" +
                "\tkv.in.raw_message=U05BUEMAAABPJERDT1JHAAAAEP8RASgBGO2vDiIzCgAhAAUBCBJAKQUIDAA8QDEJCQg" +
                "+QDkFCUiAQUBIBGj///////////8BKJEB" +
                "\tkv.in.height=30" +
                "\tkv.in.length=35" +
        ...
        expectedFields[ERROR_CODE] = "DeliveryCalc response error (ext. type=PROTOCOL_ERROR code=501): " +
            "market/report/src/external_requester.cpp:688: HTTP request to full://localhost:37716/feedOffers failed. " +
            "Error:request failed(HTTP/1.1 501 Not Implemented) 17 ms elapsed";
        expectedFields[PROTOCOL] = "http";
        expectedFields[HTTP_METHOD] = "GET";
        expectedFields[QUERY_PARAMS] = "";
        expectedFields[YANDEX_UID] = "";
        expectedFields[YANDEX_LOGIN] = "";
        expectedFields[KV_KEYS] = new String[]{"in.weight", "in.feed_id", "in.program_type", "in.raw_message", "in" +
            ".height", "in.length", "saved_response", "in.generation_id",
            "in.width", "in.warehouse_id", "out.post_bucket_ids", "out.delivery_opt_bucket_ids", "in.cargo_types",
            "out.pickup_bucket_ids"};
        expectedFields[KV_VALUES] = new String[]{"4.5", "-1", "4",
            "U05BUEMAAABPJERDT1JHAAAAEP8RASgBGO2vDiIzCgAhAAUBCBJAKQUIDAA8QDEJCQg+QDkFCUiAQUBIBGj///////////8BKJEB",
    ...
    public void testParseOk() throws Exception {
        checker.check(
            "tskv\tdate=2016-09-16T11:38:07.528+03:00\trequest_id=1471611275470/568000474bbc5fdff0c3fb5b829cbbbb/20" +
                "\ttype=OUT\trequest_method=getFiltersDescription\thttp_method=GET\terror_code=RETRIES_LIMIT_EXCEEDED" +
                "->HTTP_CLIENT_REQUEST_ERROR->ENOTFOUND\tprotocol=http\tquery_params=/buker/GetCards?collection" +
                "=filters-description&format=json&hid=7812073&ids=glprice%2C7893318%2C7925349%2C8220902%2C7957713" +
                "%2C12687116%2C12849867%2C12602446%2C8224557%2C12601845%2C7957718%2C12849913%2C12849891%2C12849876" +
                "%2C13956715%2Cmanufacturer_warranty%2Constock%2Cqrfrom%2Cfree-delivery%2Coffer-shipping" +
                "%2Chome_region%2Cdelivery-interval%2Cfesh%2Cprepay-enabled%2Cshow-book-now-only\ttarget_module" +
                "=market_kgb_buker\ttarget_host=1mslb.tst.vs.market.yandex.net\tretry_num=2\tduration_ms=42"
        );
    ...
    public void testParseWithSourceFields() throws Exception {
        ...
        args[QUERY_PARAMS] = "ip=127.0.1.1&place=offerinfo&feed_shoffer_id=200305173-7&fesh=10207612&rids=213&regset" +
            "=1&pp=18&show-booking-outlets=0&adult=1&numdoc=1&showdiscounts=1&cpa-category-filter=0" +
            "&strip_query_language=0&show-promoted=1&show-min-quantity=1&show-urls=decrypted&client=checkout&co-from" +
            "=checkouter";
        ...
        mockitoChecker.check(
            "tskv" +
                ...
                "\tquery_params=ip=127.0.1.1&place=offerinfo&feed_shoffer_id=200305173-7&fesh=10207612&rids=213" +
                "&regset=1&pp=18&show-booking-outlets=0&adult=1&numdoc=1&showdiscounts=1&cpa-category-filter=0" +
                "&strip_query_language=0&show-promoted=1&show-min-quantity=1&show-urls=decrypted&client=checkout&co" +
                "-from=checkouter" +
                "\tyandex_uid=25096041" +
                "\tretry_num=1" +
                "\ttime_millis=268" +
