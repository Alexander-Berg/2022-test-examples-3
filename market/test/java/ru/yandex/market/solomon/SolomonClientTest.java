...
public class SolomonClientTest {
    ...
    public void solomonIdLimitExceeded() {
        ...
        Assert.assertTrue(SolomonClient.getShardId("market-tst", "stable",
            toExceedLimitService).length() < shardIdLengthLimit);
        Assert.assertTrue(SolomonClient.getShardId("market-tst", "stable", okService).length() < shardIdLengthLimit);
    }
