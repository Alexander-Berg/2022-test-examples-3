package ru.yandex.market.tpl.common.excel.matchers;
...
public class ExcelSheetMatcher extends TypeSafeMatcher<byte[]> {
    ...
    public static class CellMatcher<T> implements SelfDescribing {
        ...
        public static CellMatcher<LocalDateTime> localDateTimeCell(
                int column,
                int row,
                Matcher<? super LocalDateTime> matcher
        ) {
            return CellMatcher.<LocalDateTime>builder(matcher, Type.LOCAL_DATE_TIME)
                    .setRow(row)
                    .setColumn(column)
                    .build();
        }

        public static CellMatcher.Builder<LocalDateTime> localDateTimeCell(
                int column,
                Matcher<? super LocalDateTime> matcher
        ) {
            return CellMatcher.<LocalDateTime>builder(matcher, Type.LOCAL_DATE_TIME)
        ...
        private Object extractValue(XSSFSheet sheet) {
            XSSFRow row = sheet.getRow(this.row - 1);
            if (row == null) {
                return null;
            }
