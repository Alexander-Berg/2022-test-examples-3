...
class MvrpResponseMapperTest {
    ...
    private final ObjectMapper objectMapper = ObjectMappers.TPL_DB_OBJECT_MAPPER;
    ...
    void fullResultCheck() throws Exception {
        ...
        RoutingResult correctResult = TplCoreTestUtils.mapFromResource("/vrp/vrp_mvrp_response_mapped.json",
                RoutingResult.class);
    ...
    @Test
    void mappingRawResponseWithClientReturn() throws Exception {
        when(
                depotSettingsProvider.findPropertyValueForSortingCenterOrDefault(
                        any(), any())
        ).thenReturn(false);
        var clientReturnTaskId = 54438L;

        RoutingRequest request = readRequest("/vrp/vrp-with-client-returns/vrp-request.json");
        MvrpResponse response = readResponse("/vrp/vrp-with-client-returns/vrp-response.json");
        ...
        assertThat(
                shift.getRoutePoints().stream()
                        .map(RoutingResponseRoutePoint::getItems)
                        .flatMap(Collection::stream)
                        .filter(it -> it.getSubTaskIds().contains(clientReturnTaskId))
                        .allMatch(it -> it.getSubTasks().size() == 1 &&
                                RoutingRequestItemType.CLIENT_RETURN.equals(it.getSubTasks().get(0).getType()))
        ).isEqualTo(true);
    }
    ...
    private MvrpResponse readResponse(String filename) throws Exception {
        return TplCoreTestUtils.mapFromResource(filename, MvrpResponse.class);
    }

    private RoutingRequest readRequest(String filename) throws Exception {
        return TplCoreTestUtils.mapFromResource(filename, RoutingRequest.class);
    }
