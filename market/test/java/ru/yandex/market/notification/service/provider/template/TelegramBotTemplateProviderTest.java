...
public class TelegramBotTemplateProviderTest extends BaseTemplateProviderTest {
    private static final NotificationTransport TRANSPORT = NotificationTransport.TELEGRAM_BOT;
    ...
    public void provideTextContent() {
        var templateService =
                prepareTemplateServiceMock(TEMPLATE, GENERAL_HTML_TEMPLATE, COMMON_TEMPLATE, TRANSPORT);

        var provider = new TelegramBotTemplateProvider(templateService);
    ...
    public void provideHtmlContent() {
        ...
        var provider = new TelegramBotTemplateProvider(templateDao);
        var mbiTemplate = checkAndCastTemplate(provider.provide(getContext(TRANSPORT)));
