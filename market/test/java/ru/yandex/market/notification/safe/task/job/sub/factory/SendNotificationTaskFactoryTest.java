...
import ru.yandex.market.notification.simple.util.callable.ConstantCallable;
...
public class SendNotificationTaskFactoryTest {
    ...
    public void testCreateSendTaskWithoutAddresses() throws Exception {
        ...
        assertThat(
            taskFactory.createSendTask(createNotification(MBI_WEB_UI), Collections.emptySet()),
            instanceOf(SendPersistentNotificationWithoutAddressTask.class)
        );

        final Callable<SendResultInfo> task =
            taskFactory.createSendTask(createNotification(EMAIL), Collections.emptySet());

        assertThat(task, instanceOf(ConstantCallable.class));
    ...
    public void testCreateSendTaskToAddress() throws Exception {
        ...
        assertThat(
            taskFactory.createSendTask(createNotification(MBI_WEB_UI), Collections.singleton(createAddress())),
            instanceOf(SendPersistentNotificationToAddressTask.class)
        );
    }
    ...
    public void testCreateSendTaskToAddresses() throws Exception {
        ...
        assertThat(
            taskFactory.createSendTask(createNotification(MBI_WEB_UI), asList(createAddress(), createAddress())),
            instanceOf(SendPersistentNotificationToAddressesTask.class)
        );
    }
