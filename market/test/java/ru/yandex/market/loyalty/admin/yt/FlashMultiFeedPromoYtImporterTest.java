...
public class FlashMultiFeedPromoYtImporterTest extends MarketLoyaltyAdminMockedDbTest {
    ...
    private static final long ANOTHER_ONE_FEED_ID = 123456;
    ...
    public void initMocks() {
        ...
        typicalDetails = promoDetails(
                ...
                blueFlashDetails(
                        BlueFlashDSL.item(
                                BlueFlashDSL.offer(FEED_ID, SSKU_1),
                                BlueFlashDSL.fixedPrice(10),
                                BlueFlashDSL.quantityBudget(1000)
                        ),
                        BlueFlashDSL.item(
                                BlueFlashDSL.offer(ANOTHER_FEED_ID, SSKU_2),
                                BlueFlashDSL.fixedPrice(20),
                                BlueFlashDSL.quantityBudget(1000)
                        ),
                        BlueFlashDSL.item(
                                BlueFlashDSL.offer(ANOTHER_ONE_FEED_ID, SSKU_3),
                                BlueFlashDSL.fixedPrice(30),
                                BlueFlashDSL.quantityBudget(1000)
                        ),
                        BlueFlashDSL.item(
                                BlueFlashDSL.offer(FEED_ID, SSKU_4),
    ...
    private void prepareData(PromoYtTestHelper.YtreeDataBuilder dataBuilder) {
        ...
        dataBuilder
                .promo(0, SOME_PROMO_KEY, typicalDetails.clone());
    }
    ...
    public void shouldImportFlashPromoDescription() {
        ...
        assertThat(promoDescriptions, hasSize(1));
        assertThat(promoDescriptions, hasItems(
                allOf(
                        hasProperty("feedId", is(0L)),
                        hasProperty("promoKey", is(SOME_PROMO_KEY)),
                        hasProperty("status", is(FlashPromoStatus.ACTIVE))
                )
        ));

        FlashPromoDescription flashPromoDescription = promoDescriptions.iterator().next();
    }
}
