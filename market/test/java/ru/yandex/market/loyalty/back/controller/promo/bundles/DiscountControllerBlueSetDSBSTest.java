...
public class DiscountControllerBlueSetDSBSTest extends MarketLoyaltyBackMockedDbTestBase {
    ...
    public void prepare() {
        bundleService.createPromoBundle(bundleDescription(
                ...
                item(
                        condition(blueSet(
                                FEED_ID,
                                proportion(OFFER_1, 10),
                                proportion(OFFER_2, 20)
                        )),
                        primary()
                ),
                item(
    ...
    @Test
    public void shouldApplyBiggestBundleDiscountOnDsbsOrder() {
        OrderWithBundlesRequest order = orderRequestWithBundlesBuilder()
                .withPlatform(MarketPlatform.WHITE)
                .withOrderItem(
                        itemKey(FEED_ID, OFFER_1),
