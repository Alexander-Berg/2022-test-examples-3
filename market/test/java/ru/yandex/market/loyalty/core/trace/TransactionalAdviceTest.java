...
public class TransactionalAdviceTest {
    ...
    public void checkOnlyWantedPropagationTypesUsed() {
        List<Method> methods = SourceScanner.findSpringBeans("ru.yandex.market.loyalty.core")
        ...
        List<Method> unwanted = methods
                .stream()
                .filter(m -> !WANTED_PROPAGATION.contains(m.getAnnotation(Transactional.class).propagation()))
                .collect(Collectors.toList());
