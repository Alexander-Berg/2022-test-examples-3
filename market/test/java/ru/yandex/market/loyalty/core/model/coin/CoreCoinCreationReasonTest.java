package ru.yandex.market.loyalty.core.model.coin;

import org.hamcrest.Matchers;
import org.junit.Test;

import ru.yandex.market.loyalty.api.model.coin.CoinCreationReason;

import java.util.Arrays;
import java.util.stream.Collectors;

import static org.hamcrest.MatcherAssert.assertThat;
import static org.hamcrest.Matchers.containsInAnyOrder;

/**
 * @author <a href="mailto:maratik@yandex-team.ru">Marat Bukharov</a>
 */
public class CoreCoinCreationReasonTest {

    @Test
    public void shouldAllApiCoinTypeValuesMapped() {
        assertThat(
                Arrays.stream(CoreCoinCreationReason.values())
                        .map(CoreCoinCreationReason::getApiType)
                        .collect(Collectors.toList()),
                containsInAnyOrder(
                        Arrays.stream(CoinCreationReason.values())
                                .filter(t -> t != CoinCreationReason.UNKNOWN)
                                .map(Matchers::equalTo)
                                .collect(Collectors.toList())
                )
        );
    }
}
