...
class GetInboundDetailsControllerTest extends IntegrationTest {
    ...
    public void getDetails() throws Exception {
        ...
        //then
        mockMvc.perform(MockMvcRequestBuilders.post("/api/logistic/getInboundDetails")
                .header(HttpHeaders.CONTENT_TYPE, APPLICATION_XML_VALUE)
                .content(FileContentUtils.getFileContent("api/inbound/request/get-inbound-details.xml")))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.TEXT_XML_VALUE))
                .andExpect(
                        content().xml(
                                FileContentUtils.getFileContent("api/inbound/response/get-inbound-details.xml")
                        ));
    }

    private void setupResponse() {
        stubFor(post(urlEqualTo("/receiving/INFOR_SCPRD_wmwhse1/inbound/get-inbound-details"))
                .withRequestBody(containing("a-1611242344"))
                .withHeader(HttpHeaders.CONTENT_TYPE, containing(MediaType.APPLICATION_JSON_VALUE))
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                        .withBody(FileContentUtils.getFileContent("api/inbound/get-inbound-details.json"))
                )
        );
    }
}
