...
class GetOrderStatusControllerTest extends IntegrationTest {

    @Test
    public void getOrderStatus() throws Exception {
        setupResponse();

        mockMvc.perform(MockMvcRequestBuilders.post("/api/logistic/order-status")
                        .header(HttpHeaders.CONTENT_TYPE, APPLICATION_XML_VALUE)
                        .content(FileContentUtils.getFileContent(
                                "api/logistics/server/getOrderStatus/request.xml")))
                ...
                .andExpect(
                        content().xml(
                                FileContentUtils.getFileContent(
                                        "api/logistics/server/getOrderStatus/response.xml")
                        ));
    }

    private void setupResponse() {
        stubFor(post(urlEqualTo("/api/INFOR_SCPRD_wmwhse1/order-status"))
                .withRequestBody(containing("a-1611242346"))
                .withHeader(HttpHeaders.CONTENT_TYPE, containing(MediaType.APPLICATION_JSON_VALUE))
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
                        .withBody(FileContentUtils.getFileContent(
                                "api/logistics/server/getOrderStatus/wms-api-response.json"))
                )
        );
    }
}
