...
import ru.yandex.market.wms.scheduler.dao.CleanIdDao;
...
class CleanIdServiceTest extends SchedulerIntegrationTest {
    ...
    void executeWhenSomeKeysFoundTest() throws InterruptedException {
        ...
        setUpDaoMock(KEY_LIST_1, KEY_LIST_2, 3);
    ...
    void executeWhenDeletionNotAllowedTest() throws InterruptedException {
        ...
        String actualResult = cleanIdService.execute();
        String expectedResult = "Deleting in ID not enabled";
        Assertions.assertEquals(expectedResult, actualResult);
    ...
    private void setUpConfigServiceMock(int deletionEnabled, int partitionSize) {
        ...
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_ENABLED"),
                anyInt(), anyInt(), anyInt())).thenReturn(deletionEnabled);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_BATCH_SIZE"),
                anyInt(), anyInt(), anyInt())).thenReturn(defaultBatchSize);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_PART_SIZE"),
                anyInt(), anyInt(), anyInt())).thenReturn(partitionSize);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_ITERATION_TIME"),
                anyInt(), anyInt(), anyInt())).thenReturn(defaultIterationTime);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_TIME_LIMIT"),
                anyInt(), anyInt(), anyInt())).thenReturn(DEFAULT_TIME_LIMIT);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_SLEEP_TIME"),
                anyInt(), anyInt(), anyInt())).thenReturn(defaultSleepTime);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_FAILURE_SLEEP_TIME"),
                anyInt(), anyInt(), anyInt())).thenReturn(defaultFailSleepTime);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_DELETE_TIMEOUT"),
                anyInt(), anyInt(), anyInt())).thenReturn(defaultDeleteTimeout);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_SELECT_TIMEOUT"),
                anyInt(), anyInt(), anyInt())).thenReturn(defaultSelectTimeout);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_RETRY_ATTEMPTS"),
    ...
    private void verifyDbConfigServiceMock(int getConfigTimes) {
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_ENABLED"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_BATCH_SIZE"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_PART_SIZE"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_ITERATION_TIME"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_TIME_LIMIT"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_SLEEP_TIME"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_FAILURE_SLEEP_TIME"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_DELETE_TIMEOUT"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_SELECT_TIMEOUT"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_EMPTY_ID_RETRY_ATTEMPTS"), anyInt(), anyInt(), anyInt());
    }

    private String getResult(int deletedRowsCounter, int failedAttemptsNumber, long stopExecutionTime) {
        return String.format("Deleted %s rows in ID%s%s", deletedRowsCounter,
