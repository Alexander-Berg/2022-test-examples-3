...
class CleanIdDaoTest extends SchedulerIntegrationTest {

    private static final int DEFAULT_BATCH_SIZE = 10000;
    private static final int DEFAULT_SELECT_TIMEOUT = 10;
    private static final int DEFAULT_DELETE_TIMEOUT = 3;
    private static final int DEFAULT_ARCHIVE_DAYS_THRESHOLD = 3;
    private static final List<Integer> LIST_TO_DELETE = List.of(1, 2, 3, 4, 13);

    @Autowired
    private CleanIdDao dao;

    @Test
    @DatabaseSetup(value = "/db/dao/clean/clean-id/before.xml", connection = "wmwhseConnection")
    @ExpectedDatabase(value = "/db/dao/clean/clean-id/before.xml",
            connection = "wmwhseConnection", assertionMode = NON_STRICT_UNORDERED)
    void getEventIdsForDeleteTest() {
        List<Integer> eventIds = dao.getKeyList(DEFAULT_BATCH_SIZE, DEFAULT_SELECT_TIMEOUT,
    ...
    @Test
    @DatabaseSetup(value = "/db/dao/clean/clean-id/before.xml", connection = "wmwhseConnection")
    @ExpectedDatabase(value = "/db/dao/clean/clean-id/after.xml",
            connection = "wmwhseConnection", assertionMode = NON_STRICT_UNORDERED)
