...
import ru.yandex.market.wms.scheduler.dao.CleanErrorLogsDao;
...
public class CleanErrorLogsServiceTest extends SchedulerIntegrationTest {
    ...
    @InjectMocks
    CleanErrorLogsService cleanErrorLogsService;
    ...
    void executeWhenSomeKeysFoundTest() throws InterruptedException {
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_BATCH_SIZE"),
        ...
        String actualResult = cleanErrorLogsService.execute();
    ...
    void executeWhenDeletionNotAllowedTest() throws InterruptedException {
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_ALLOWED"),
                anyInt(), anyInt(), anyInt())).thenReturn(DELETION_NOT_ALLOWED);

        String actualResult = cleanErrorLogsService.execute();
        String expectedResult = "Deletion in ERRLOG not allowed";
        Assertions.assertEquals(expectedResult, actualResult);
    ...
    private void setUpConfigServiceMock() {
        ...
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_ARCHIVE_DAYS"),
                anyInt(), anyInt(), anyInt())).thenReturn(DEFAULT_ARCHIVE_DAYS);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_SAMPLE_SIZE"),
                anyInt(), anyInt(), anyInt())).thenReturn(DEFAULT_SAMPLE_SIZE);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_EXEC_TIME"),
                anyInt(), anyInt(), anyInt())).thenReturn(DEFAULT_TIME_LIMIT);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_SLEEP_TIME"),
                anyInt(), anyInt(), anyInt())).thenReturn(DEFAULT_TIME_LIMIT / 2);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_SELECT_TIMEOUT"),
                anyInt(), anyInt(), anyInt())).thenReturn(DEFAULT_SELECT_TIMEOUT);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_DELETE_TIMEOUT"),
                anyInt(), anyInt(), anyInt())).thenReturn(DEFAULT_DELETE_TIMEOUT);
        when(dbConfigService.getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_RETRY_ATTEMPTS"),
    ...
    private void verifyDbConfigServiceMock(int getConfigTimes) {
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_ARCHIVE_DAYS"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_SAMPLE_SIZE"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_BATCH_SIZE"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_EXEC_TIME"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_SLEEP_TIME"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_SELECT_TIMEOUT"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_DELETE_TIMEOUT"), anyInt(), anyInt(), anyInt());
        verify(dbConfigService, times(getConfigTimes))
                .getConfigAsIntegerBetween(eq("DEL_ERROR_LOGS_RETRY_ATTEMPTS"), anyInt(), anyInt(), anyInt());
    }
    ...
    private String getResult(int deleteRowsCount, int failedAttemptsNumber, long stopExecutionTime) {
        return String.format("Records clean: ERRLOG %d%s%s", deleteRowsCount,
