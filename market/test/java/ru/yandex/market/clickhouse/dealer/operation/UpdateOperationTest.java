...
public class UpdateOperationTest {
    ...
    private TmTaskState tmTaskState1 = new TmTaskState(tmTaskId1, TmTaskState.Status.COMPLETED, "", null, now, now,
        now);
    private TmTaskState tmTaskState2 = new TmTaskState(tmTaskId2, TmTaskState.Status.COMPLETED, "", null, now, now,
        now);
    private TmTaskState tmTaskState3 = new TmTaskState(tmTaskId3, TmTaskState.Status.COMPLETED, "", null, now, now,
        now);
    ...
    public void testOperation() throws Exception {
        ...
        UpdateOperation operation = new UpdateOperation(clickHousePartition, Arrays.asList(partitionState1(),
            partitionState2()));
    ...
    public void testNotCancelableStates() throws Exception {
        OperationContext context = defaultOperationContextMock();
        Mockito.doThrow(RuntimeException.class).doNothing().when(context)
            .replacePartitionFromTempTable(clickHousePartition);
        UpdateOperation operation = new UpdateOperation(clickHousePartition, Arrays.asList(partitionState1(),
            partitionState2()));
