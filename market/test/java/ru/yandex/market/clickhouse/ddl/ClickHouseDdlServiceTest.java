...
public class ClickHouseDdlServiceTest {
    //TODO добавить после апрува в contib: https://st.yandex-team.ru/CONTRIB-2413
//    @Mock
...
//    @Before
//    public void setup() {
//        String existingTable = "CREATE TABLE db1.table1 ( `date` Date, `string` String, `uInt8` UInt8, `int64`
//        Int64 ) " +
//                               "ENGINE = MergeTree() PARTITION BY toYYYYMM(date) ORDER BY (uInt8, string) SETTINGS
//                               index_granularity = 8192";
//        Mockito.doReturn(true).when(mockJdbcTemplate).queryForObject("EXISTS TABLE db1.table1", Boolean.class);
//        Mockito.doReturn(existingTable).when(mockJdbcTemplate).queryForObject("SHOW CREATE TABLE db1.table1",
//        String.class);
//    }
//
//    @Test
...
//            new ExpectedQuery("CREATE TABLE IF NOT EXISTS db1.table1 ( `date` Date, `string` String, `uInt8` UInt8,
//            `int64` Int64, `other_string.new` String ) ENGINE = MergeTree() PARTITION BY toYYYYMM(date) ORDER BY
//            (uInt8, string) SETTINGS index_granularity = 8192")
//        );
//
...
//                Assert.assertFalse(ddl.getUpdates().isEmpty());
//            }
//        }
//
//        mockJdbcFactory.checkQueriesExecuted();
...
//
//    @Test
//    public void testApplyDdlIfAddNewColumnInConfigFile() {
//
//        ClickHouseTableDefinition tableDefinition = new ClickHouseTableDefinitionImpl(
//            "db1", "table1",
//            Arrays.asList(
...
//
//        TestCluster cluster = createCl1();
//        ClickHouseDdlService service = createClickHouseDdlService(cluster);
//
//        TableDdlState ddlState = service.applyDdl(tableDefinition);
//        assertDdlStatus(ddlState, MonitoringStatus.OK, Assert::assertTrue);
//    }
//
//    @Test
//    public void testApplyDdlIfDropOrModifyColumnInConfigFile() {
//
//        ClickHouseTableDefinition tableDefinition = new ClickHouseTableDefinitionImpl(
//            "db1", "table1",
//            Arrays.asList(
//                new Column("date", ColumnType.Date),
//                new Column("string", ColumnType.String),
//                new Column("uInt8", ColumnType.UInt16)
//            ),
//            MergeTree.fromOldDefinition("date", Arrays.asList("uInt8", "string"), 8192)
//        );
//
...
//
//        TableDdlState ddlState = service.applyDdl(tableDefinition);
//        assertDdlStatus(ddlState, MonitoringStatus.OK, Assert::assertFalse);
//    }
//
//    @Test
//    public void testUpdateStatusIfConfigFileIsNotChanged() {
//
//        ClickHouseTableDefinition tableDefinition = new ClickHouseTableDefinitionImpl(
//            "db1", "table1",
//            Arrays.asList(
...
//
//    @Test
//    public void testUpdateStatusIfTryToDropColumnFromEngineInConfigFile() {
//
//        ClickHouseTableDefinition tableDefinition = new ClickHouseTableDefinitionImpl(
//            "db1", "table1",
//            Arrays.asList(
...
//
//        TableDdlState ddlState = service.applyDdl(tableDefinition);
//        assertDdlStatus(ddlState, MonitoringStatus.CRITICAL, Assert::assertFalse);
//    }
//
//    private void assertDdlStatus(TableDdlState ddlState, MonitoringStatus monitoringStatus, Consumer<Boolean>
//    stateAssertion) {
//        Assert.assertEquals(monitoringStatus, ddlState.getGlobalStatus());
//        for (TableDdlState.ShardState shardState : ddlState.getShardStates()) {
//            Assert.assertEquals(monitoringStatus, shardState.getShardStatus());
//            for (TableDdlState.ServerState serverState : shardState.getServerStates()) {
//                Assert.assertEquals(monitoringStatus, serverState.getReplicaStatus());
//                stateAssertion.accept(serverState.getDdl().getUpdates().stream()
//                    .anyMatch(update -> update.getType() == DdlQueryType.ADD_COLUMN));
//            }
//        }
//    }
//
//    private ClickHouseDdlService createClickHouseDdlService(TestCluster cluster) {
//        MockJdbcFactoryWithHostMockTemplates mockJdbcFactory = new MockJdbcFactoryWithHostMockTemplates(cluster);
//        return ClickHouseDdlService.create(
//            Collections.singletonList("seedHost"), new ClickHouseProperties(), "cl1", mockJdbcFactory,
//            DangerousDdlApprover.NOOP
//        );
//    }
...
//            rs.addColumn("replica_num", servers.stream().map(TestServer::getReplicaNumber).collect(Collectors
//            .toList()));
//            return Arrays.asList(new ExpectedQuery("SELECT * FROM system.clusters WHERE cluster = ?", rs,
//            clusterName));
//        }
//
...
//    }
//
//    private class MockJdbcFactoryWithHostMockTemplates implements ClickHouseDdlService.JdbcTemplateFactory {
//
//        private TestCluster testCluster;
//        private JDBCTestModule clusterTestModule;
//        private Map<String, JDBCTestModule> hostTestModules = new HashMap<>();
//
//        public MockJdbcFactoryWithHostMockTemplates(TestCluster testCluster) {
//            this.testCluster = testCluster;
//        }
//
//        @Override
...
//        PreparedStatementResultSetHandler psRsHandler = factory.getMockConnection()
