...
class MdmYtSecondaryIndexModelTest {
    ...
    private lateinit var model: MdmYtSecondaryIndexModel

    private fun prepareIndexOfType(type: SecondaryIndexType): MdmYtSecondaryIndexModel {
        return MdmYtSecondaryIndexModel("", listOf(
    ...
    @Test
    fun testRawValuesExtractionLinear() {
        // given
        model = prepareIndexOfType(SecondaryIndexType.ONE_2_MANY_LINEAR)
        ...
        // when
        val extracted = model.extractIndexValues(entity).map{ it - NamedMdmField.MDM_ID.name }.toList()

        // then
        extracted shouldBe listOf(mapOf(
    ...
    @Test
    fun testRawValuesExtractionCartesian() {
        // given
        model = prepareIndexOfType(SecondaryIndexType.ONE_2_MANY_CARTESIAN)
        val serviceId1 = 4245325L
        val serviceId2 = 4245326L
        val serviceId3 = 4245327L
        val shopSku = "My name is Yoshikage Kira"
        ...
        extracted shouldBe listOf(
            ...
            mapOf(
                "service_id" to serviceId2,
                "shop_sku" to shopSku,
                "source_type" to sourceType,
                "source_id" to sourceId
            ),
            mapOf(
                "service_id" to serviceId3,
                "shop_sku" to shopSku,
                "source_type" to sourceType,
                "source_id" to sourceId
            )
    ...
    fun testSupportCheck() {
        model = prepareIndexOfType(SecondaryIndexType.ONE_2_ONE)
