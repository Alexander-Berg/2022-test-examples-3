...
public class YaManagerResolverTest {
    ...
    public void ignoreAgencyStrategyTest() {
        YaManagerResolver yaManagerResolver = new YaManagerResolver(true);
        yaManagerResolver.setAgencyService(getMockAgencyService(false, false));
        yaManagerResolver.setIdsResolver(getMockIdsResolver());
        yaManagerResolver.setMarketManagerService(getMockDatasourceManagerService(true));

        Collection<String> emails = yaManagerResolver.resolveAddresses("testAlias", getMockNotificationContext());
        Assert.assertTrue(Iterables.elementsEqual(emails, Arrays.asList(MANAGER_EMAIL)));
    }

    @Test
    public void agencyStrategyTest() {
        YaManagerResolver yaManagerResolver = new YaManagerResolver(false);
        yaManagerResolver.setAgencyService(getMockAgencyService(true, false));
        yaManagerResolver.setIdsResolver(getMockIdsResolver());
        yaManagerResolver.setMarketManagerService(getMockDatasourceManagerService(false));

        Collection<String> emails = yaManagerResolver.resolveAddresses("testAlias", getMockNotificationContext());
        Assert.assertTrue(Iterables.elementsEqual(emails, Arrays.asList(AGENCY_EMAIL)));
    }

    @Test
    public void campaignStrategyTest() {
        YaManagerResolver yaManagerResolver = new YaManagerResolver(false);
        yaManagerResolver.setAgencyService(getMockAgencyService(true, true));
        yaManagerResolver.setIdsResolver(getMockIdsResolver());
        yaManagerResolver.setMarketManagerService(getMockDatasourceManagerService(true));
