...
public class EntityCollectionAssert<T extends Entity> extends FactoryBasedNavigableIterableAssert<EntityCollectionAssert<T>, Iterable<? extends T>, T, ObjectAssert<T>> {
    ...
    public EntityCollectionAssert(Iterable<T> actual) {
        super(actual, EntityCollectionAssert.class, new ObjectAssertFactory<>());
    }

    public static <T extends Entity> EntityCollectionAssert<T> assertThat(Iterable<T> actual) {
        return new EntityCollectionAssert<>(actual);
    }

    @Override
    protected EntityCollectionAssert<T> newAbstractIterableAssert(Iterable<? extends T> iterable) {
        return new EntityCollectionAssert<T>((Iterable<T>) iterable);
    }
