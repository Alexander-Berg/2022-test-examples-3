...
public class InMemoryLockDao implements LockDao {
    ...
    public synchronized Set<String> tryAddLocks(Collection<String> keys, String instanceKey) {
        ...
        for (String key : keys) {
            var lock = locks.computeIfAbsent(key, k -> new LockInfo(new ReentrantLock(), Now.localDateTime(),
                    instanceKey));
        ...
        for (String key : keys) {
            locks.computeIfPresent(key, (k, l) -> {
                if (l.instanceKey().equals(instanceKey)) {
                    return new LockInfo(l.lock(), Now.localDateTime(), instanceKey);
                }
                return l;
            });
        }
    ...
    public void deleteOrphanedLocks(long orphanedPeriod) {
        for (String key : locks.keySet()) {
            var lock = locks.get(key);
            if (lock.lastActivity().isBefore(Now.localDateTime().minus(Duration.ofMillis(orphanedPeriod)))) {
