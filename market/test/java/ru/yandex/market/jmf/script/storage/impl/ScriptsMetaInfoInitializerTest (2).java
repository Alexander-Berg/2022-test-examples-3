...
class ScriptsMetaInfoInitializerTest {
    ...
    void testScriptDependenciesInitialization() {
        ...
        assertThat(script.getDependencies())
                .filteredOn(dependency -> dependency.getType() == EXPLICIT).hasSize(2)
                .extracting(ScriptDependency::getCode).containsExactlyInAnyOrder("metaDependency", "commonDependency");
        assertThat(script.getDependencies())
                .filteredOn(dependency -> dependency.getType() == HEURISTIC).hasSize(7)
                .extracting(ScriptDependency::getCode).containsExactlyInAnyOrder(
                    "commonDependency",
                    "optionalDependency",
                    "requiredDependency",
