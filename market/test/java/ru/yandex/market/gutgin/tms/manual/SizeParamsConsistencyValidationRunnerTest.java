...
import ru.yandex.market.gutgin.tms.pipeline.good.taskaction.datacamp_pipeline.SendResultsToDatacampTaskAction;
...
public class SizeParamsConsistencyValidationRunnerTest {

    private SizeParamsConsistencyValidation action;
    ...
    public void setUp() throws Exception {
        this.action = new SizeParamsConsistencyValidation(gcSkuValidationDao, gcSkuTicketDao, categoryDataHelper);
    }
    ...
    public void run() {
        ...
        long databucket = copyDataFromProduction(41461622);
    ...
    private long copyDataFromProduction(long pipelineId) {
        ...
        tickets.forEach(gcSkuTicket -> {
            ...
            gcSkuTicket.setStatus(GcSkuTicketStatus.VALIDATION_STARTED);
        });
        ...
        Map<Long, List<ValidationWithMessages>> validationMessagesMap =
                gcSkuTicketDaoProduction.getTicketsValidationMessagesMap(tickets.stream()
        ...
        validationMessagesMap.values()
                .stream()
                .flatMap(Collection::stream)
                .forEach(validationWithMessages -> {
                            List<ProtocolMessage> messages = validationWithMessages.getMessages();
                            GcSkuValidation validation = validationWithMessages.getValidation();
                            protocolMessageDao.insert(messages);
                            validationDao.insert(validation);
                            gcValidationMessageDao.newMessageLinks(validation.getId(),
                                    messages.stream().map(ProtocolMessage::getId).collect(Collectors.toList()));
                        }
                );
