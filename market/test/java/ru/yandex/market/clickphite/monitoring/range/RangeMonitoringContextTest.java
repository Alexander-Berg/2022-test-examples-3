...
import ru.yandex.market.health.configs.clickphite.config.monitoring.MonitoringConfig;
import ru.yandex.market.health.configs.clickphite.graphite.Metric;
import ru.yandex.market.health.configs.clickphite.monitoring.range.RangeMonitoringContext;
...
public class RangeMonitoringContextTest {
    private static final String GRAPHITE_URL = "market-graphite-unused-test-url.yandex-team.ru";
    ...
    public void onMetric() throws MalformedURLException {
        ...
        sut.onMetric(new Metric(timeStampSeconds, 60, "", "0.95"));
        sut.onMetric(new Metric(timeStampSeconds + 60, 3000.12345678, "", "0.95"));
        sut.onMetric(new Metric(timeStampSeconds + 120, 1, "", "0.95"));
        ...
        URL expectedUrl = new URL(
            PROTOCOL + GRAPHITE_URL +
                "/render?width=800&height=600&from=-4hours&target=one_min.tsum.nginx.timings.ALL.0_95"
        );
    ...
    public void testFullMetricString() {
        ...
        GraphiteMetricConfig graphiteMetricConfig = gson.fromJson(
            "{\n" +
                ...
                "    'period': 'ONE_SEC',\n" +
                "    'metricField': 'resptime_ms',\n" +
                "    'filter': \"vhost in ('market.yandex.ru', 'market.yandex.ua', " +
                "'market.yandex.by', 'market.yandex.kz') and dynamic = 1 and http_code = 200\",\n" +
                "    'type': 'QUANTILE_TIMING'\n" +
                "}",
            GraphiteMetricConfig.class
        );
