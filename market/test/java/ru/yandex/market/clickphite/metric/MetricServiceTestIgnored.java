...
@Ignore
public class MetricServiceTestIgnored {
    ...
    public void testStatface() throws Exception {
        String data = "fielddate\tvalue\tcategory_id\n" +
            "2015-12-30 03:00:00\t0.0\t\\tВсе товары    Детские товары    Игрушки и игровые комплексы    " +
            "Сюжетно-ролевые игры    Магазин\n" +
            "2015-12-30 03:00:00\t212.0\t\\tВсе товары\\tОдежда, обувь и аксессуары\\tЖенская одежда\\tСвадебная " +
            "мода\\tАксессуары\\t\n" +
            "2015-12-30 03:00:00\t757.0\t\\tВсе товары\\tОдежда, обувь и аксессуары\\tДетская одежда\\tОдежда для " +
            "спорта\\t\n" +
...
//        statfaceClient.sendAnyData("Market/IR/models-clusterizer/import/totalModelsChangedCount/daily", data);
    ...
    public void test() throws Exception {
        ...
        MetricContext metricContext = configurationService.createMetricContext(
        ...
        metricService.updateMetricGroup(
            MetricContextGroupImpl.create(Collections.singletonList(metricContext)),
            timeRange,
            QueryWeight.LIGHT,
            "TEST_POOL"
        );
