package ru.yandex.market.tsum.infrasearch.upload.tasks.xslb.fslb;
...
import ru.yandex.market.tsum.infrasearch.upload.tasks.xslb.configloader.model.general.Environment;
...
public class FslbBalancersServiceInfoConverterTest {
    @Rule
    ...
    public void exportTest() throws Exception {
        ...
        makeStubs(Environment.TESTING.getShortCut());
        makeStubs(Environment.PRODUCTION.getShortCut());
        ...
        FslbBalancersServiceInfoConverter converter = new FslbBalancersServiceInfoConverter(arcanumClient, infraSearchDao);

        List<SaasDocument> saasDocuments = converter.makeSaasDocumentList(0);
        Assert.assertEquals(1, saasDocuments.size());
        Assert.assertEquals(getTestResourceAsString("infrasearch/upload/tasks/xslb/fslb/saas_document.json"),
        ...
        Assert.assertEquals(getTestResourceAsString("infrasearch/upload/tasks/xslb/fslb/saas_yt_document.json")
                .replace("https://fslb.yandex-team.ru", "http://localhost:"+wireMockRule.port()),
    ...
    private void makeStubs(String env) throws IOException {
        String directoryUrl = String.format("market/sre/conf/fslb/%s/values-available", env);
        String ipMapUrl = String.format(
            "/tree/blob/trunk/arcadia/market/sre/conf/fslb/%s/values-static/00-ip-service-map.yaml", env);
        ...
        wireMockRule.stubFor(get(
            ...
            .willReturn(aResponse().withStatus(200).withBody(
                getTestResourceAsString("infrasearch/upload/tasks/xslb/fslb/config_directory.json"))));

        wireMockRule.stubFor(get(
            urlEqualTo(ipMapUrl))
            .withHeader("Authorization", equalTo("OAuth qwerty"))
            .willReturn(aResponse().withStatus(200).withBody(
                getTestResourceAsString("infrasearch/upload/tasks/xslb/fslb/ip_map.yaml"))));

        wireMockRule.stubFor(get(urlEqualTo("/tree/blob/trunk/arcadia/" + directoryUrl + "/config.yaml"))
            .withHeader("Authorization", equalTo("OAuth qwerty"))
            .willReturn(aResponse().withStatus(200).withBody(
                getTestResourceAsString("infrasearch/upload/tasks/xslb/fslb/config-response.yaml"))));
    }
    ...
    private void createStaticStubs(String directoryUrl) throws IOException {
        wireMockRule.stubFor(get(
            ...
            .withHeader("Authorization", equalTo("OAuth qwerty"))
            .willReturn(aResponse().withStatus(200).withBody(
                getTestResourceAsString("infrasearch/upload/tasks/xslb/fslb/empty_directory.json"))));
    }
    ...
    private void createStubsForHaProxy(String directoryUrl) throws IOException {
        wireMockRule.stubFor(get(urlEqualTo("/v2/repos/arc/tree/trunk?path=" +
            ...
            .withHeader("Authorization", equalTo("OAuth qwerty"))
            .willReturn(aResponse().withStatus(200).withBody(
                getTestResourceAsString("infrasearch/upload/tasks/xslb/fslb/haproxy_directory.json"))));
        wireMockRule.stubFor(get(urlEqualTo("/tree/blob/trunk/arcadia/" + directoryUrl + "/haproxy.yaml"))
            .withHeader("Authorization", equalTo("OAuth qwerty"))
            .willReturn(aResponse().withStatus(200).withBody(
                getTestResourceAsString("infrasearch/upload/tasks/xslb/fslb/haproxy-response.yaml"))));
    }
}
