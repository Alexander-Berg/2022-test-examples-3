...
public class GitHubPollTaskTest {
    ...
    public void setUp() {
        ...
        sut = new GitHubPollTask(
            gitHubClient, projectsDao, deliveryMachineStateDao, vcsPollingStateDao, complicatedMonitoring
        );
    }
    ...
    public void shouldIgnoreTeamcityCommits() throws Exception {
        Iterator<RepositoryCommit> commitIterator = Stream.of(
            commit("3", TEAMCITY_USER), commit("2", TEAMCITY_USER), commit("1")
        ).iterator();
        when(gitHubClient.getCommitIterator(any(), any())).thenReturn(commitIterator);
            ...
            Collections.singletonList(
                new DeliveryMachineEntity(
                    DeliveryMachineSettings.builder()
                        .withGithubSettings(REPOSITORY_FULL_NAME, MAIN_BRANCH)
