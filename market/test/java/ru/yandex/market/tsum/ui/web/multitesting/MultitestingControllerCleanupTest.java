...
public class MultitestingControllerCleanupTest {
    ...
    @Test
    public void returnsOkAndStartsCleanup() throws Exception {
        ...
        MultitestingEnvironmentDto response = OBJECT_MAPPER.readValue(
            mockMvc
                .perform(post("/api/multitestings/project/test/environments/n1/cleanup"))
        ...
        assertEquals(MultitestingEnvironment.Status.CLEANUP_TO_IDLE, response.getStatus());
    ...
    @Test
    public void returnsConflictAndDoesNotStartCleanup_whenSomeJobsAreRunning() throws Exception {
        multitestingService.launchMultitestingEnvironment(
            multitestingService.createEnvironment(MultitestingTestData.defaultEnvironment("test", "n1").build()),
            ResourceContainer.EMPTY,
            "user"
    ...
    @Test
    public void returnsOk_whenThereAreNoLaunches() throws Exception {
        multitestingService.createEnvironment(MultitestingTestData.defaultEnvironment("test", "n1").build());
        ...
        assertEquals(
            MultitestingEnvironment.Status.IDLE,
            multitestingService.getEnvironment("test--n1").getStatus()
        );
    }
}
