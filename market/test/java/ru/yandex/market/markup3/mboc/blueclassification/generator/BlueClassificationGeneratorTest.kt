package ru.yandex.market.markup3.mboc.blueclassification.generator
...
import ru.yandex.market.markup3.mboc.PriorityUtil.calculatePriority
import ru.yandex.market.markup3.mboc.blueclassification.MbocBlueClassificationConstants.GROUP_KEY
...
import ru.yandex.market.markup3.tasks.TaskType.BLUE_CLASSIFICATION
import ru.yandex.market.markup3.testutils.CommonTaskTest

class BlueClassificationGeneratorTest : CommonTaskTest() {
    @Autowired
    ...
    fun `Generates tasks for each category-supplier pair`() {
        ...
        generator.generate()
        val ticket0BeforeTask = offerTaskRepository.findByKey(OfferTaskKey(BLUE_CLASSIFICATION, GROUP_KEY, 20))
        ticket0BeforeTask!!.priority shouldBe calculatePriority(100, false)
        ...
        byTask.values shouldContainExactlyInAnyOrder listOf(
            setOf(20L) to calculatePriority(110, false),
            setOf(21L) to calculatePriority(110, false),
            setOf(1L, 2L) to calculatePriority(300,false),
            setOf(3L, 4L) to calculatePriority(300, true),
            setOf(5L) to calculatePriority(300, false),
            setOf(10L, 11L) to calculatePriority(200, false),
        )
        all shouldHaveSize 9
        all.all { it.status == WAITING_FOR_RESULTS } shouldBe true
    }

    @Test
    fun `Cancels tasks that have inactive offers`() {
        mbocMock.addTaskOffers(
            taskOffer(1, 15, 1, deadline = 200, ticketId = 1),
            taskOffer(2, 15, 2, deadline = 201, ticketId = 2),
        )
    ...
    fun `Changes priority of tasks`() {
        ...
        taskEventDbService.findByTaskId(taskId).any { it.eventType == "ChangePriorityEvent" } shouldBe false
        ...
        taskEventDbService.findByTaskId(taskId).any { it.eventType == "ChangePriorityEvent" } shouldBe true
    }
