...
public class SolomonMonitoringProblemReturnsSchedulerTest extends IntegrationTest {

    private SolomonMonitoringProblemRetrurnsScheduler executor;
    ...
    @Autowired
    private ShopRequestReplicaRepository shopRequestRepository;
    ...
    @BeforeEach
    public void init() {
        executor = new SolomonMonitoringProblemRetrurnsScheduler(solomonPushClient, dateTimeService,
                shopRequestRepository, environmentParamService);
    }

    @Test
    @DatabaseSetup("classpath:scheduler/solomon-monitoring-problem-returns/before.xml")
    public void dbQueueSolomonMonitoringCreatedCorrect() throws IOException, JSONException {
        String expectedMonitoringToSolomon = FileContentUtils.getFileContent(
                "scheduler/solomon-monitoring-problem-returns/monitoring-to-solomon.json");
