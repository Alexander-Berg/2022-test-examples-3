...
public class OrdersWithdrawQueueConsumerTest extends IntegrationTestWithDbQueueConsumers {
    ...
    // заявка 11-го типа сваливается в 5-ый статус если лог. точка не найдена
    @Test
    @DatabaseSetup({"classpath:service/shop-request-validation/fulfillment-service.xml",
            ...
            "classpath:service/shop-request-validation/before-orders-withdraw.xml"})
    @ExpectedDatabase(value = "classpath:service/shop-request-validation/after-orders-withdraw-invalid.xml",
            assertionMode = NON_STRICT_UNORDERED)
    public void ordersWithdrawInvalidBecauseLogisticsPointNotFound() {
        TaskExecutionResult result = executeTask();
        assertThat(result).isEqualTo(TaskExecutionResult.finish());
    }

    // заявка 11-го типа успешно проходит валидацию если лог. точка уже прикопана
    @Test
    @DatabaseSetup({"classpath:service/shop-request-validation/fulfillment-service.xml",
            "classpath:service/shop-request-validation/logistics-point-found.xml",
            "classpath:service/shop-request-validation/before-orders-withdraw.xml"})
    @ExpectedDatabase(value = "classpath:service/shop-request-validation/after-orders-withdraw-valid.xml",
            assertionMode = NON_STRICT_UNORDERED)
    public void ordersWithdrawValidLogisticsPointFoundLocally() {
        TaskExecutionResult result = executeTask();
        assertThat(result).isEqualTo(TaskExecutionResult.finish());
    }

    // заявка 11-го типа успешно проходит валидацию если есть лог. точка в ЛМС, прикапываем (апдейтим)
    @Test
    @DatabaseSetup({"classpath:service/shop-request-validation/fulfillment-service.xml",
            "classpath:service/shop-request-validation/logistics-point-empty.xml",
            "classpath:service/shop-request-validation/before-orders-withdraw.xml"})
    ...
    @ExpectedDatabase(value = "classpath:service/shop-request-validation/logistics-point-found.xml",
            assertionMode = NON_STRICT_UNORDERED)
    public void ordersWithdrawValidLogisticsPointFoundRemotely() {
