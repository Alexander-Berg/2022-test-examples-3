...
public class QaCommentsComplaintControllerTest extends BaseComplaintsControllerTest {

    @Test
    public void testGetComplaintsCount() throws Exception {
        int complaintCount = 10;
        when(qaClient.getQaComplaintsCount()).thenReturn(new CountDto(complaintCount));
        String response = mvc.perform(get("/api/qa/comment/complains/count")
    ...
    public void testGetComplaints() throws Exception {
        mockQaClient();
        String response = mvc.perform(get("/api/qa/comment/complains")
            .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().is2xxSuccessful())
            .andDo(print())
            .andReturn().getResponse().getContentAsString();
    ...
    public void testModerate() throws Exception {
        ...
        mvc.perform(post("/api/qa/comment/complains")
            .content(getContent("data/qa/moderate.json"))
        ...
        checkComplaint(moderatorId, first, 1, ComplaintType.COMMENT_QA);

        final Optional<CommentComplaintModerationLogObject> second = list.stream()
            .filter(it -> it.complaintId == 2).findFirst();
        checkComplaint(moderatorId, second, 2, ComplaintType.COMMENT_QA);
    }
}
