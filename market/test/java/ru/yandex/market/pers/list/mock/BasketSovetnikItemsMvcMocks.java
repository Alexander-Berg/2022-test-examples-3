 ...
 */
@Service
public class BasketSovetnikItemsMvcMocks extends AbstractMvcMocks {
    ...
    public String getItemsMvc(BasketOwner owner, BasketCrTimeToken token, int pageSize, ResultMatcher resultMatcher) {
        return invokeAndGet(
            get(String.format("/sovetnik/items/%s/%s", owner.getUserIdType().name(), owner.getId()))
    ...
    public String getExistingItemsMvc(BasketOwner owner, MarketplaceColor color, List<ItemReference> items, Boolean noCache) {
        return invokeAndGet(
            post(String
                .format("/sovetnik/items/%s/%s/existing", owner.getUserIdType().name(), owner.getId()))
    ...
    public String addItemsMvc(BasketOwner owner,
        ...
        return invokeAndGet(
            post(String
                .format("/sovetnik/items/%s/%s", owner.getUserIdType().name(), owner.getId()))
    ...
    public String deleteItemsMvc(BasketOwner owner, long itemId, Integer regionId) {
        return invokeAndGet(
            delete(String.format("/sovetnik/items/%s/%s/%s", owner.getUserIdType().name(), owner.getId(), itemId))
                .param(BasketClientParams.REGION_ID, regionId != null ? String.valueOf(regionId) : null)
                .contentType(MediaType.APPLICATION_JSON),
            status().is2xxSuccessful()
        );
