...
import ru.yandex.market.partner.content.common.db.SequenceStartRandomizer;
import ru.yandex.market.partner.content.common.spring.SpringTransactionProvider;
...
public class TestDataBaseConfiguration {
    ...
    private static final String SCHEMA_NAME = "partner_content";
    private static final String CHANGELOG = "classpath:/liquibase/changelog.xml";
    ...
    @Bean(name = {"roTransactionAwareDataSourceProxy"})
    public DataSource roTransactionAwareDataSourceProxy(DataSource dataSource) {
        return new TransactionAwareDataSourceProxy(dataSource);
    }
    ...
    @Bean(name = "dataSourceConnectionProviderRo")
    DataSourceConnectionProvider connectionProviderRo(
        @Qualifier("roTransactionAwareDataSourceProxy") DataSource jooqDataSource) {
        return new DataSourceConnectionProvider(jooqDataSource);
    }
    ...
    @Bean
    SpringTransactionProvider transactionProvider(@Qualifier("dataSourceTransactionManager") DataSourceTransactionManager transactionManager,
    ...
    @Bean(name = {"jooq.config.configuration"})
    @Primary
    org.jooq.Configuration configuration(@Qualifier("dataSourceConnectionProvider") DataSourceConnectionProvider connectionProvider,
    ...
    @Bean(name = {"jooq.config.configuration.ro"})
    org.jooq.Configuration configurationRO(@Qualifier("dataSourceConnectionProviderRo") DataSourceConnectionProvider connectionProvider,
                                           SpringTransactionProvider springTransactionProvider) {
        DefaultConfiguration config = new DefaultConfiguration();
        config.setSQLDialect(SQLDialect.POSTGRES_9_5);
        config.setConnectionProvider(connectionProvider);
