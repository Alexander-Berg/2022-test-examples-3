 ...
 * Проверка того, что когда при получении рекендаций для ТП на поисковой выдаче (рекомендации на поиске маркета),
 * в репорт передаются ожидаемые feature параметры.
 */
@RunWith(MockitoJUnitRunner.Silent.class)
public class GetSearchWithMarketRecommendationsReportTranslationTest extends AuctionServantletMockBase {
    @InjectMocks
    ...
    public void test_regionTranslation_when_noRegionSpecifiedInRecsByGroup_should_sendReportRequestWithLocalRegion() {
        mockServantletPassedArgs("" +
                "q=" + SOME_SEARCH_QUERY +
                "&type=" + SearchQueryRecommendationsType.MARKET_SEARCH
        );

        searchAuctionOffersServantlet.process(servRequest, servResponse);

        BidRecommendationRequest hybridRecRequest = extractRecommendatorPassedRequest(marketSearchBidRecommendator);
