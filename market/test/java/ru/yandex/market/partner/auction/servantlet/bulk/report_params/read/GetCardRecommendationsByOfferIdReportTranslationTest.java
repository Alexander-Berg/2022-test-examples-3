...
/**
 * Проверяем, что связный запрос в репорт при получении карточных рекомендациях по идентификатору ТП
 * содержит ожидаемый набор аттрибутов для ТП.
 * ТП берется из мока ставок для группы в {@link AuctionService}.
 */
@ExtendWith(MockitoExtension.class)
class GetCardRecommendationsByOfferIdReportTranslationTest extends AuctionBulkServantletlMockBase {
    ...
    void test_getRecommendationsByOfferId() {
        ...
        BidRecommendationRequest hybridRecRequest = extractRecommendatorLoadPassedRequest(modelCardBidRecommendator);
        assertThat(hybridRecRequest, hasRegionId(LOCAL_DELIVERY_REGION_ID));
        assertThat(hybridRecRequest, hasOfferId(SOME_FEED_OFFER_ID));
        assertThat(hybridRecRequest, hasShopId(PARAM_DATASOURCE_ID));
        assertThat(hybridRecRequest, hasSearchQuery(null));
