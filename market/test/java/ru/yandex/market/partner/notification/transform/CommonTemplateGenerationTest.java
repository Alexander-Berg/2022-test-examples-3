...
import ru.yandex.market.partner.notification.service.providers.data.SalutationDataProvider;
...
import ru.yandex.market.partner.notification.transform.common.TemplateConstants;
import ru.yandex.market.partner.notification.transform.common.TemplateId;

import static org.assertj.core.api.Assertions.assertThat;
import static ru.yandex.market.partner.notification.service.providers.data.TransportTypeDataProvider.createData;
...
class CommonTemplateGenerationTest extends AbstractTemplateGenerationTest {
    ...
    void testBodyWithHeader() throws Exception {
        var xsl = generateSimpleXsl(true, false);
        var data = generateTestData();
        final Map<String, String> result = render(xsl, data);
        // Приветствие удалено в MBI-82338
        assertThat(result).containsEntry(BaseXslContentProvider.BODY_KEY, TEST_BODY);
    }
    ...
    private Collection<Object> generateTestData() {
        ...
        data.add(new NamedContainer(SalutationDataProvider.KEY_SALUTATION, HEADER));
    ...
    private String getCommonXsl() {
        return getTemplate(TemplateId.builder()
                .buildForId(TemplateConstants.COMMON_TEMPLATE_ID)
        );
    }
