    ...
    public void testConvert() {
        for (int i = 0; i < ITERATIONS; i++) {
            ...
            Assertions.assertThat(
                dumpCategory.getVendorGoodContentExclusionList()
                    .stream().map(Long::intValue).collect(Collectors.toList())).containsExactlyInAnyOrderElementsOf(
                mbocCategory.getVendorGoodContentExclusion());
        }
    }
}
