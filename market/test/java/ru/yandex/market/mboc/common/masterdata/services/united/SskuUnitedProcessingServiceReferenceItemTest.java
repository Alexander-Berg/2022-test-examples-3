...
public class SskuUnitedProcessingServiceReferenceItemTest extends UnitedProcessingServiceSetupBaseTest {
    ...
    public void testThinGroupByBizKeyHasHandledVerdicts() {
        ...
        processShopSkuKeys(List.of(STAGE3_KEY1));
        ...
        Assertions.assertThat(sskuGoldenVerdictRepository.findByIds(List.of(BUSINESS_KEY, STAGE3_KEY1, STAGE3_KEY2))
                .stream())
    ...
    public void whenStage3GroupRecomputedShouldAffectEvenUnchangedItems() {
        ...
        processShopSkuKeys(List.of(BUSINESS_KEY, STAGE3_KEY1, STAGE3_KEY2));
    ...
    public void testUpdateVersionIdForCertainSourceTypes() {
        ...
        for (MdmIrisPayload.MasterDataSource masterDataSource : MdmIrisPayload.MasterDataSource.values()) {
            ...
            processShopSkuKeys(List.of(key));
    ...
    public void whenNoRslRulesRemoveOldRsl() {
        ...
        processShopSkuKeys(List.of(BUSINESS_KEY, STAGE2_KEY, STAGE3_KEY1, STAGE3_KEY2));
    ...
    public void testWhenWhiteOrUnknownSupplierThenNoAction() {
        ...
        processShopSkuKeys(List.of(k1, k2));
    ...
    public void readExistingReferenceItemsFromGoldenParamTable() {
        ...
        processShopSkuKeys(List.of(shopSkuKey));
    ...
    @Test
    public void testSskuAndMskuWithoutSskuBothInBatch() {
        // given
        prepareBusinessGroup(false);
        Instant ts = Instant.now();
        ...
        // when
        long mskuId1 = 3333L; // не повлияет
        CommonMsku mskuWithoutSsku = prepareTestMskuWithLongLength(categoryId, mskuId1);
        mskuRepository.insertOrUpdateMsku(mskuWithoutSsku);
        mskuAndSskuQueue.enqueueSskus(List.of(BUSINESS_KEY, STAGE3_KEY1), MdmEnqueueReason.CHANGED_SSKU_DATA,
            MdmQueuePriorities.NORMAL_PRIORITY);
        mskuAndSskuQueue.enqueueMsku(mskuId1, MdmEnqueueReason.CHANGED_SSKU_DATA, MdmQueuePriorities.NORMAL_PRIORITY);
        ...
        Assertions.assertThat(referenceItem.getCombinedItemShippingUnit().getWeightGrossMg().getValue()).isEqualTo(
            eoxSsku.getBaseValue(KnownMdmParams.WEIGHT_GROSS).get().getNumeric().get().longValue() * 1000000);
        Assertions.assertThat(referenceItem.getCombinedItemShippingUnit().getLengthMicrometer().getValue()).isEqualTo(
            eoxSsku.getBaseValue(KnownMdmParams.LENGTH).get().getNumeric().get().longValue() * 10000);
    ...
    private CommonMsku prepareTestMskuWithLongLength(Integer categoryId, Long mskuId) {
        ...
        CommonMsku commonMsku = new CommonMsku(new ModelKey(categoryId, mskuId),
            List.of(length, width, height, weight))
            .setMskuId(mskuId);
        return commonMsku;
    }
    ...
    public void testCalculateMasterDataAndRefItemWithMasterDataVersion() {
        ...
        Map<ShopSkuKey, SskuVerdictResult> goldenVerdicts = sskuGoldenVerdictRepository
            .findByIds(List.of(BUSINESS_KEY, STAGE3_KEY1, STAGE3_KEY2)).stream()
            .collect(Collectors.toMap(SskuVerdictResult::getKey, Function.identity()));
        var expectedVerdictByBusinessKey = new SskuVerdictResult();
        expectedVerdictByBusinessKey.setKey(BUSINESS_KEY);
        expectedVerdictByBusinessKey.setValid(true);
    ...
    public void testWhenWhiteOrUnknownSskuThenNoPartnerVerdict() {
        ...
        // when
        processShopSkuKeys(List.of(UNKNOWN_KEY, WHITE_LONELY_KEY));

        // then
        List<SskuPartnerVerdictResult> results = sskuPartnerVerdictRepository
            .findByIds(List.of(UNKNOWN_KEY, WHITE_LONELY_KEY));
