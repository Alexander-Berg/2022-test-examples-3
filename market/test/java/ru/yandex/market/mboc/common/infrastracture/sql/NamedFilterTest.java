package ru.yandex.market.mboc.common.infrastracture.sql;
...
public class NamedFilterTest {
    ...
    public void testAndCondition() {
        ...
        assertEquals("where something > :something\n and other = :other", where.getQuery("where "));
    ...
    public void testTypeCast() {
        where.and("'123'::int = :x", 123);
        assertEquals("where '123'::int = :x", where.getQuery("where "));
        assertEquals(ImmutableMap.of("x", 123), where.getNamedParams());
    }
    ...
    public void testDoubleValuesOkIfEqual() {
        ...
        assertEquals("where a = :a\n and b = :a", where.getQuery("where "));
        assertEquals(ImmutableMap.of("a", 1), where.getNamedParams());
    }
    ...
    @Test
    public void testIn() {
        where.and("something in (:ids)", Arrays.asList(1, 2));
        assertEquals(where.getNamedParams(), ImmutableMap.of("ids", Arrays.asList(1, 2)));
    }
}
