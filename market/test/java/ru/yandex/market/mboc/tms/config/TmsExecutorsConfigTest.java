package ru.yandex.market.mboc.tms.config;
 ...
 * Расписание задаётся в аннотации {@link ru.yandex.market.tms.quartz2.spring.CronTrigger}, параметры мониторинга
...
public class TmsExecutorsConfigTest {
    private static final Set<String> EXCEPTIONS = ImmutableSet.of(
        "checkMappingFixNameExecutor", // нестандартное расписание
        "checkMappingFixNameSendReportExecutor", // нестандартное расписание
        "monitoringOffersConsistencyBetweenMbocAndYt", // временно отключено
        "erpSyncExecutor" // временно отключено
    );
    ...
    public static void beforeClass() throws IOException {
        ...
        var classesInPackage = ClassPath.from(classLoader)
            .getTopLevelClassesRecursive("ru.yandex.market");
        try {
    ...
    public void testExecutorsProperlyAnnotated() {
        assertTrue(allJobsHaveMonitoringAndCronAnnotations());
    }

    @Test
    public void testSmallPeriodJobsHaveTenMinsTolerance() {
        assumeTrue(allJobsHaveMonitoringAndCronAnnotations());
    ...
    private boolean allJobsHaveMonitoringAndCronAnnotations() {
        ...
        return Objects.equals(monitored, triggered);
    }
    ...
    private List<Method> getJobsToCheck(Class<? extends Annotation> annotation) {
        ...
        Set<String> ignoredNames = new HashSet<>(EXCEPTIONS);
        List<String> scheduledIgnored = getMethodsAnnotatedWith(MonitoringConfig.class)
            .stream()
            .filter(m -> {
            ...
            .map(Method::getName)
            .collect(Collectors.toList());
        ignoredNames.addAll(scheduledIgnored);
