...
import ru.yandex.market.logistics.lom.jobs.processor.validation.validator.WaybillSegmentC2CTagEnricher;

@DisplayName("Обогащение сегментов тэгом C2C")
@ParametersAreNonnullByDefault
class WaybillSegmentC2CTagEnricherTest extends AbstractTest {
    private final WaybillSegmentC2CTagEnricher enricher = new WaybillSegmentC2CTagEnricher();
    ...
    void enrichSuccess() {
        Order order = Order.builder()
            .waybill(List.of(
                ...
                getWaybillSegment(PartnerType.DELIVERY, SegmentType.SORTING_CENTER, 1),
                getWaybillSegment(PartnerType.SORTING_CENTER, SegmentType.SORTING_CENTER, 2),
        ...
        softly.assertThat(resultWaybill.get(0).hasTag(WaybillSegmentTag.C2C)).isFalse();
        softly.assertThat(resultWaybill.get(1).hasTag(WaybillSegmentTag.C2C)).isTrue();
        softly.assertThat(resultWaybill.get(2).hasTag(WaybillSegmentTag.C2C)).isFalse();
        softly.assertThat(resultWaybill.get(3).hasTag(WaybillSegmentTag.C2C)).isTrue();
    }
