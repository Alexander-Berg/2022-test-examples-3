...
const categoriesMatchMock: Mock = require('./__mocks__/categories-match.sec-scr');
...
const primeTextMock: Mock = require('./__mocks__/prime.sec-scr');
const primeCategoryText: Mock = require('./__mocks__/prime-category-text.sec-scr');
const shopInfoMock: Mock = require('./__mocks__/shop-info.sec-scr');

/* For Mock Implementations */
const { shops } = require('./__mocks__/shops.sec-scr');
...
const CLID_FROM_SETTINGS = 1234567;
const CLID_FROM_COOKIES = 2210590;

let mockScopes = raiseMocks();

const cookieSecondScript = {
    applicationName: 'Яндекс.Советник',
    affId: '1008',
    clid: CLID_FROM_COOKIES,
    scriptVersion: '202103241221',
    vid: null,
    offerEnabled: false,
    optOutAccepted: null,
    offerAccepted: null,
...
const encryptedCookie = `svt-partner=${Cookie.encrypt(JSON.stringify(cookieSecondScript))}`;
...
describe('Rule: second-script', () => {
    ...
    test('should get second-script rule', async () => {
