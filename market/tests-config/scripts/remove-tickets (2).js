...
const {readStOAuthToken} = require('../readOAuthToken');
const ask = require('asker-as-promised');

const oAuthToken = process.env['ST_OAUTH_TOKEN'] || readStOAuthToken();
...
async function getFromTracker(path, query) {
    if (!oAuthToken) throw Error('Not provided oAuth token');
