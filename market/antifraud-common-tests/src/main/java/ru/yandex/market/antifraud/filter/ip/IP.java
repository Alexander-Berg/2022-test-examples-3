package ru.yandex.market.antifraud.filter.ip;
...
public class IP {
    ...
    public static Set<String> generateRandomSetUniqueIp4FromRange(final String cidr, final Integer count) {
        return IP4.generateRandomSetUniqueIpFromRange(cidr, count).
                stream().map(it -> IP4.ntoa(it)).collect(Collectors.toSet());
    }
    ...
    public static Set<String> generateRandomSetUniqueIp6FromRange(final String cidr, final Integer count) {
        return IP6.generateRandomSetUniqueIpFromRange(cidr, count).
                stream().map(it -> IP6.ntoa(it)).collect(Collectors.toSet());
    }
...
//        return IPWithGeoId.generateYandexIPv4ForGeoId();
//    }
//
//    public static String generateNoYandexIPv4ForGeoId() {
//        return IPWithGeoId.generateNoYandexIPv4ForGeoId();
//    }
//
//    public static String getGeoIdForIPv4(String ipv4) {
//        return IPWithGeoId.getGeoIdForIPv4(ipv4);
//    }
    //====================================================================================================
    ...
    public static List<String> extractIPv4Addresses(String ips, String separator) {
        return Arrays.asList(ips.split(separator)).stream().filter(new Predicate<String>() {
            @Override
            public boolean test(String ip) {
                return IP4.isIPLiteralAddress(ip) || isIPv4LiteralAddressAsIPv6(ip);
            }
    ...
    public static List<String> extractIPv6Addresses(String ips, String separator) {
        return Arrays.asList(ips.split(separator)).stream().filter(new Predicate<String>() {
            @Override
            public boolean test(String ip) {
                return IP6.isIPLiteralAddress(ip) && !isIPv4LiteralAddressAsIPv6(ip);
            }
        }).collect(Collectors.toList());
    }
    ...
    public static String generateIpFromBlackSubnet() {
        int i = RandomUtils.nextInt(0, blackIps.size());
