...
@Stories("Перемещение ставок в другую группу")
@Issue("AUTOTESTMARKET-46")
public class DeleteGroupWithBidsMovingToAnotherGroupTest {
    ...
    private BidGroup hostGroup;
    private PostBidsRequestBody postBidsBody;

    @Before
    public void before() {
        bidding.backend.groups(shopId).deleteAll();
        BidGroup groupToDelete = bidding.backend.groups(shopId).create("GroupToDelete");
        ...
        hostGroup = bidding.backend.groups(shopId).create("HostGroup");

        bidding.backend.group(shopId, groupToDelete.id).deleteAndMoveBidsTo(hostGroup.id);

        for (PostBidsRequestBody.ChangeByTitle changeByTitle : postBidsBody.changeByTitleList) {
            changeByTitle.ext.group = hostGroup.id;
        }
        hostGroup.size = 2L;
    }
    ...
    public void deletedGroupShouldNotBePresentInGetGroupsResult() {
        bidding.backend.groups(shopId).getGroupsShouldReturn(defaultBidGroup(shopId), hostGroup);
    }

    @Test
    @Title("Удалённой группы не должно быть в БД")
    public void deletedGroupShouldNotBePresentInDatabase() {
        bidding.database.groups(shopId).shouldContain(hostGroup);
    }

    @Test
    @Title("Ставки из удалённой группы должны быть в группе hostGroupId")
