...
import static ru.yandex.autotests.market.bidding.beanfactories.PostBidsRequestBodyFactory.buildPostOfferIdBidsRequestBody;
...
public class PostOfferIdBidsTest {
    ...
    private final PostBidsRequestBody postBidsBody = buildPostOfferIdBidsRequestBody();
    ...
    public void bidsShouldChangeInBackend() {
        bidding.backend.bids(shopId).offer().shouldReturnOfferIdBids(bidding.database, postBidsBody);
    }

    @Test
    @Title("Ставки должны обновиться в БД")
    public void bidsShouldChangeInDatabase() {
        bidding.database.categoryBids(shopId).shouldContainOfferIdBids(postBidsBody);
    }
}
