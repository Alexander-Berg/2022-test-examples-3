...
import static ru.yandex.autotests.market.bidding.beanfactories.PostBidsRequestBodyFactory.buildPostOfferTitleBidsRequestBody;
...
@Stories("Установка ставок для офферов по title")
@Issue("AUTOTESTMARKET-56")
public class PostOfferTitleBidsTest {
    ...
    private final PostBidsRequestBody postBidsBody = buildPostOfferTitleBidsRequestBody();
    ...
    public void bidsShouldChangeInBackend() {
        bidding.backend.bids(shopId).offer().shouldReturnTitleBids(bidding.database, postBidsBody);
    }

    @Test
    @Title("Ставки должны обновиться в БД")
    public void bidsShouldChangeInDatabase() {
        bidding.database.categoryBids(shopId).shouldContainOfferTitleBids(bidding.database, postBidsBody);
    }
}
