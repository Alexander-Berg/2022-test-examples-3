package ru.yandex.autotests.market.bidding.tests.renamegroup;
 ...
 * Date: 29.09.2014
 */
@Aqua.Test(title = "Запрос POST /groups/{groupId}/rename")
@Features("POST /groups/{groupId}/rename")
@Issue("AUTOTESTMARKET-45")
public class RenameGroupTest {
    ...
    public void before() {
        ...
        bidding.backend.group(shopId, group.id).rename("RenamedGroup");
        group.name = "RenamedGroup";
    }

    @Test
    @Title("Переименованная группа должна быть в выдаче запроса GET /groups")
    public void renamedGroupShouldBePresentInGetGroupsResult() {
        bidding.backend.groups(shopId).getGroupsShouldReturn(defaultBidGroup(shopId), group);
    }

    @Test
    @Title("Переименованная группа должна быть в БД")
    public void renamedGroupShouldBePresentInDatabase() {
