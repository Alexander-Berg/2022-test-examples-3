...
/// Простой тест. На вход получаем 1 бакет с 2мя опциями (дешевая и быстрая) для региона 1, а также
/// 2 локальные опции для приоритетного региона (213) - тоже дешевую и быструю. На выходе просто имеем все
/// 4 опции для двух регионов.
    ...
    TVector<NYT::TNode> ExpectedData() {
        return {
            NYT::TNode()
                ...
                ("region_id", 1u)
                ("currency_id", 1u)
                ("carrier_ids", NYT::TNode().Add(1u).Add(2u))
                ("option_type", 0u)
