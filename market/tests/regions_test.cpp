...
/// На вход подается 2 бакета в разных файлах. В первом бакете 3 региона (2, 3, 225),
/// во втором 2 региона (3, 225).
/// На выходе 2 региона с лучшими опциями в них (регион совпадающий с родителем отфильтровывается).
...
///      /    \       +      /    \      =      /    \
///     /      \            /      \           /      \
///  3 (25р)  2 (75р)    3 (80р)  empty     3 (25р)  empty (т.к. совпадает с родителем)
    ...
    TData RegionalDeliveryData() {
        return TData{
            ...
            .Regions = {
                ...
                {.BucketId = 1, .RegionId = 2, .GroupId = 2},
...
TEST(ShopDeliveryOptionsBuilder, Regions) {
    auto client = NYT::NTesting::CreateTestClient();
    auto options = PrepareOptions(client, RegionalDeliveryData());

    constexpr auto GENLOG_FIRST_TABLE_PATH = "//regions/genlog_table_1";
    constexpr auto GENLOG_SECOND_TABLE_PATH = "//regions/genlog_table_2";
