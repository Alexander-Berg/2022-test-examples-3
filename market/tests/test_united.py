...
import yatest
import tempfile
from hamcrest import assert_that, has_items, equal_to, empty
...
from market.idx.datacamp.proto.offer.OfferTechInfo_pb2 import MiningTrace
from market.idx.datacamp.proto.offer.DataCampOffer_pb2 import WHITE, BLUE
...
DATACAMP_MESSAGES = [{
    'united_offers': [{
        'offer': [
            # Оффер с неизвестным цветом для магазина которого нет в shopsdat
            {
                'basic': {
                    'identifiers': {
                        'business_id': 999,
                        'offer_id': 'unknown_shop_offer',
                    },
                },
                'service': {
                    999: {
            ...
            # Синий оффер из единого каталога, будет майниться
            {
                'basic': {
                    'identifiers': {
                        'business_id': 1,
                        'offer_id': 'blue_offer_from_united_catalog',
                    },
                },
                'service': {
                    3: {
            ...
            # Оффер без цвета для известного магазина помайнится после проставления цвета
            {
                'basic': {
                    'identifiers': {
                        'business_id': 1,
            ...
            # Оффера из ассортиментного каталога под бизнесом без сервисных частей
            {
                'basic': {
                    'identifiers': {
                        ...
                        'offer_id': 'business_catalog_offer_1',
                    },
                    'meta': {
                        'business_catalog': {
                            'flag': True
                    ...
                    'content': {
                        'partner': {
                            'original': {
                                'name': {
                                    'value': 'business_catalog_offer_1_name',
                                }
                            }
                        }
                    }
            ...
            },
            {
                'basic': {
                    'identifiers': {
                        'business_id': BUSINESS_CATALOG_OFFERS_BUSINESS_ID,
                        'offer_id': 'business_catalog_offer_2',
                    },
                    'meta': {
                        'business_catalog': {
                            'flag': True
                    ...
                    'content': {
                        'partner': {
                            'original': {
                                'name': {
                                    'value': 'business_catalog_offer_2_name',
                                },
                                'country_of_origin': {
                                    'value': [
                                        'Италия'
            ...
            # Оффер без сервисных чаcтей, но не из каталога под бизнесом - не должны майнить его
            {
                'basic': {
                    'identifiers': {
                        'business_id': 12345,
                        'offer_id': 'offer_not_in_business_catalog_but_without_service',
                    },
                    'content': {
                        'partner': {
                            'original': {
                                'name': {
                                    'value': 'offer_not_in_business_catalog_but_without_service_name',
                                }
                            }
                        }
                    }
            ...
            # Оффер в united-формате с N актуальными сервисными частями после майнинга
            # превратится в N разных united офферов без актуальных частей
            {
                'basic': {
                    'identifiers': {
                        'business_id': 1,
                        'offer_id': 'offer_in_united_format',
                    },
                    'tech_info': {
                        'last_mining': {
                            'mode': MiningTrace.UNITED_FULL,
                        }
                    }
                },
                'service': {
                        ...
                        'warehouse': {
                            3: {
                                'identifiers': {
                                    ...
                                    'warehouse_id': 3,
                                    'extra': {
                                        'ware_md5': 'Xo-uzKsu5hIhU6v7Couaya'
                                    }
                                },
                                'meta': {
                                    'rgb': WHITE
                            ...
                            4: {
                                'identifiers': {
                                    ...
                                    'warehouse_id': 4,
                                    'extra': {
                                        'ware_md5': 'Xo-uzKsu5hIhU6v7Couayb'
                                    }
                                },
                                'meta': {
                                    'rgb': WHITE
...
def test_united_miner(miner, input_topic, output_topic):
    ...
    dc_requests = miner.resources['dc_server'].GetRequests()
    assert_that(len(dc_requests), equal_to(6))
    assert_that(dc_requests, has_items(has_items('/shopDeliveryMeta?shopId=2')))
    assert_that(dc_requests, has_items(has_items('/shopDeliveryMeta?shopId=1')))
    # blue offer with shopId=3 ignored by delivery enricher
                ...
                'service': IsProtobufMap({
                    1: {
                        'identifiers': {
                            ...
                            'warehouse_id': 1,
                            'extra': {
                                'ware_md5': 'bx4I_8gyTqMOTsEtDT_h5w'
                            }
                        },
                        'status': {
                            'disabled': [
                ...
                'service': IsProtobufMap({
                    2: {
                        'identifiers': {
                            ...
                            'warehouse_id': 0,
                            'extra': {
                                'ware_md5': 'Xo-uzKsu5hIhU6v7Couayg'
                            }
                        },
                        'status': {
                            'disabled': [
    ...
    assert_that(len(msg.united_offers[0].offer), equal_to(8))
