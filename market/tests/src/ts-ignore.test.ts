...
describe('ts-ignore plugin', () => {
  it('adds ignore comment', async () => {
    ...
    expect(result)
      .toBe(`// @ts-expect-error ts-migrate(123) FIXME: diagnostic message
comsole.log('Hello');`);
  });
      ...
      mockPluginParams({
        ...
        semanticDiagnostics: [
          mockDiagnostic(text, 'comsole', {
            messageText:
              'This message is too long to print and should be truncated',
          }),
        ],
      }),
    );
...
`;
    ...
    const result = await tsIgnorePlugin.run(
      mockPluginParams({
        text,
        semanticDiagnostics: [
          mockDiagnostic(text, 'this_module_does_not_exist'),
        ],
      }),
    );
