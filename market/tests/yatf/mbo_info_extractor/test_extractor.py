...
from market.idx.yatf.resources.mbo.models_pb import ModelsPb, SkusPb
...
@pytest.fixture(scope="function")
def resources(models, category_parameters, skus):
    return {
        'models': ModelsPb(models, CATEGORY_ID),
        'parameters': ParametersPb(category_parameters),
        'sku': SkusPb(skus, CATEGORY_ID)
    }


@pytest.fixture(scope="function")
def workflow(resources):
