...
from market.idx.offers.yatf.test_envs.glue_model_reducer import GlueModelReducerTestEnv
from market.idx.yatf.resources.yt_stuff_resource import get_yt_prefix
from market.idx.library.glue.proto.GlueConfig_pb2 import FieldValue
from market.idx.library.glue.proto.GlueMessages_pb2 import ModelFields
...
def glue_model_data():
    ...
    for glue_model in data:
        glue_model['model_id'] = YsonUint64(glue_model['model_id'])
        for glue_field in glue_model['glue_fields']:
...
def workflow(yt_server, offers, glue_model_data):
    offers_table = YtTableResource(yt_stuff=yt_server, path=ypath_join(get_yt_prefix(), 'input', 'offers'), data=offers)
    offers_table.dump()
    schema = TableSchema().add_column('model_id', ti.Uint32).add_column('available_for_fake_msku', ti.Optional[ti.Bool]).add_column(
        'glue_fields',
        ti.List[
            ti.Struct[
    ...
    glue_model_table = YtTableResource(
        yt_stuff=yt_server,
        path=ypath_join(get_yt_prefix(), 'input', 'glue_model'),
        data=glue_model_data,
        sort_key='model_id',
        attributes={'schema': schema},
    )
    glue_model_table.dump()

    output_table_path = ypath_join(get_yt_prefix(), 'out_model_keys')

    with GlueModelReducerTestEnv(
            yt_server,
            output_yt_path=output_table_path,
            input_offers_yt_paths=[offers_table.table_path],
            input_model_glue_yt_paths=[glue_model_table.table_path],
    ) as env:
        env.execute()
        env.verify()
        yield env
