        ...
        make_sku_protobuf(
            ...
            parameter_hypothesis=[
                ParameterValueHypothesis(
                    ...
                    xsl_name=HYP_PARAM_1_XSL_NAME,
                    str_value=[
                        MboParameters_pb2.Word(
                            name=HYP_PARAM_1_NAME
                        ),
                    ],
                ),
                ParameterValueHypothesis(
...
def workflow(
    ...
    with MskuUploaderTestEnv(
        jump_table_path='//home/test/jump_table',
        contex_enabled=True,
        yt_input_dir=yt_dir,
        **resources
    ) as env:
...
def test_table_size_yt(result_yt_table, expected_param_values):
    """
    ...
    assert_that(
        ...
        equal_to(expected_num),
        'Number of records in table is unexpected'
    )


def test_parameters_values_and_is_original_yt(result_yt_table, expected_param_values):
    """
...
def test_image_picker_yt(result_yt_table, expected_param_values):
    """
...
def test_show_on_yt(result_yt_table, expected_param_values):
    """
...
def test_exp_count_yt(result_exp_records):
    assert_that(
...
def test_msku_exp_yt(result_exp_records):
    """
    ...
    for record in result_exp_records:
        ...
        assert_that(
            exp_to_original_mskus[msku_exp],
            equal_to(msku),
            "Unexpected msku {} for msku_exp {}".format(msku, msku_exp)
        )


def test_msku_exp_model_id_yt(result_exp_records):
    """
