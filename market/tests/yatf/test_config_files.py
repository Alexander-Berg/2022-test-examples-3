...
from market.amore.output.lbdumper_amore.yatf.test_envs.test_env import LB_DUMPER_BIN
...
def test_config_files(env, dc):
    etc_dir = yatest.common.source_path(os.path.join('market', 'amore', 'output', 'lbdumper_amore', 'etc'))

    conf = os.path.join(etc_dir, 'lbdumper.cfg')
    variable_files = [os.path.join(etc_dir, 'env', '{}.{}'.format(env, dc))]
