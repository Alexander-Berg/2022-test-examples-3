...
def genlog_rows():
    ...
    for i in range(len(offers)):
        offers.append(
            default_genlog(
                category_id=offers[i]['category_id'],
                ware_md5='1'+offers[i]['ware_md5'][1:],
                offer_params=offers[i]['offer_params']
            )
        )
...
def workflow(yt_server, genlog_table, cataloger_navigation_old_tree, tovar_tree, cataloger_navigation_white_tree, cataloger_navigation_blue_tree, recipes, gl_mbo):
...
def test_accept_parents_recipe_nids(workflow, case):
    assert_that(
        workflow,
        HasGenlogRecord({
            'ware_md5': case['ware'],
            'nids_literals': has_items(*case['nids']),
        }),
        case['description'])
