...
def workflow(yt_server, tovar_categories):
    ...
    with YtDataUploadTestEnv(**resources) as env:
        env.execute(yt_server, type='categories', output_table="//home/test/categories", use_old_stub=True)
        env.verify()
        yield env
...
def test_primary_blue_nid(result_yt_table):
    ...
    # In navigation tree: <node id="54440" parent_id="54415" name="Электроника" hid="198119" guru_category_id="0" is_primary="1"  is_green="1" is_blue="1">
