...
def yt_in_offer_factors(yt_stuff):
    rows = [
        {'doc_id': 0, 'image2text_v10_embedding_str': _get_compressed_image2text_v10_str(1)},
        ...
        {'doc_id': 2, 'image2text_v10_embedding_str': _get_compressed_image2text_v10_str(2)},
        # нет doc_id - не дампим
        {'doc_id': None, 'image2text_v10_embedding_str': _get_compressed_image2text_v10_str(2)},
        # нет эмбедов - не дампим
        {'doc_id': None, 'image2text_v10_embedding_str': None},
        # неверная длина эмбедов - не дампим
        {'doc_id': None, 'image2text_v10_embedding_str': _get_compressed_image2text_v10_str(3, length=199)},
    ]
...
def test_index_file(workflow):
    ...
    expexted_data = b'\x00\x00\x00\x00\x01\x00\x00\x00'    # offset=0 + offset=1
    buf = b''
...
def test_embeds_file(workflow):
    ...
    expected_size = 2 * IMAGE2TEXT_V10_SIZE
    workflow.verify([HasOutputFiles({file}), HasOutputFileSize(file, expected_size)])

    expexted_data = bytearray(b'')
    for i in range(IMAGE2TEXT_V10_SIZE):
        expexted_data += b'1'    # 1st embed
    for i in range(IMAGE2TEXT_V10_SIZE):
        expexted_data += b'2'    # 2nd embed
    buf = b''
