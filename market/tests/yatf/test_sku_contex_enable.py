...
from market.proto.indexer.contex_pb2 import TContexRelations as ContexRelations
...
def workflow(
    ...
    with MskuUploaderTestEnv(
            contex_table_path='//home/test/msku_contex',
            contex_enabled=True,
            **resources
    ) as env:
...
@pytest.fixture(scope='module')
def result_contex_yt_table(workflow):
    return workflow.outputs.get('result_contex_table')
    ...
    return [
        ...
        ContexRelations(
            ...
            relations=[{'id': EXPERIMENTAL_SKU_ID, 'type': EXPERIMENTAL_SKU},
                       {'id': EXPERIMENTAL_MODEL_ID, 'type': EXPERIMENTAL_MODEL},
                       {'id': MODEL_ID, 'type': EXPERIMENTAL_BASE_MODEL}],
            experiment_flag=None,
            is_sku=True,
        ),
        ContexRelations(
            ...
            relations=[{'id': EXPERIMENTAL_MODEL_ID, 'type': EXPERIMENTAL_MODEL},
                       {'id': ORIGINAL_SKU_ID, 'type': EXPERIMENTAL_BASE_SKU},
                       {'id': MODEL_ID, 'type': EXPERIMENTAL_BASE_MODEL}],
            experiment_flag='some_test_id',
            is_sku=True,
        ),
...
def test_results_exist(yt_server, result_yt_table):
    assert_that(
...
def test_results_contex_exist(yt_server, result_contex_yt_table):
    assert_that(
        yt_server.get_yt_client().exists(result_contex_yt_table.get_path()),
...
def test_result_contex_table_schema(result_contex_yt_table):
    result_list = sorted(
        extract_column_attributes(list(result_contex_yt_table.schema)),
        key=lambda column: column["name"]
    )
    expected_list = sorted(
        [
            {'required': False, "name": "msku",               "type": "uint64", "sort_order": "ascending"},
            {'required': False, "name": "msku_exp",           "type": "uint64", "sort_order": "ascending"},
            {'required': False, "name": "experimental_model_id", "type": "uint64"},
            {'required': False, "name": "msku_experiment_id", "type": "string"},
            {'required': False, "name": "feed_id",            "type": "uint64"},
            {'required': False, "name": "session_id",         "type": "uint64"},
            {'required': False, "name": "offer_id",           "type": "string"},
            {'required': False, "name": "offer",              "type": "string"},
...
def test_result_contex_table_row_count(result_contex_yt_table):
    assert_that(
        len(result_contex_yt_table.data),
        equal_to(3),
        "Incorrect yt table rows count"
    )


def test_big_sku(result_contex_yt_table, result_yt_ext_table):
    result_record = result_contex_yt_table.data[0]
    ...
    expected_uc = market.proto.ir.UltraController_pb2.EnrichedOffer(
        ...
        params=[
            ...
            # Карта переходов строится только по параметрам с базовых
...
def test_original(result_contex_yt_table):
    result_record = result_contex_yt_table.data[1]
    ...
    assert_that(
        fact_offer.genlog.title,
        equal_to("Оригинальный MSKU 100000000002"),
        "original msku"
    )

    contex_info = fact_offer.genlog.contex_info
    assert_that(contex_info.experiment_id, equal_to("green"))
    assert_that(contex_info.experimental_msku_id, equal_to(100000000003))
    assert_that(contex_info.original_msku_id, equal_to(0))
    assert_that(contex_info.original_model_id, equal_to(1713074440))
    assert_that(contex_info.experimental_model_id, equal_to(1713074441))
    assert_that(contex_info.is_experimental, equal_to(False))


def test_experimental(result_contex_yt_table):
    result_record = result_contex_yt_table.data[2]
    ...
    assert_that(
        fact_offer.genlog.title,
        equal_to("Экспериментальная MSKU для 100000000002"),
        "original msku"
    )

    contex_info = fact_offer.genlog.contex_info
    assert_that(contex_info.experiment_id, equal_to("some_test_id"))
    assert_that(contex_info.experimental_msku_id, equal_to(0))
    assert_that(contex_info.original_msku_id, equal_to(100000000002))
    assert_that(contex_info.original_model_id, equal_to(1713074440))
    assert_that(contex_info.experimental_model_id, equal_to(1713074441))
    assert_that(contex_info.is_experimental, equal_to(True))


def test_lookup_result(yt_server, result_yt_table):
    """
    ...
    expected_msku = [100000000001, 100000000002, 100000000003]
    actual_msku = []

    path = result_yt_table.get_path()

    rows = yt_client.lookup_rows(path, [
        {'msku': 100000000001},  # out of any experiments
        {'msku': 100000000002},  # original
        {'msku': 100000000003},  # experiment
    ])
    for row in rows:
    ...
    assert_that(
        expected_msku,
        equal_to(sorted(actual_msku)),
        'Some msku not found'
    )


def test_lookup_result_contex(yt_server, result_contex_yt_table):
    """
    Тест проверяет, что полученная таблица является подобием key-value storage и в ней можно искать по msku
    """
    ...
    expected_msku = [100000000001, 100000000002, 100000000002]
    actual_msku = []

    path = result_contex_yt_table.get_path()

    rows = yt_client.lookup_rows(path, [
        {'msku': 100000000001, 'msku_exp': 0},  # out of any experiments
        {'msku': 100000000002, 'msku_exp': 0},  # original
        {'msku': 100000000002, 'msku_exp': 100000000003},  # experiment
    ])
    for row in rows:
        actual_msku.append(row['msku'])
