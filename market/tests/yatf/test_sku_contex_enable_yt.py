...
def test_original(result_contex_yt_table):
    ...
    assert_that(contex_info.experimental_msku_id, equal_to(EXPERIMENTAL_SKU_ID))
    assert_that(contex_info.original_msku_id, equal_to(0))
    assert_that(contex_info.original_model_id, equal_to(MODEL_ID))
    # assert_that(contex_info.experimental_model_id, equal_to(EXPERIMENTAL_MODEL_ID))
...
def test_experimental(result_contex_yt_table):
    ...
    assert_that(contex_info.original_msku_id, equal_to(ORIGINAL_SKU_ID))
    assert_that(contex_info.original_model_id, equal_to(MODEL_ID))
    assert_that(contex_info.experimental_model_id, equal_to(EXPERIMENTAL_MODEL_ID))
...
def test_lookup_result(yt_server, result_yt_table):
    ...
    expected_msku = [100000000001, ORIGINAL_SKU_ID, EXPERIMENTAL_SKU_ID]
    actual_msku = []
    ...
    rows = yt_client.lookup_rows(path, [
        ...
        {'msku': ORIGINAL_SKU_ID},  # original
        {'msku': EXPERIMENTAL_SKU_ID},  # experiment
    ])
    for row in rows:
...
def test_lookup_result_contex(yt_server, result_contex_yt_table):
    ...
    expected_msku = [100000000001, ORIGINAL_SKU_ID, ORIGINAL_SKU_ID]
    actual_msku = []
    ...
    rows = yt_client.lookup_rows(path, [
        ...
        {'msku': ORIGINAL_SKU_ID, 'msku_exp': 0},  # original
        {'msku': ORIGINAL_SKU_ID, 'msku_exp': EXPERIMENTAL_SKU_ID},  # experiment
    ])
    for row in rows:
        actual_msku.append(row['msku'])
