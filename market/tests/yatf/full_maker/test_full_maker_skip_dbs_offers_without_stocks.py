...
def make_id(prefix, dbs, ignore_stocks, disable_by_market_stock):
    return '{}_(is_dsbs={};_ignore_stocks={};_disable_by_market_stock={})'.format(prefix, dbs, ignore_stocks, disable_by_market_stock)


def create_disabled(value, ts=OLD_TIME):
    if value is None:
        return []
    return [
        {
            'meta': {
                'source': MARKET_STOCK,
                'timestamp': ts.strftime(time_pattern),
            },
            'flag': value
        }
    ]
...
def datacamp_offers_table_data(ts=OLD_TIME):
    ...
    for color, offer_id in zip([BLUE, WHITE], [OFFER_BLUE, OFFER_WHITE]):
        for feed_data in PARTNERS_DATA[color]['shops_dat_info']:
            ...
            sp = SupplyPlan.UNKNOWN
            for is_dsbs in [False, True]:
                for is_ignore_stocks in [False, True]:
                    for disable_by_market_stock in [False, True, None]:
                        shop_sku = make_id(offer_id, is_dsbs, is_ignore_stocks, disable_by_market_stock)
                        result.append({
                            ...
                            'offer': message_from_data(
                                {
                                    ...
                                    'partner_info': {
                                        ...
                                        'supplier_id': shop_id,
                                        'is_dsbs': is_dsbs,
                                        'is_ignore_stocks': is_ignore_stocks,
                                    },
                                    'status': {
                                        'disabled': create_disabled(disable_by_market_stock, ts)
                                    },
...
def make_item(feed_id, offer_id_prefix, dbs, ignore_stocks, has_market_stocks):
    offer_id = make_id(offer_id_prefix, dbs, ignore_stocks, has_market_stocks)
    return {
        'feed_id': feed_id,
        'offer_id': offer_id
    }
...
def test_all_cases(full_maker, offers_raw_ids):
    for dbs in [False, True]:
        for ignore_stocks in [False, True]:
            for disable_by_market_stock in [False, True, None]:
                if dbs and not ignore_stocks and disable_by_market_stock is None:
                    assert_that(offers_raw_ids, not_(has_item(make_item(FEED_ONE, OFFER_BLUE, dbs, ignore_stocks, disable_by_market_stock))))
                    assert_that(offers_raw_ids, not_(has_item(make_item(FEED_TWO, OFFER_WHITE, dbs, ignore_stocks, disable_by_market_stock))))
                else:
                    assert_that(offers_raw_ids, has_item(make_item(FEED_ONE, OFFER_BLUE, dbs, ignore_stocks, disable_by_market_stock)))
                    assert_that(offers_raw_ids, has_item(make_item(FEED_TWO, OFFER_WHITE, dbs, ignore_stocks, disable_by_market_stock)))
