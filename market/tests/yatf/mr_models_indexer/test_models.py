...
from market.idx.models.yatf.test_envs.mr_models_indexer import MrModelsIndexerTestEnv
...
def models():
    ...
    vcluster1 = ExportReportModel(id=VCLUSTER_ID,
                                  category_id=CATEGORY_ID,
                                  vendor_id=CATEGORY_ID,
                                  current_type='CLUSTER',
                                  published_on_market=True)
    models.append(vcluster1)

    model_base_group = ExportReportModel(id=MODEL_ID_GROUP,
                                         category_id=CATEGORY_ID,
                                         vendor_id=CATEGORY_ID,
                                         current_type='GURU',
                                         published_on_market=True,
    ...
    model_base_modification = ExportReportModel(id=MODEL_ID_MODIFICATION,
                                                category_id=CATEGORY_ID,
                                                vendor_id=CATEGORY_ID,
                                                current_type='GURU',
                                                published_on_market=True,
    ...
    pmodel = ExportReportModel(id=PMODEL_ID,
                               category_id=CATEGORY_ID,
                               vendor_id=CATEGORY_ID,
...
def workflow_indexer(resources):
    with MrModelsIndexerTestEnv(**resources) as env:
        env.execute()
        env.verify()
        yield env
...
def test_indexer(workflow_indexer):
    assert_that(workflow_indexer, all_of(
        ...
        HasDocs().attributes(hyper=str(VCLUSTER_ID))
                 .attributes(hyper_ts=str(VCLUSTER_ID))
                 .attributes(ModelId=str(VCLUSTER_ID))
                 .attributes(hyper_ts_blue=str(VCLUSTER_ID))
                 .attributes(ungrouped_hyper_blue=None)
                 .attributes(url="market.yandex.ru/product/2")
                 .attributes(is_pmodel=None),
