# coding: utf-8
"""
    Проверяем, что main-idx перекладывает офферы Лавки в шарды
"""
...
def lavka_offer():
    offer = Offer(
        ...
        offer_flags64=OfferFlags.IS_LAVKA | OfferFlags.DELIVERY | OfferFlags.CPA,
...
def shops_dat():
    return ShopsDat([{
        'datafeed_id': LAVKA_FEED_ID,
        'shop_id': LAVKA_SHOP_ID,
        'is_lavka': 'true',
        'cpa': 'REAL',
        'cpc': 'NO',
    }])
...
def test_lavka_offers(or3_config_data, main_idx):
    """ Офферы Лавки перекладываются в шарды, обогащения от UC нет """
    if or3_config_data['lavka']['indexation_enabled'] == 'true':
        assert_that(
            main_idx.outputs['offers_by_offer_id'],
            has_entries({
                'lavka_offer_1': all_of(
                    has_entries({
                        'offer_id': 'lavka_offer_1',
                        'genlog': has_entries({
                            'flags': str(OfferFlags.IS_LAVKA | OfferFlags.DELIVERY | OfferFlags.CPA),
                            'delivery_flag': True,
                            'title': 'crab chowder',
                            'description': 'yummy sweet crab chowder',
                            'language_tag': 'en',
