...
WHITE_OFFER8 = 'padding___white_offer8'
...
def __make_offer(
    ...
    category_id,
    is_blue_offer=False,
    is_dsbs_offfer=False,
    is_dsbs_for_blue_shard=False,
    offer_score=0.0,
    offer_policy=0
):
    result = {
        ...
        'offer_score_policy_id': offer_policy,
    }
...
def _make_blue_offer(feed, msku, offer_id, ware_md5, category_id, offer_score=0.0, offer_policy=0):
    return __make_offer(feed, msku, offer_id, ware_md5, category_id, is_blue_offer=True, offer_score=offer_score, offer_policy=offer_policy)


def _make_white_offer(feed, msku, offer_id, ware_md5, category_id, offer_score=0.0, offer_policy=0):
    return __make_offer(feed, msku, offer_id, ware_md5, category_id, offer_score=offer_score, offer_policy=offer_policy)


def _make_dsbs_offer(feed, msku, offer_id, ware_md5, category_id, for_blue_shard=False, offer_score=0.0, offer_policy=0):
    return __make_offer(feed, msku, offer_id, ware_md5, category_id, is_dsbs_offfer=True, is_dsbs_for_blue_shard=for_blue_shard, offer_score=offer_score, offer_policy=offer_policy)
...
def source_offers_raw():
    ...
    return [
        _make_white_offer(418656, 1, '1115112', WHITE_OFFER1, ROOT_CATEGORY_ID, offer_score=3.0, offer_policy=1),
        _make_white_offer(418656, 2, '1097602', WHITE_OFFER2, MSKU_SHARDING_CATEGORY_ID_PARENT, offer_score=1.0, offer_policy=0),
        _make_white_offer(418656, 1, '1126379', WHITE_OFFER3, MSKU_SHARDING_CATEGORY_ID_CHILD, offer_score=2.0, offer_policy=0),
        _make_white_offer(418656, 111, '7982163', WHITE_OFFER4, FEED_OFFER_ID_SHARDING_CATEGORY_ID, offer_score=3.0, offer_policy=0),
        _make_white_offer(418656, 111, '1074153', WHITE_OFFER5, category_id=None, offer_score=4.0, offer_policy=1),  # нет категории
        _make_white_offer(418656, None, '1133022', WHITE_OFFER6, MSKU_SHARDING_CATEGORY_ID_PARENT, offer_score=5.0, offer_policy=0),  # нет msku
        _make_dsbs_offer(418656, 2, '1026681', DSBS_OFFER1, FEED_OFFER_ID_SHARDING_CATEGORY_ID, offer_score=6.0, offer_policy=1),
        _make_white_offer(418656, -1, '1106521', WHITE_OFFER7, MSKU_SHARDING_CATEGORY_ID_PARENT, offer_score=7.0, offer_policy=0),  # msku = -1
        _make_white_offer(418656, 1, '1115112', WHITE_OFFER8, ROOT_CATEGORY_ID, offer_score=10.0, offer_policy=2),
    ]
...
def source_blue_offers_raw():
    return [
        _make_blue_offer(418656, 111, '1087631', BLUE_OFFER1, FEED_OFFER_ID_SHARDING_CATEGORY_ID, offer_score=2.0, offer_policy=1),
        _make_blue_offer(418656, 111, '1032020', BLUE_OFFER2, FEED_OFFER_ID_SHARDING_CATEGORY_ID, offer_score=3.0, offer_policy=0),
        _make_dsbs_offer(418656, 111, '0198037', DSBS_OFFER2, FEED_OFFER_ID_SHARDING_CATEGORY_ID, for_blue_shard=True, offer_score=1.0, offer_policy=0),
    ]
...
def main_idx(yt_server, or3_config, source_yt_tables, tovar_tree):
    ...
    bound_offer_score_by_policy = {'0': 2.0, '1': 4.0, '2': YsonEntity()}
    white_table_path = ypath_join(yt_home_path, 'mi3', MI3_TYPE, GENERATION, 'work', 'offers_raw')
    yt_client.set(ypath_join(white_table_path, '@bound_offer_score_by_policy'), bound_offer_score_by_policy)

    blue_table_path = ypath_join(yt_home_path, 'mi3', MI3_TYPE, GENERATION, 'work', 'blue_offers_raw')
    yt_client.set(ypath_join(blue_table_path, '@bound_offer_score_by_policy'), bound_offer_score_by_policy)
...
def test_sortdc_score_filter(main_idx):
    """Проверяем шардирование офферов
    """
    assert_that(len(main_idx.outputs['offers']), equal_to(9))  # 4 white + 1 blue + 3 msku + 1 dsbs
    assert_that(len(main_idx.outputs['blue_offers']), equal_to(4))  # 1 blue + 3 msku

    white_ware_md5 = list()
    for shard_num, table in enumerate(main_idx.outputs['offers_shards']):
        for offer in table:
            white_ware_md5.append(offer['ware_md5'])

    blue_ware_md5 = list()
    for shard_num, table in enumerate(main_idx.outputs['blue_offers_shards']):
        for offer in table:
            blue_ware_md5.append(offer['ware_md5'])

    expected_white_offers = [WHITE_OFFER4, WHITE_OFFER6, WHITE_OFFER7, WHITE_OFFER8]
    expected_blue_offers = [BLUE_OFFER2]
    expected_dsbs_offers = [DSBS_OFFER1]
    expected_msku = [MSKU1, MSKU2, MSKU3]

    assert sorted(white_ware_md5) == sorted(expected_white_offers + expected_blue_offers + expected_dsbs_offers + expected_msku)
    assert sorted(blue_ware_md5) == sorted(expected_blue_offers + expected_msku)
