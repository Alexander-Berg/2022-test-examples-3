...
def make_offer(feed_id, offer_id, msku, category_id, shop_id, flags, is_dsbs, promo):
    result = {
        'feed_id': feed_id,
        'offer_id': offer_id,
...
def make_white_offer(feed_id, offer_id, category_id, shop_id, flags, is_dsbs, promo):
    return make_offer(feed_id, offer_id, None, category_id, shop_id, flags, is_dsbs, promo)
...
@pytest.yield_fixture(scope="module")
def promo_code_details_data_datacamp(promo_details_promo_code_datacamp, promo_details_direct_discount_datacamp):
    return [
        {
            'feed_id': WHITE_FEED_ID,
            'session_id': SESSION_ID,
...
def collected_promo_details_data(promo_details_data, promo_code_details_data, promo_code_details_data_datacamp):
    return sorted(
        promo_details_data + promo_code_details_data + promo_code_details_data_datacamp,
...
def or3_config(collected_promo_details_output_dir):
    misc = {
        'disable_promo_matching_in_final_reduce': False,  # если поставить True, то тест падает, т.к. матчинг отключается
    }
...
def test_dsbs_promo(
    ...
    ''' Проверяeм что в выходной белой таблице промо к белым dsbs-офферам применяются.
        При этом, акции матчатся к офферам и с использованием отдельных таблиц PromoDetails,
        так и с использованием объединённой таблицы.
    '''
    ...
    if collected_promo_details_output_dir:
        assert_that(
            ...
            'white dsbs offer has new matched cheapest_as_gift promo and original blue_set promo'
        )
        assert_that(
            ...
            'white dsbs offer has generic_bundle promo'
        )
        assert_that(
            ...
            'white dsbs offer has both generic_bundle and cheapest_as_gift promos'
        )
        for offer_id in [WHITE_OFFER_ID_6, WHITE_OFFER_ID_7, WHITE_OFFER_ID_8]:
            assert_that(
                offers[offer_id],
                has_entries({
                    'feed_id': WHITE_FEED_ID,
                ...
                'white dsbs offer has promo_code matched by rules'
            )
        assert_that(
            ...
            'white dsbs offer has promo_code from datacamp'
        )
        assert_that(
            ...
            'white dsbs offer has direct_discount from datacamp'
        )
        assert_that(
