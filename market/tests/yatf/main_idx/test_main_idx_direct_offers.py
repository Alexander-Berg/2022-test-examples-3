# coding=utf-8
"""
Проверяем что офферы из таблицы Директа складываются в белые шарды
"""

import pytest
import time
from datetime import datetime
from hamcrest import assert_that, has_entries, equal_to, all_of, is_not, has_key
...
def test_run(main_idx):
    offers = main_idx.outputs['offers_by_offer_id']
    assert_that(len(offers), equal_to(1))
    assert_that(
        offers,
        has_entries({
            OFFER_DIRECT: all_of(
