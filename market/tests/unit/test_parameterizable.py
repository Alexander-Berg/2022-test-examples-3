...
class BaseThing(Parameterizable):
    ...
    unrequired_parameter = Parameter(default="default")
...
def test_default_parameter_value_can_be_evaluated():
    ...
    assert base_thing.unrequired_parameter == "default"
    assert base_thing.evaluated_parameter == "based on x"


def test_default_parameter_value_can_be_overridden():
    base_thing = BaseThing(required_parameter="x", unrequired_parameter="y", evaluated_parameter="z")
    assert base_thing.required_parameter == "x"
    assert base_thing.unrequired_parameter == "y"
...
def test_derived_class_can_shadow_inherited_parameters():
    ...
    assert derived_thing.unrequired_parameter == "hidden"
    ...
    with pytest.raises(UnknownParameterError):
        DerivedThing(unrequired_parameter="z")
...
def test_parameterizable_name_depends_on_class():
    derived_thing = DerivedThing()
    similar_base_thing = BaseThing(required_parameter="no longer required", unrequired_parameter="hidden")
...
def test_parameterization_works_well_with_multiple_inheritance():
    ...
    class ConcreteNameableThing(AbstractNameableThing, Parameterizable):
...
def test_parameterizable_provides_access_to_all_qualified_parameter_values():
    thing = BaseThing(required_parameter="0", unrequired_parameter="1", evaluated_parameter="2")
    assert thing.parameters["required_parameter"].value == "0"
    assert repr(thing.parameters["required_parameter"]) == "'0'"
    assert thing.parameters["unrequired_parameter"].value == "1"
    assert repr(thing.parameters["unrequired_parameter"]) == "'1'"
