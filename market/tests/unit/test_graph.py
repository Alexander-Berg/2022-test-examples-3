...
def test_adding_items_increases_graph_size():
    graph = Graph()
    for i in range(1, 6):
        graph.add(i)
        assert len(graph) == i
...
def test_graph_refuses_to_link_non_existing_nodes():
    graph = Graph()
    with pytest.raises(AssertionError):
        graph.link("1", "2")
    graph.add("1")
...
def test_graph_allows_to_get_nodes_by_item_exemplar():
    ...
    item_clone = (0, "o")
    assert item_clone is not item
    assert graph[item_clone].item is item
...
def test_graph_supports_node_annotation():
    ...
    assert graph[item]["completed"] == True
