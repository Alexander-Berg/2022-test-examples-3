...
def test__tail(tmpdir):
    ...
    with open(fname, 'r') as fh:
        assert list(common.fs.tail(fh, 2)) == lines[-2:]
        assert list(common.fs.tail(fh, lines * 2)) == lines
        partial = mmap.ALLOCATIONGRANULARITY / size + 1
        assert list(common.fs.tail(fh, partial)) == lines[-partial:]
...
def test_allocate_file(tmpdir):
    ...
    size = 10 * 1024 ** 2
    assert common.fs.allocate_file(fname1, size)
    assert common.fs.allocate_file(fname2, size, force_seek=True)
