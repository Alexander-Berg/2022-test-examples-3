...
def get_all_config_files():
    root_dir = yatest.common.source_path('market/report/runtime_cloud/solomon_agent/zeus_tmpl')
...
SOLOMON_AGENT_CONFIG_FILES = get_all_config_files()
...
def all_contexts_for_role(role, env_list):
    return [
        ...
        for env in env_list
        for file in SOLOMON_AGENT_CONFIG_FILES
    ]
...
def make_test_name(ctx):
    ctx = ctx['PYTEST_INFO']
    name = '{}_{}_{}'.format(ctx['role'], ctx['env'], ctx['filepath'])
    return name.replace('-', '_').replace('.', '_')


def generate_config(ctx):
    filename = ctx['PYTEST_INFO']['filepath']
    root_dir = yatest.common.source_path('market/report/runtime_cloud/solomon_agent')
