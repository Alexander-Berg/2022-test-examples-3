...
def service_search_table(yt_server, config):
    return DataCampServiceSearchOffersTable(yt_server, yt.ypath_join('//home/test_datacamp/search', str(uuid.uuid4())), data=[
        ...
        offer_to_search_service_row(generate_offer('feed_disabled_2')),
    ])
...
def actual_service_search_table(yt_server, config):
    return DataCampActualServiceSearchOffersTable(yt_server, yt.ypath_join('//home/test_datacamp/actual_service_search', str(uuid.uuid4())), data=[
        ...
        offer_to_search_actual_row(generate_offer('feed_stock_1')),
        offer_to_search_actual_row(generate_offer('table_out_of_stock'), True),
        offer_to_search_actual_row(generate_offer('feed_stock_2')),
    ])
...
def test_qparser_stock(push_parser, input_topic, output_topic, mds, datacamp_output_topic, complete_offers_topic, config, qp_runner):
                ...
                'offer': [
                    {
                        'basic': {
                            'identifiers': {
                                'business_id': BUSINESS_ID,
                                'offer_id': '{}xXx{}'.format(FEED_ID, 'feed_stock_1'),
                            }
                        },
                ...
                'offer': [
                    {
                        'basic': {
                            'identifiers': {
                                'business_id': BUSINESS_ID,
                                'offer_id': '{}xXx{}'.format(FEED_ID, 'feed_stock_2'),
                            }
                        },
                        'actual': IsProtobufMap({
                            SHOP_ID: {
