...
<channel>
    ...
    <item>
        <g:id>2</g:id>
        <g:shipping_label>dog food</g:shipping_label>
    </item>
</channel>
</rss>
)wrap");
...
TEST(RssFeedParser, ShippingLabel) {
    const auto actual = RunRssFeedParser<NCommon::TGoogleMerchantRssParser>(
        ...
        [] (const TQueueItem& item) {
            ...
            if (merchant.shipping_detail().has_shipping_label()) {
                result["ShippingLabel"] = merchant.shipping_detail().shipping_label();
            }
            return result;
        }
    );
