...
static const TString INPUT_CSV(R"wrap(ID,Item title,Final URL,Item category
    1,Donut,http://somesite.ru,Pastries
    2,Item2,http://somesite.ru,
)wrap");

static const TString EXPECTED_JSON = TString(R"wrap(
[
    {"id":"1", "category": "Pastries"},
    {"id":"2", "category": ""},
]
)wrap");

TEST(GoogleCustomParser, ItemCategory) {
  const auto actual = RunFeedParserWithTrace<NGoogleCustom::TCsvFeedParser>(
      ...
      [](const TQueueItem &item) {
        ...
        if (item->DataCampOffer.content().type_specific_content().has_google_custom()) {
          ...
          result["category"] = googleCustom.category();
        }
        return result;
      },
