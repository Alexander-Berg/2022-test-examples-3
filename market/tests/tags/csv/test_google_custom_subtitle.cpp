...
static const TString INPUT_CSV(R"wrap(ID,Item title,Final URL,Item subtitle
    1,Item1,http://somesite.ru,Low price
    2,Item2,http://somesite.ru,
)wrap");

static const TString EXPECTED_JSON = TString(R"wrap(
[
    {"id":"1", "subtitle": "Low price"},
    {"id":"2", "subtitle": ""},
]
)wrap");

TEST(GoogleCustomParser, Subtitle) {
  const auto actual = RunFeedParserWithTrace<NGoogleCustom::TCsvFeedParser>(
      ...
      [](const TQueueItem &item) {
        ...
        if (item->DataCampOffer.content().type_specific_content().has_google_custom()) {
          ...
          result["subtitle"] = googleCustom.subtitle();
        }
        return result;
      },
