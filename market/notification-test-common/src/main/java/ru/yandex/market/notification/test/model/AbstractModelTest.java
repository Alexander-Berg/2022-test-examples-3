...
import ru.yandex.market.notification.model.common.HasCode;
...
public abstract class AbstractModelTest {
    ...
    protected <T extends Enum<T> & HasCode<?>> void checkEnum(@Nonnull final Class<T> enumClass, final int count) {
        final T[] enumConstants = enumClass.getEnumConstants();
        final Collection<?> codes = Arrays.stream(enumConstants)
            .map(enumConstant -> enumConstant.getCode())
    ...
    protected <T> void assertThatCode(@Nonnull final HasCode<T> transport, @Nonnull final T code) {
        assertThat(transport.getCode(), equalTo(code));
    }
