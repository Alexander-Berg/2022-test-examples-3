...
import OperationalRatingCard from '@self/platform/components/OperationalRatingCard/__pageObject__';
...
// Тест проверяет наличие бейджа операционного рейтинга на ДО и топ-6
            ...
            childSuites: [
                {
                    ...
                    capture(actions) {
                        actions.waitForElementToShow(`${DefaultOffer.root} ${OperationalRatingCard.root}`);
                    },
                },
                {
                    ...
                    before(actions) {
                        hideElementBySelector(actions, StickyOffer.root);
                        actions.click(`${DefaultOffer.root} ${OperationalRatingCard.root}`);
            ...
            childSuites: [
                {
                    ...
                    capture(actions) {
                        actions.waitForElementToShow(`${ProductOfferSnippet.root} ${OperationalRatingBadge.root}`);
                    },
                },
                {
                    suiteName: 'clickOnStarShouldOpenADrawer',
                    before(actions) {
                        hideElementBySelector(actions, StickyOffer.root);
                        actions.click(`${ProductOfferSnippet.root} ${OperationalRatingBadge.root}`);
                        actions.waitForElementToShow(OperationalRatingDrawer.content);
                    },
                    selector: OperationalRatingDrawer.content,
                    capture() {
