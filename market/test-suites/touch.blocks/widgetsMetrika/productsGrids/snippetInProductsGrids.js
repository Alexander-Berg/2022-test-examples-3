            ...
            async beforeEach() {
                ...
                if (this.getMeta('environment') === 'kadavr') {
                    if (!this.params.productId) {
                        throw new Error('Для теста необходимо передать параметр {productId}');
                    } else {
                        /**
                         * для точного поиска метрики в ProductsGrids нужен productId
                         * без него метрика visible вернёт 3 значения, а в метрике visible ждём уникальное событие
                         */
                        this.params.payloadSchema.productId = this.params.productId;
                    }
                }
