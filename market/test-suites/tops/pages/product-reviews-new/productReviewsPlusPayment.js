...
// page-objects
import ProductReviewFormMicroMainFields
    from '@self/platform/components/ReviewForm/ProductMainFields/__pageObject';

// utils
import {profiles} from '@self/platform/spec/hermione/configs/profiles';
import {createUserReviewWithoutPhotos} from '@self/platform/spec/hermione/fixtures/review';
    ...
    story: mergeSuites(
        {
            async beforeEach() {
                await this.setPageObjects({
                    productReviewFormMicroMainFields: () => this.createPageObject(
                        ProductReviewFormMicroMainFields
                    ),
                });
        ...
        makeSuite('Если пользователь выставлял оценку товару.', {
            story: mergeSuites(
                {
                    async beforeEach() {
                        const reviewMock = {
                            ...createUserReviewWithoutPhotos(productId, testUser.uid),
                            averageGrade: 5,
                            comment: '',
                            pro: '',
                            contra: '',
                        ...
                        await this.browser.setState('schema', {
                            gradesOpinions: [reviewMock],
                            modelOpinions: [reviewMock],
                        });
                    },
                },
                plusPaymentQueryParamSuite,
