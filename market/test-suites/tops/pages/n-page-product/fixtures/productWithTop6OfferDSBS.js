...
const buildOffer = (offer, id) => ({
    ...offer,
    cpc: `${offer.cpc || 'DqqPjIrWS5xIT'}${id || offer.id}`,
    id: id || offer.id,
    wareId: id || offer.id,
});

const dsbsOffer = buildOffer(offerDSBSMock, offer1Id);

const offers = [
    dsbsOffer,
    buildOffer(offer, offer2Id),
    buildOffer(offer, '3333'),
    buildOffer(offer, '4444'),
    buildOffer(offer, '5555'),
    buildOffer(offer, '6666'),
];
...
const state = mergeState([
    ...
    product,
    ...offers.map(offer => createOffer(offer, offer.id)),
]);
