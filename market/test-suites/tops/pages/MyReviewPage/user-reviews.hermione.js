...
import ShopHeadline from '@self/platform/components/PersonalCabinetShopHeadline/__pageObject';
import ProductHeadline from '@self/platform/components/PersonalCabinetProductHeadline/__pageObject';
...
const prepareUserReviewsTabPage = async (ctx, params = {}) => {
    ...
    return ctx.browser.yaProfile('ugctest3', 'market:my-reviews');
};

// eslint-disable-next-line import/no-commonjs
module.exports = makeSuite('Личный кабинет. Вкладка с отзывами пользователя.', {
    environment: 'kadavr',
    issue: 'MARKETFRONT-7245',
    story: mergeSuites(
        {
            async beforeEach() {
                this.setPageObjects({
        ...
        {
            'Опубликованный товарный отзыв с лайками.': mergeSuites(
                {
                    async beforeEach() {
                        ...
                        this.params = {
                            ...
                            expectedReviewTypeLink: await this.browser.yaBuildURL('market:product', {
                            ...
                            expectedReviewPageLink: await this.browser.yaBuildURL('market:product-review', {
                                slug: DEFAULT_PRODUCT.slug,
                                productId: DEFAULT_PRODUCT.id,
        ...
        {
            'Опубликованный магазинный отзыв с лайками.': mergeSuites(
                {
                    async beforeEach() {
                        await this.setPageObjects({
                        ...
                        this.params = {
                            ...
                            totalLikesCount: DEFAULT_VOTES_COUNT,
                            totalDislikesCount: DEFAULT_VOTES_COUNT,
                            expectedReviewTypeLink: await this.browser.yaBuildURL('market:shop', {
                                slug: DEFAULT_SHOP_INFO.slug,
                                shopId: DEFAULT_SHOP_INFO.id,
                            }),
