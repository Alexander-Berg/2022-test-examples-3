 ...
 * @param {PageObject.ShopName} ShopInfo
 */
export default makeSuite('Карусель "Товары от магазина"', {
    story: mergeSuites(
        {
            'По умолчанию': {
                'содержится на странице trololo': makeCase({
                    async test() {
            ...
            'При переходе по товару из карусели ': {
                'магазин товара, по которому перешли, совпадает с магазином товара, со страницы которого сделан переход trololo': makeCase({
                    async test() {
