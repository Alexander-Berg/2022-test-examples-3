...
module.exports = makeSuite('Ошибки купонов.', {
                    ...
                    'При применении купона': {
                        'появляется ошибка "Что-то пошло не так".': makeCase({
                            ...
                            async test() {
                                ...
                                await this.browser.yaSafeAction(this.orderInfoPreloader.waitForHidden(3500), false)
                                    .then(
                                        () => this.orderInfoPreloader.isVisible().should.eventually.be.equal(
                                            false,
                                            'таймаут скрытия прелоадера'
                                        )
                                    );
...
async function deleteSku(index = 0) {
    ...
    const carts = this.params.items.map((item, index) => buildCheckouterBucket({
        cartIndex: index,
        items: [{
...
async function reduceAmount() {
    ...
    await this.browser.yaSafeAction(this.orderInfoPreloader.waitForVisible(3500), true);
