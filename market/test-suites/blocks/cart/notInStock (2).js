    ...
    story: {
        ...
        'Замена товара на похожие': makeCase({
            async test() {
                ...
                await this.popupModal.waitForVisible();
                await this.browser.allure.runStep('Закрываем попап нажатием на крестик',
                    async () => {
                ...
                return this.browser.allure.runStep('Проверяем актуализированные офферы',
                    async () => {
                        await this.browser.waitUntil(
                            () => this.firstOfferDetails.getTitle()
                                .should.eventually.to.be.equal(
                                    sock.skuMock.titles.raw,
                                    `Название первого товара в корзине должно быть "${sock.skuMock.titles.raw}"`
                                ),
                            ACTUALIZATION_TIMEOUT
                        );
                        return this.browser.waitUntil(
                            () => this.secondOfferDetails.getTitle()
                                .should.eventually.to.be.equal(
                                    kettle.skuMock.titles.raw,
                                    `Название второго товара в корзине должно быть "${kettle.skuMock.titles.raw}"`
                                ),
                            ACTUALIZATION_TIMEOUT
                        );
                    }
