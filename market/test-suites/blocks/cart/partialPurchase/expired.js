...
export default makeSuite('Отсутствие чекбоксов и кнопки "Удалить выбранное" при условии, что в корзине только недоступные для заказа товары.', {
    feature: 'Отсутствие чекбоксов и кнопки "Удалить выбранное" при условии, что в корзине только недоступные для заказа товары',
    environment: 'kadavr',
    story: {
        async beforeEach() {
            this.setPageObjects({
        ...
        'На странице корзины должны отсутствовать отображение элементов частичной покупки при условии, что в корзине есть разобранные товары': makeCase({
            async test() {
