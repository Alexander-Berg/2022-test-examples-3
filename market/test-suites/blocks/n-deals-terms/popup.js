            ...
            'должен открывать попап описания акции': makeCase({
                test() {
                    return this.browser.allure.runStep('Проверяем открытие попапа по наведению на бейдж', async () => {
                        await this.dealsBadge.content.moveToObject();
                        await this.dealsDescriptionPopup.waitForVisible();
                        const isPopupVisible = await this.dealsDescriptionPopup.isVisible();
