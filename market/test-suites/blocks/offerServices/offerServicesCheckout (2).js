    ...
    story: {
        ...
        'Изменение даты и времени в чекауте': makeCase({
            async test() {
                ...
                await this.expect(this.mountingDateSelectButton.getText())
                    .to.be.equal('воскресенье, 25 февраля', 'Выбрана дата по-умолчанию');
                await this.expect(this.mountingTimeSelectButton.getText())
                    .to.be.equal('с 08:00 до 14:00', 'Выбрано время по-умолчанию');
                ...
                await this.expect(this.mountingDateSelectButton.getText())
                    .to.be.equal(newDate, 'Дата услуги изменилась');
                ...
                return this.expect(this.mountingTimeSelectButton.getText())
                    .to.be.equal(newTime, 'Время услуги изменилось');
            },
        }),
        ...
        'Установка в день заказа': makeCase({
            async test() {
                ...
                await this.expect(this.mountingDateSelectButton.getText())
                    .to.be.equal('воскресенье, 25 февраля', 'Выбрана дата установки');
                await this.expect(this.mountingTimeSelectButton.getText())
                    .to.be.equal('с 08:00 до 14:00', 'Выбрано время установки');
                ...
                await this.expect(this.mountingDateSelectButton.getText())
                    .to.be.equal('суббота, 24 февраля', 'Дата услуги изменилась');
