...
import DefaultText
    from '@self/root/src/widgets/parts/ReturnCandidate/components/Final/components/DefaultText/__pageObject';
import DropshipDefaultText
    from '@self/root/src/widgets/parts/ReturnCandidate/components/Final/components/DropshipDefaultText/__pageObject';
...
import {INSTALLED_DELIVERY_APP} from '@self/root/src/constants/queryParams';
...
export default makeSuite('Дропшип.', {
    ...
    story: {
        async beforeEach() {
            ...
            await this.browser.yaProfile('pan-topinambur', PAGE_IDS_COMMON.CREATE_RETURN, {
                orderId,
                [INSTALLED_DELIVERY_APP]: 'YANDEX',
            });
        },
                ...
                'Есть контакты для возврата.': {
                    ...
                    'Доставка курьерской службой.': {
                        'Контакты для возврата правильные.': makeCase({
                            async test() {
                                ...
                                const name = `${SHOP_RETURN_CONTACTS.CARRIER.lastName} ${SHOP_RETURN_CONTACTS.CARRIER.firstName} ${SHOP_RETURN_CONTACTS.CARRIER.middleName}`;
                    ...
                    'Доставка пользователем лично': makeCase({
                        async test() {
                            ...
                            const name = `${SHOP_RETURN_CONTACTS.SELF.lastName} ${SHOP_RETURN_CONTACTS.SELF.firstName} ${SHOP_RETURN_CONTACTS.SELF.middleName}`;
