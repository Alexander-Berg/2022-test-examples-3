...
const GRADIENT_IN_CSS = 'rgba(0,0,0,0)radial-gradient(106.26%150.99%at-3.13%19.4%,rgb(80,90,221)'
    + '37.5%,rgb(190,64,192)65.1%,rgb(251,168,43)92.19%)repeatscroll0%0%/autopadding-boxtext';

const createExpectedText = (value, format) =>
    `${value} ${format !== 'short' ? getBonusString(value) : ''}${format === 'full' ? ' на Плюс' : ''}`;
...
export default makeSuite('Кешбэк', {
    ...
    story: {
        ...
        'При нажатии у тултипа ожидаемое поведние': makeCase({
            async test() {
                await this.cashbackDealTerms.onClick();
