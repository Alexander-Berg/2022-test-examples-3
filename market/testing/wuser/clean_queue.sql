...
--changeset chmilevfa:MBI-17658.5 endDelimiter:/
CREATE OR REPLACE PROCEDURE wuser.clean_queue(i_table_name IN VARCHAR2)
IS
  ...
  BEGIN
    ...
    FROM MST_RCV_QUEUE WHERE FROM_TRANS_ID = (SELECT min(FROM_TRANS_ID) FROM MST_RCV_QUEUE WHERE table_name = LOWER(i_table_name) AND status IN (2, 6));
    ...
    THEN
      BEGIN
        UPDATE tran_limits SET FIRST_TRANS_ID = broken_to_trans_id, LAST_TRANS_ID = broken_to_trans_id WHERE TABLENAME = UPPER(i_table_name);
        EXECUTE IMMEDIATE 'delete from ' || i_table_name || ' where TRANS_ID <= ' || broken_to_trans_id || ' and trans_id>= ' || broken_from_trans_id;
      END;
    END IF;
  END;
/
