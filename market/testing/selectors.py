"""
This module declares and implements a $TestSelector interface.
"""
...
class AllTestSelector(TestSelector):
    """
    A test selector that selects all test scenarios from the "tests" annotation of the node.
    ...
    def select(self, workflow, subject):
        return workflow[subject]["tests"]


class RegexTestSelector(TestSelector):
    """
    A test selector that selects test scenarios from the "tests" annotation of the node which
