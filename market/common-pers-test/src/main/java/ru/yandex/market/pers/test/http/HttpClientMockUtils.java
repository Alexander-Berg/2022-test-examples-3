...
public class HttpClientMockUtils {
    ...
    public static ArgumentMatcher<HttpUriRequest> withHeader(String header, String value) {
        return request -> {
            Header[] headers = request.getHeaders(header);
            if(value == null && (headers == null || headers.length == 0)) {
                return true;
            }
    ...
    public static <T> ArgumentMatcher<HttpUriRequest> withBody(T expectedBody, Class<T> expectedClass) {
        return request -> {
            try {
                ObjectMapper objectMapper = new ObjectMapper();
                HttpEntity requestEntity = ((HttpEntityEnclosingRequestBase) request).getEntity();
                T actualBody = objectMapper.readValue(requestEntity.getContent(), expectedClass);
                return Objects.equals(expectedBody, actualBody);
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        };
