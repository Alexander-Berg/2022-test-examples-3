...
beforeAll(async () => {
    ...
    await jestLayer.backend.runCode(() => {
        jest.spyOn(require('@self/project/src/resolvers/request/resolveRetpathSync'), 'resolveRetpathSync')
            .mockReturnValue('');
    }, []);
});
...
describe('UserReviews', () => {
    ...
    describe.each([
        ...
        describe('Блок c информацией об авторе.', () => {
            ...
            it('По умолчанию отображается', async () => {
                expect(screen.getByTestId('review-author-headline')).toBeInTheDocument();
            });

            it('Содержит аватарку пользователя', async () => {
                const userInfo = screen.queryByTestId('review-author-headline');
            ...
            it('Содержит правильное имя пользователя', async () => {
                const userInfo = screen.queryByTestId('public-user-info');
        ...
        describe('Блок оценки в отзыве.', () => {
            ...
            it('По умолчанию отображается', async () => {
                expect(screen.getByTestId('review-grade')).toBeInTheDocument();
            });

            it('Количество звезд соответствует оценке', async () => {
                const ratingStars = screen.getByTestId('rating-stars');
                const rating = Number(ratingStars.getAttribute('data-rating-value'));
        ...
        describe('Шапка сниппета отзыва.', () => {
            beforeAll(async () => {
                await prepareKadavrState(kadavrLayer, stateParams);
                /*
                    На текущий момент передача publicId в mandrelLayer.initContext() не поддерживается.
                    Чтобы идентифицировать отзыв как собственный, в контекст передается UID, равный publicId.
                */
            ...
            it('Содержит верную ссылку на товар или магазин', async () => {
                ...
                const href = within(headline).getByRole('link').getAttribute('href');
    ...
    describe('Товарный сниппет отзыва. Футер c лайками и комментариями', () => {
        ...
        it('Кнопка "Комментировать" по умолчанию отображается', async () => {
            expect(screen.getByRole('link', {name: /комментировать/i})).toBeInTheDocument();
        });

        it('Кнопка "Комментировать" ведет на страницу отзыва', async () => {
            const link = screen.getByRole('link', {name: /комментировать/i});
            const href = link.getAttribute('href');
            expect(href).toBe(
                buildUrl('touch:product-review', {
                    productId: String(productCfg.productId),
                    slug: productCfg.slug,
