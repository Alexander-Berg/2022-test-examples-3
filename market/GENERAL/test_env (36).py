...
_STUBS_DIR = os.path.join(yatest.common.source_path(), 'market', 'tools',
                          'wizard-model-reviews-json2pb-converter', 'yatf',
                          'resources', 'stubs')
    ...
    def execute(self, path=None):
        if path is None:
            relative_path = os.path.join('market', 'tools',
                                         'wizard-model-reviews-json2pb-converter',
                                         'wizard-model-reviews-json2pb-converter')
        ...
        model_wizard_reviews_pb_path = os.path.join(
            self.output_dir,
            'best_grades_for_koldunshik.pbuf.sn'
        )

        cmd = [
            path,
            model_wizard_reviews_json_path,
            model_wizard_reviews_pb_path
        ]
        ...
        self.outputs.update({
            'model_wizard_reviews_pb': FileResource(
                model_wizard_reviews_pb_path
            ),
        })
