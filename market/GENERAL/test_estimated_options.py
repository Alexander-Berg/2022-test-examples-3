...
class T(TestCase):
    ...
    def test_long_delivery_offer_filter(self):
        '''
        Проверяем, что оффер с долгой доставкой будет скрыт c флагом market_long_local_delivery
        '''
        ...
        def long_offer_pickup_only_exist(report_response):
            # оффер только с пикапной опцией доставки на данный момент
            # не будет фильтроваться
            self.assertFragmentIn(
                report_response,
                {
                    "entity": "offer",
    ...
    def test_actual_delivery_payment_methods(self):
        # проверяем способы оплаты для офферов с долгой доставкой
        def long_offer_exist(report_response):
            # Проверяем, отображение уникального оффера
        ...
        def long_offer_not_exist(report_response):
            # проверяем, что оффер c долгой доставкой скрыт
            self.assertFragmentNotIn(
                report_response,
                {
                    "entity": "offer",
