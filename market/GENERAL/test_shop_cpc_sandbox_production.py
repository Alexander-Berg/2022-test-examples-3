...
class T(TestCase):
    @classmethod
    def prepare(cls):
        # В этом репорте магазин с cpc=sandbox НЕ должен становиться real
    ...
    def test_shop_cpc_sandbox_must_be_no(self):
        """
        Магазин CPC sandbox, превращается в NO (и CPA, и CPC отключены)
        Оффер CPC

        Делаем запрос оффера, оффера не должно быть
        """
    ...
    def test_shop_cpa_and_cpc_sandbox_must_be_no(self):
        """
        Магазин CPA + CPC sandbox, превращается в CPA-only
        Оффер CPA+CPC

        Делаем запрос оффера, оффер должен быть но без CPC-части: т.е. шифрованного урла нет
        """
