...
import { AnaplanIdFilterProps as ComponentProps, AnaplanIdFilter, FILTER_NAME } from './AnaplanIdFilter';
...
describe('<AnaplanIdFilter />', () => {
  ...
  const renderWithProps = (props: ComponentProps = DEFAULT_PROPS) => {
    return render(<AnaplanIdFilter {...props} />);
  };
  ...
  it('render without errors and contains input with appliedValue', () => {
    renderWithProps({
      ...DEFAULT_PROPS,
      appliedValue: ['#123', '#321', '#555'],
    });

    expect(getInput()).toHaveProperty('value', '#123, #321, #555');
  });
