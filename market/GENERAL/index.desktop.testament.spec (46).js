...
import {AVAILABILITY_OF_PARTNER_PROGRAM_POPUP_ID} from '@self/root/src/constants/referralProgram';
...
import {setup, prepareState, makeContext} from './helpers';
...
describe('Widget: ReferralLandingInfo: Реферальная программа', () => {
    ...
    describe('Попап доступности партнерской программы', () => {
        ...
        it('отображается при первой загрузке страницы + после достижения макс. кол-ва баллов в акции', async () => {
            ...
            /**
             * Кидаем экшн для открытия попапа в ручную, т.к экшн, который отправляется из эпика виджета, не обрабатывается
             * тестаментом. Убрать этот вызов эмиттера сразу как тестамент начнёт понимать экшны в эпиках, сейчас этот тест
             * кейс не ловит все баги.
             */
            new SharedActionEmitter().dispatch({
                type: requestShowPopup.toString(),
                payload: {id: AVAILABILITY_OF_PARTNER_PROGRAM_POPUP_ID},
            });
