#include <market/dynamic_pricing/pricing/dynamic_pricing/autostrategy/optimizer/margin_v1.h>
...
TEST(TestPricingMarginV1, TestLinear_StockUnlimited)
{
    ...
    for (const auto&[id, price] : result.Prices) {
        Y_UNUSED(id);
        EXPECT_EQ(price.NewPrice, expected.Value(price.ShopSkuId, -100));
    }
}
...
TEST(TestPricingMarginV1, Test_UsePromo_Disabled)
{
    ...
    const TMargin targetMargin = 0.2;
    const auto& config = TPricingConfig(CreateUsePromoConfig(targetMargin, TARGET_MARGIN_THRESHOLD, useSkuWithPromo));
    TMarginCalculatorV1 calculator(data, nullPriceRandomizer, metricsCalculator, config);

    const TOutputResult result = calculator.Calculate();
    const TMetric optimalMetric = metricsCalculator.CalcMetric(result.Prices, config.GetSkuFilter());

    EXPECT_NEAR(optimalMetric.Profit, 953, STEP);
    EXPECT_NEAR(optimalMetric.Gmv, 4765, STEP);
    EXPECT_NEAR(optimalMetric.Margin, targetMargin, TARGET_MARGIN_THRESHOLD);
...
TEST(TestPricingMarginV1, Test_UsePromo_Enabled)
{
    ...
    EXPECT_NEAR(optimalMetric.Profit, 953*3, STEP);
    EXPECT_NEAR(optimalMetric.Gmv, 4765*3, STEP);
    EXPECT_NEAR(optimalMetric.Margin, targetMargin, TARGET_MARGIN_THRESHOLD);
