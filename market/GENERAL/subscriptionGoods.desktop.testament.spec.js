...
import {OFFER, PRODUCT, PRODUCT_ID, OFFER_ID, FEE_SHOW} from './__mocks__/subscriptionGoodsMocks';
...
const widgetPath = require.resolve('@self/platform/widgets/content/OfferCardTitle');
...
describe('ProductCardTitle', function () {
    describe('Станция по подписке', () => {
        ...
        test.skip('Указано для цены "/мес"', async () => {
        ...
        // SKIPPED MARKETFRONT-96354
        // eslint-disable-next-line jest/no-disabled-tests
        test.skip('Указано "Вместе с Плюс Мульти"', async () => {
            await makeContext({exps});
            const {container} = await apiaryLayer.mountWidget(widgetPath, widgetProps);
        ...
        // SKIPPED MARKETFRONT-96354
        // eslint-disable-next-line jest/no-disabled-tests
        describe.skip('Кнопка "В корзину"', () => {
