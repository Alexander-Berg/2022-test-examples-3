...
class T(TestCase):
    ...
    def test_offer_with_standart_dimensions(self):
        """
        Проверяем что синие оферы без синих габаритов будут искать их в снипетном
        """
                ...
                "results": [
                    {
                        ...
                        "offers": {
                            "items": [
                                {"wareId": "Sku1Price5-IiLVm1Goleg", "weight": "5", "dimensions": ["20", "30", "10"]}
                            ]
                        },
                    },
                ]
    ...
    def test_offer_with_blue_dimensions(self):
        """
        Проверяем, что если указана синие весогабориты, то в первую очередь учитываются они
        """
        response = self.report.request_json('place=sku_offers&market-sku=2&show-urls=direct&rids=213&rgb=BLUE')
    ...
    def test_white_offer_dimensions(self):
        """
        Проверям что для белых оферов габариты продолжают браться из снипетого
        """
        response = self.report.request_json(
            'place=offerinfo&offerid=whitewhiteoffer111111w&rids=213&show-urls=&regset=1'
        )
