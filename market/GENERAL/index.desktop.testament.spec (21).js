...
import {createProductQuestions, questionText} from './__mocks__/questions';
import {products, productId} from './__mocks__/products';
...
const widgetPath = require.resolve('@self/platform/widgets/parts/ProductQuestionsLayout/QuestionList');
...
describe('ProductQuestionList', () => {
    const widgetProps: Config = {productId: productId};
    ...
    describe.each([
        ...
        describe('вопросы отсутствуют', () => {
            ...
            it('отображаться заголовок', async () => {
                expect(screen.queryByText('Пока никто не задавал вопросов об этом товаре — станьте первым')).toBeInTheDocument();
            });
        ...
        describe('сниппет вопроса на товар', () => {
            const questionsCount = 9;
            const questionsState = createProductQuestions(questionsCount);
            const links = {headerLink: 'market:product', questionLink: 'market:product-question'};
