...
describe('CheckoutParcel', () => {
    ...
    // SKIPPED MARKETFRONT-96354
    // CheckoutParcel Должен содержать ожидаемые данные для офера с ювелиркой [marketfront-5078]
    // Падений > 19%
    // eslint-disable-next-line jest/no-disabled-tests
    ...
    describe.skip('Признак примерки у FBS товара', () => {
        ...
        test('с примеркой [marketfront-6112]', async () => {
        ...
        test('без примерки [marketfront-6113]', async () => {
            await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
    ...
    describe('Информер примерки.', () => {
        describe('Нет сохранённых БПВЗ.', () => {
            test('FBY товар с примеркой. [marketfront-6114]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
                    isFBS: false,
                    deliveryType: 'DELIVERY',
            ...
            test('FBS товар с примеркой [marketfront-6115]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
        ...
        describe('Нет сохранённых БПВЗ, ПВЗ и постаматов.', () => {
            test('Фешн товар без примерки. [marketfront-6116]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
                    deliveryType: 'DELIVERY',
                    hasPickupPresets: false,
                    isTryingAvailable: false,
        ...
        describe('Есть сохранённые БПВЗ.', () => {
            test('FBY товар с примеркой. [marketfront-6117]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
                    isFBS: false,
                }]);
                await apiaryLayer.mountWidget(widgetPath, {
            ...
            test('FBS товар с примеркой. [marketfront-6118]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
                    deliveryType: 'DELIVERY',
                }]);
                await apiaryLayer.mountWidget(widgetPath, {
    ...
    describe('Признак примерки на пресете.', () => {
        describe('Есть сохранённые БПВЗ.', () => {
            test('FBY товар с примеркой. [marketfront-6119]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
                    isFBS: false,
                }]);
                await apiaryLayer.mountWidget(widgetPath, {
                ...
                await checkActiveTab('pickup');
                await hasTryingPresetInfo(ADDRESS.PICKUP);
            });
            test('FBS товар с примеркой. [marketfront-6120]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
        ...
        describe('Есть сохранённые БПВЗ и ПВЗ.', () => {
            test('Фешн товар без примерки. [marketfront-6121]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
                    isTryingAvailable: false,
                }]);
                await apiaryLayer.mountWidget(widgetPath, {
                ...
                const button = screen.getByRole('button', {name: ADDRESS.PICKUP});
                const tryingInfo = within(button).queryByTestId('tryingInfo');
                const icon = within(button).queryByRole('img', {name: /без примерки/i});
                expect(tryingInfo).not.toBeInTheDocument();
                expect(icon).not.toBeInTheDocument();
            });
        });
        describe('Есть сохранённые адреса доставки курьером.', () => {
            test('FBS товар с примеркой. [marketfront-6123]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
                    isTryingAvailable: true,
                    deliveryType: 'DELIVERY',
                }]);
                ...
                await checkActiveTab('delivery');
                await hasWithoutTryingPresetInfo(ADDRESS.DELIVERY);
            });
            test('FBY товар с примеркой. [marketfront-6122]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
                    isFBS: false,
                    deliveryType: 'DELIVERY',
                }]);
                ...
                await checkActiveTab('delivery');
                await hasTryingPresetInfo(ADDRESS.DELIVERY);
            });
            test('Фешн товар без примерки. [marketfront-6124]', async () => {
                await jestLayer.backend.runCode(baseMockFunctionality, ['@self/root/src/widgets/content/checkout/common/__spec__/mockData/fashion', {
                    isTryingAvailable: false,
                    deliveryType: 'DELIVERY',
                }]);
                ...
                const button = screen.getByRole('button', {name: ADDRESS.DELIVERY});
                const tryingInfo = within(button).queryByTestId('tryingInfo');
                const icon = within(button).queryByRole('img', {name: /без примерки/i});
                expect(tryingInfo).not.toBeInTheDocument();
                expect(icon).not.toBeInTheDocument();
