...
const dataProvider = params => {
    const {warningCode, warningText, specificationGroups, themeId} = params;

    const data = compose(
        themeId ? setStateWidgetThemeId(themeId) : identity,
        createInitialState({warningCode, warningText, specificationGroups}),
    )();

    return {
        type: WidgetTypes.Specifications,
        data,
    };
};
