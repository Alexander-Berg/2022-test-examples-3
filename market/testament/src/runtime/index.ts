...
class Runtime {
    ...
    private globalAliasModuleCache: Map<string, any> = new Map<string, any>();
    ...
    static createHasteMap(
        ...
        return new HasteMap({
            ...
            extensions: [Snapshot.EXTENSION].concat(
    ...
    requireModule<T = unknown>(
        ...
        const moduleResource =
            moduleName && this._resolver.getModule(moduleName);
        const manualMock =
        ...
        // be satisfied.
        const localModule: Partial<InitialModule> = {
            children: [],
            exports: {},
            filename: modulePath,
            id: modulePath,
    ...
    requireMock<T = unknown>(from: Config.Path, moduleName: string): T {
        ...
        if (this._mockRegistry.get(moduleID)) {
    ...
    private _loadModule(
        localModule: Partial<InitialModule>,
        from: Config.Path,
        ...
        if (path.extname(modulePath) === '.json') {
            ...
            localModule.exports = (this._environment.global as any).JSON.parse(
                transformedFile,
            );
    ...
    requireModuleOrMock<T = unknown>(from: Config.Path, moduleName: string): T {
        ...
        const aliasModule = this.globalAliasModuleCache.get(moduleName);
        if (aliasModule) {
            return aliasModule;
        }
        ...
        try {
            ...
            if (this._shouldMock(from, moduleName)) {
                result = this.requireMock<T>(from, moduleName);
            } else {
                result = this.requireModule<T>(from, moduleName);
            }

            if (moduleName.startsWith('@self/')) {
                this.globalAliasModuleCache.set(moduleName, result);
            }
            ...
            if (moduleNotFound) {
                if (
                    ...
                    moduleNotFound.siblingWithSimilarExtensionFound ===
                        undefined
                ) {
    ...
    resetModules(): void {
        ...
        if (this._environment) {
            if (this._environment.global) {
                ...
                (Object.keys(envGlobal) as Array<any>).forEach(key => {
    ...
    private _execModule(
        ...
        const origCurrExecutingManualMock =
        ...
        if (typeof this._environment.getVmContext === 'function') {
            ...
            runScript =
                this._environment.runScript<RunScriptEvalResult>(script);
        }
        ...
        const lastArgs: [Jest | undefined, ...Array<Global.Global>] = [
            ...
            ...this._config.extraGlobals.map<Global.Global>(globalVariable => {
                ...
                throw new Error(
                    `You have requested '${globalVariable.toString()}' as a global variable, but it was not present. Please check your config or your global environment.`,
                );
            }),
        ];
    ...
    private transformFile(
        ...
        if (transformedFile.sourceMapPath) {
            this._sourceMapRegistry.set(
                filename,
                transformedFile.sourceMapPath,
            );
        }
    ...
    private _createRequireImplementation(
        ...
        moduleRequire.cache = (() => {
            ...
            return new Proxy<typeof moduleRequire['cache']>(
                Object.create(null),
                {
    ...
    private _createJestObjectFor(from: Config.Path): Jest {
        ...
        const _getFakeTimers = () => {
            if (
                ...
                this._logFormattedReferenceError(
                    'You are trying to access a property or method of the ' +
                        'Jest environment after it has been torn down.',
                );
                process.exitCode = 1;
            }
        ...
        const setTimeout = (timeout: number) => {
            if (this._environment.global.jasmine) {
                this._environment.global.jasmine._DEFAULT_TIMEOUT_INTERVAL =
                    timeout;
            } else {
    ...
    private constructInjectedModuleParameters(): Array<
        ...
        if (moduleNotFoundError) {
            if (!moduleNotFoundError.requireStack) {
                ...
                for (
                    ...
                    moduleNotFoundError.requireStack.push(
