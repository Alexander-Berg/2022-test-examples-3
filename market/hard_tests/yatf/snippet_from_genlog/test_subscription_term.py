...
test_data = [
    {
        'offer_id': '1',
        'expected_subscription_term': None,
    },
    {
        'offer_id': '2',
        'subscription_term': 12,
        'expected_subscription_term': '12',
    },
    {
        'offer_id': '3',
        'subscription_term': 24,
        'expected_subscription_term': '24',
    },
]
...
def genlog_rows():
    ...
    for data in test_data:
        offer = default_genlog(
            ...
            subscription_term=data.get('subscription_term', None),
        )
        feed.append(offer)
    return feed
...
def test_subscription_term_snippet(genlog_snippet_workflow):
    expected_subscription_terms = [
        {
            'offer_id': x['offer_id'],
            'subscription_term': x['expected_subscription_term'],
        }
        for x in test_data
    ]
    for expected in expected_subscription_terms:
        assert_that(genlog_snippet_workflow, HasOutputStateRecord({'value': expected}))
