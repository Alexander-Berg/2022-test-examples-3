...
test_data = [
    {
        'offer_id': '1',
        'model_quantity_value': str(QUANTITY_VALUE),
        'model_quantity_unit': QUANTITY_UNIT,
    },
]
...
def model_quantities():
    return ModelQuantities({1: ['number_new', QUANTITY_VALUE]})
...
def offers_processor_workflow(yt_server, model_quantities, genlog_table):
...
def test_model_quantity_snippet(genlog_snippet_workflow):
    expected_archive = [
        {
            'offer_id': x['offer_id'],
            'model_quantity_value': x['model_quantity_value'],
            'model_quantity_unit': x['model_quantity_unit'],
        }
        for x in test_data
    ]
