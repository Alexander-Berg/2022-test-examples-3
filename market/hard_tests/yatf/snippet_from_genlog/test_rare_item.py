...
test_data = [
    {
        'offer_id': '1',
        'expected_rare_item': None,
    },
    {
        'offer_id': '2',
        'rare_item': True,
        'expected_rare_item': '1',
    },
]
...
def genlog_rows():
    ...
    for data in test_data:
        offer = default_genlog(
            ...
            rare_item=data.get('rare_item', None),
        )
        feed.append(offer)
    return feed
...
def test_rare_item_snippet(genlog_snippet_workflow):
    expected_rare_item = [
        {
            'offer_id': x['offer_id'],
            'rare_item': x['expected_rare_item'],
        }
        for x in test_data
    ]
    for expected in expected_rare_item:
        assert_that(
            genlog_snippet_workflow,
