...
test_data = [
    {
        'offer_id': '1',
        'incoming_step_quantity': 0,
        'expected_step_quantity': '0',
    },
    {
        'offer_id': '2',
        'incoming_step_quantity': 1,
        'expected_step_quantity': '1',
    },
    {
        'offer_id': '3',
        'incoming_step_quantity': 4,
        'expected_step_quantity': '4',
    },
    {
        'offer_id': '4',
        'expected_step_quantity': '1',
...
def genlog_rows():
    ...
    for data in test_data:
        offer = default_genlog(
            ...
            step_quantity=data.get('incoming_step_quantity', None),
        )
        feed.append(offer)
...
def test_step_quantity_snippet(genlog_snippet_workflow):
    expected_step_quantitys = [
        {
            'offer_id': x['offer_id'],
            'step_quantity': x.get('expected_step_quantity', None),
        }
        for x in test_data
    ]
    for expected in expected_step_quantitys:
        assert_that(
            genlog_snippet_workflow,
