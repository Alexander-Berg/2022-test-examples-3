...
public class GetPartnerExternalParamsTest extends AbstractLmsLomTest {

    @Test
    @SneakyThrows
    @DisplayName("Поиск параметров партнёров по типу")
    public void getPartnerExternalParamsByType() {
        List<PartnerExternalParamGroup> lmsPartnerExternalParams =
            LMS_STEPS.getPartnerExternalParam(EXTERNAL_PARAM_TYPES).getEntities();
        List<PartnerExternalParamGroup> ytPartnerExternalParams =
            LOM_LMS_YT_STEPS.getPartnerExternalParamValues(EXTERNAL_PARAM_TYPES);

        GetPartnerExternalParamsResponseCompareUtils.comparePartnerExternalParamGroups(
            softly,
            lmsPartnerExternalParams,
            ytPartnerExternalParams
        );
    }
}
