...
import static ru.yandex.market.checkout.checkouter.order.promo.PromoType.SPREAD_DISCOUNT_COUNT;
...
/**
 * @author Anastasiya Emelianova / orphie@ / 8/2/21
 */
public class SpreadDiscountCountTest extends AbstractWebTestBase {

    private static final String PROMO = "spread-discount-count";
    @Autowired
                ...
                Matchers.allOf(
                        ...
                        hasProperty("promos", Matchers.hasItems(
                                hasProperty("promoDefinition", Matchers.allOf(
                                        hasProperty("type", equalTo(SPREAD_DISCOUNT_COUNT)),
    ...
    @Test
    void noPromoAfterSpendSpreadDiscount() {
        final OrderItemProvider.OrderItemBuilder primaryOffer = orderItemWithSortingCenter()
                .offer(PRIMARY_OFFER)
                .price(1000)
                .count(2);
        ...
        loyaltyConfigurer.mockCalcsWithDynamicResponse(requestParameters);
        loyaltyService.spendDiscounts(cart, createTestContext(primaryOffer, requestParameters.getBuiltMultiCart()));
        loyaltyService.spendDiscounts(cart, createTestContext(secondOffer, requestParameters.getBuiltMultiCart()));
    ...
    private LoyaltyContext createTestContext(OrderItemProvider.OrderItemBuilder orderItemBuilder, MultiCart multiCart) {
        var foundOffers = Collections.singletonList(FoundOfferBuilder.createFrom(orderItemBuilder.build())
                .promoKey(PROMO)
                .promoType(PromoType.SPREAD_COUNT.getCode())
                .build());
