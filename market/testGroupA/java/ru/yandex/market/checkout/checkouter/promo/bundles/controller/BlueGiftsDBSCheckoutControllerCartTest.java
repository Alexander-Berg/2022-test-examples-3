...
public class BlueGiftsDBSCheckoutControllerCartTest extends AbstractWebTestBase {
    ...
    public void configure() {
        primaryOffer = dsbsOrderItem()
                .label("some-id-1")
                .offer(PRIMARY_OFFER)
                .price(10000);

        secondaryOffer = dsbsOrderItem()
                .label("some-id-2")
                .offer(GIFT_OFFER)
                .price(2000);
    ...
    @Test
    public void shouldCheckDSBSOrderWithBundles() {
        MultiCart cart = MultiCartProvider.single(shopDeliveryOrder()
        ...
        Parameters parameters = dropshipRequestFor(cart, reportOffers,
                ShopSettingsHelper::getDsbsShopPrepayMeta, config ->
        ...
        WhiteParametersProvider.applyTo(parameters);
