package ru.yandex.market.checkout.checkouter.pvz;
...
public class PvzClientTest extends AbstractWebTestBase {

    private static final int DEFAULT_NEW_THREAD_ALLOCATION_ERROR_MS = 100;

    @Autowired
    private PvzLogisticsClient pvzLogisticsClient;
    @Autowired
    private WireMockServer pvzMock;
    @Value("${market.checkouter.tpl-pvz.api.readTimeout}")
    private int readTimeout;
    @Value("${market.checkouter.tpl-pvz.api.maxConnectionsPerRoute}")
    private int maxConnectionsPerRoute;
    @Value("${market.checkouter.tpl-pvz.api.connectionRequestTimeout}")
    ...
    public void requestShouldHaveTvmHeader() {
        pvzMock.stubFor(
                get(urlPathEqualTo("/logistics/pickup-point/orders/123/reschedule-expiration"))
                        .willReturn(okJson("[\"2021-08-09\", \"2021-08-10\", \"2021-08-11\", \"2021-08-12\"]")));
        pvzMock.addMockServiceRequestListener((request, response) -> {
        ...
        var response = pvzLogisticsClient.getExpirationDates("123");
    ...
    public void readTimeoutsShouldWork() throws InterruptedException, ExecutionException {
        ...
        try {
            pvzLogisticsClient.getExpirationDates("123");
        } catch (Exception e) {
        }
    ...
    public void connectionRequestTimeoutShouldWork() throws InterruptedException, ExecutionException {
        ...
        + DEFAULT_NEW_THREAD_ALLOCATION_ERROR_MS)));
