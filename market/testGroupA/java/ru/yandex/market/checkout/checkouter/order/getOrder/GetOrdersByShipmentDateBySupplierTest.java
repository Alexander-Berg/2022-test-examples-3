...
class GetOrdersByShipmentDateBySupplierTest extends AbstractWebTestBase {
    ...
    private static Instant extractShipmentDate(Order o) {
        return o.getDelivery().getParcels().get(0)
    ...
    void setUp() {
        ...
        order1 = orderCreateHelper.createOrder(
                buildParameters(ZonedDateTime.parse("2020-04-04T12:00:00-05:00"))
        );
        order2 = orderCreateHelper.createOrder(
                buildParameters(ZonedDateTime.parse("2020-04-05T12:00:00-05:00"))
        );
    }
    ...
    @Test
    void shouldFilterByShipmentDateTo() {
        OrderSearchRequest request = new OrderSearchRequest();
        request.shipmentDateBySupplierTo = Date.from(extractShipmentDate(order2));
    ...
    @Test
    void shouldFilterByShipmentDateFrom() {
        OrderSearchRequest request = new OrderSearchRequest();
        request.shipmentDateBySupplierFrom = Date.from(extractShipmentDate(order2));
    ...
    @Test
    void shouldFilterByShipmentDateFromAndTo() {
        OrderSearchRequest request = new OrderSearchRequest();
        request.shipmentDateBySupplierFrom = Date.from(extractShipmentDate(order1));
        request.shipmentDateBySupplierTo = Date.from(extractShipmentDate(order2));
    ...
    @Test
    void shouldFilterByShipmentDateToAndPaymentStatus() {
        OrderSearchRequest request = new OrderSearchRequest();
        request.shipmentDateBySupplierFrom = Date.from(extractShipmentDate(order2));
    ...
    private Parameters buildParameters(ZonedDateTime shipmentDateTimeBySupplier) {
        Parameters parameters = yandexMarketDeliveryHelper.newMarDoOrderBuilder()
                .withDeliveryServiceId(MOCK_DELIVERY_SERVICE_ID)
                .withShipmentDateTimeBySupplier(shipmentDateTimeBySupplier)
