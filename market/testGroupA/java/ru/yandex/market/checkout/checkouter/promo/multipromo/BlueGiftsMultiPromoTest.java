...
public class BlueGiftsMultiPromoTest extends AbstractWebTestBase {
    ...
    void shouldApplyWithDirectDiscount() {
        ...
        Order order = bundleOrderHelper.createTypicalOrderWithBundles(parameters -> {
            propertiesRef.set(parameters);

            promoConfigurer.importFrom(parameters);

            OrderItem item1 = itemOf(parameters.getOrder(), PRIMARY_OFFER);
            OrderItem item2 = itemOf(parameters.getOrder(), GIFT_OFFER);
        ...
        assertThat(order.getItems(), hasItems(
                allOf(
                        ...
                        hasProperty("buyerPrice", comparesEqualTo(BigDecimal.valueOf(8999))),
                        hasProperty("promos", hasItems(allOf(
                                ...
                                hasProperty("promoDefinition", hasProperty("type",
                                        is(PromoType.GENERIC_BUNDLE))),
                                hasProperty("buyerDiscount", comparesEqualTo(BigDecimal.valueOf(1)))
                        )))
                ),
                allOf(
        ...
        var itemRequest = discountRequest.getOrders().stream()
                ...
                .filter(i -> i.getOfferId().equals(PRIMARY_OFFER))
                .findFirst().orElseThrow();

        assertThat(discountRequest, notNullValue());
        assertThat(itemRequest.getPrice(), comparesEqualTo(BigDecimal.valueOf(9000)));
    ...
    void shouldApplyWithPriceDrop() {
        ...
        assertThat(itemRequest.getPrice(), comparesEqualTo(BigDecimal.valueOf(10000)));
    ...
    void shouldApplyWithPriceDropWithBluePromo() {
        ...
        assertThat(order.getItems(), hasItems(
                allOf(
                        hasProperty("offerId", is(PRIMARY_OFFER)),
                        hasProperty("buyerPrice", comparesEqualTo(BigDecimal.valueOf(9699))),
                        hasProperty("primaryInBundle", is(true)),
                        hasProperty("promos", hasItems(allOf(
                                hasProperty("promoDefinition", hasProperty("type",
                                        is(PromoType.PRICE_DROP_AS_YOU_SHOP))),
        ...
        assertThat(itemRequest.getPrice(), comparesEqualTo(BigDecimal.valueOf(9800)));
        assertThat(itemRequest.getPromoDiscounts(), hasSize(1));
        assertThat(itemRequest.getPromoDiscounts(), hasItem(allOf(
                hasProperty("promoType", is(PromoType.PRICE_DROP_AS_YOU_SHOP.getCode())),
