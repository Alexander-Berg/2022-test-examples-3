...
from dmock import make_dmock_config
...
def _main():
    ...
    arg_parser.add_argument('--model-collection-service', action='store_true')
    ...
    parser_install.add_argument('--enable-dmock', action='store_true', help='Enable dmock for external services')
    ...
    def add_blue_arg(subparser):
        subparser.add_argument('--blue', action='store_true', help='Use blue report logic')
    add_blue_arg(parser_unpack_index)
    add_blue_arg(current_index)
    add_blue_arg(parser_start)
    add_blue_arg(parser_restart)

    subparsers.add_parser('make_dmock_config')
    ...
    if args.command == 'lock':
        ...
        if args.command == 'install':
            ...
            if args.enable_dmock:
                install_dmock(svn_dir, args.cluster)
            ...
            if use_git:
                build_report_from_git_branch(git_dir, svn_dir, git_branch, build_type, args.dist_build, args.extra_build_options, revision_descr, args.model_collection_service)
            else:
                build_custom_report(svn_dir, svn_revision, svn_branch, svn_patch, build_type, args.dist_build, args.extra_build_options, revision_descr, args.model_collection_service)

            report_type = REPORT_NAMES.index(args.config_type)
            generate_config(args.cluster, svn_dir, report_type, args.enable_docfetcher, args.report_port, args.model_collection_service, args.enable_dmock)
            install_custom_report_on_hp(args.cluster, svn_dir, build_type, report_type, args.report_port, args.model_collection_service, args.enable_dmock)
        elif args.command == 'start':
            start_report_on_hp_and_wait(args.cluster, args.model_collection_service, args.blue, BUILD_RELEASE, args.report_port)
        elif args.command == 'stop':
            stop_report_on_hp_and_wait(args.cluster)
        elif args.command == 'restart':
            restart_report_on_hp_and_wait(args.cluster, args.model_collection_service, args.blue, BUILD_RELEASE, args.report_port)
        ...
        elif args.command == 'make_dmock_config':
            make_dmock_config(args.cluster)
