 ...
 */
public class ShopId {
    private static final ImmutableMap<String, String> SHOPS_WITH_DELIVERY_TO_COUNTRY;
    private static final ImmutableMap<String, String> SHOPS_WITH_DELIVERY_TO_REGION;
    private static final ImmutableMap<String, String> SHOPS_FOR_SNG_DELIVERY;
    private static final ImmutableMap<String, String> SHOPS_WITHOUT_DELIVERY_TO_REGION;
    private static final ImmutableMap<String, String> SHOPS_FOREIGN_DELIVERY;
    public static List<String> TEST_SHOPS = ImmutableList.of("774", "64407", "81832", "82036", "59048", "61494");

    static {
        //select shop_id from shops where shop_id is NOT NULL and array_contains(regions,225) limit 500
        Map<String, String> shopGeoId = createMapFromShopIdsWithGeoId("213",
        ...
        SHOPS_WITH_DELIVERY_TO_COUNTRY = ImmutableMap.<String, String>builder().putAll(shopGeoId).build();
    }
    ...
    static {
        ...
        Map<String, String> shopGeoId = createMapFromShopIdsWithGeoId("213",
                ...
                404745);
        SHOPS_WITH_DELIVERY_TO_REGION = ImmutableMap.<String, String>builder().putAll(shopGeoId).build();
    }

    static {
        //select shop_id from shops where array_contains(regions,166) limit 3
        Map<String, String> shopGeoId = createMapFromShopIdsWithGeoId("213", 404057, 399614, 399525);
        SHOPS_FOR_SNG_DELIVERY = ImmutableMap.<String, String>builder().putAll(shopGeoId).build();
    }
    ...
    static {
        ...
        SHOPS_WITHOUT_DELIVERY_TO_REGION = ImmutableMap.<String, String>builder().putAll(shopGeoId).build();
    }

    static {
        Map<String, String> shopGeoId = new HashMap<>();
        ...
        SHOPS_FOREIGN_DELIVERY = ImmutableMap.<String, String>builder().putAll(shopGeoId).build();
    }

    public static Integer generate() {
        return Integer.valueOf(getRandomShopWithDelivery("213"));
    }

    public static String generate(String geoId) {
        return getRandomShopWithDelivery(geoId);
    }

    public static String generateFakeTestShop() {
        return "777" + randomNumberAsId();
    }

    public static String testShop() {
        List<String> testShopsAllowedForTest = new ArrayList<>(TEST_SHOPS);
        testShopsAllowedForTest.remove("774");
        return RandomUtils.choice(testShopsAllowedForTest);
    }

    public static Map<String, String> getShopsWithDeliveryToCountry() {
        return SHOPS_WITH_DELIVERY_TO_COUNTRY;
    }

    public static Map<String, String> getShopsWithDeliveryToRegion() {
        return SHOPS_WITH_DELIVERY_TO_REGION;
    }

    private static Map<String, String> createMapFromShopIdsWithGeoId(String geoId, Integer... shopIds) {
        return Stream.of(shopIds).map(it -> new String[]{String.valueOf(it), geoId})
                .collect(Collectors.toMap(it -> it[0], it -> it[1], (key1, key2) -> key2));
    }

    private static String getRandomShopWithDelivery(String geoId) {
        Map<String, String> shopsWithGeo = RandomUtils.getRnd().nextBoolean() ? getShopsWithDeliveryToRegion()
                : getShopsWithDeliveryToCountry();
        List<String> shopsWithDelivery = shopsWithGeo.entrySet().stream().filter(e -> e.getValue().equals(geoId))
                .map(Map.Entry::getKey).collect(toList());
        if (shopsWithDelivery.isEmpty()) {
            throw new IllegalArgumentException("Сan't find shop with delivery for geo_id " + geoId);
        }
        return RandomUtils.choice(shopsWithDelivery);
    }

    private static String randomNumberAsId() {
        return String.valueOf(RandomUtils.getRnd().nextInt(269000) + 1000);
    }
}
