...
class MskuGenlogFilterTestEnv(BaseEnv):
    ...
    def __init__(self, yt_stuff, rows_feed, **resources):
        super(MskuGenlogFilterTestEnv, self).__init__(**resources)
        ...
        self.msku_filtered_genlog_dir = ypath_join(yt_prefix, 'filtered_genlog')
        self.expected_filtered_genlog_table_path = ypath_join(self.msku_filtered_genlog_dir, SHARD_TABLE)
    ...
    def description(self):
        return 'msku_genlog_filter'

    def execute(self, path=None):
        if path is None:
            relative_path = os.path.join(
                'market', 'idx', 'generation', 'msku_genlog_filter', 'msku_genlog_filter'
            )
        ...
        cmd = [
            ...
            '--yt-src-genlog-dir', self.genlog_input_dir,
            '--yt-dst-filtered-genlog-dir', self.msku_filtered_genlog_dir,
            '--parts-count', str(self.parts_count),
        ]
