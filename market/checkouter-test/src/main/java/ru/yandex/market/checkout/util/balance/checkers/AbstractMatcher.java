...
abstract class AbstractMatcher extends CustomMatcher<JsonElement> {
    ...
    protected static boolean matchValue(JsonObject object, String name, Object expectedValue) {
        ...
        if (expectedValue instanceof Matcher) {
            JsonElement actualElement = object.get(name);
            String actual = actualElement.isJsonObject() ? actualElement.toString() : actualElement.getAsString();
            return ((Matcher) expectedValue).matches(actual);
        }
    ...
    protected static boolean matchDateValue(JsonObject object, String name, Date expectedValue) {
        ...
        BigDecimal seconds = milliseconds.divide(new BigDecimal(TimeUnit.SECONDS.toMillis(1)), 3,
