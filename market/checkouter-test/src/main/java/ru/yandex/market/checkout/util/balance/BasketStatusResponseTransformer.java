...
public class BasketStatusResponseTransformer extends ResponseDefinitionTransformer {

    public static final String BASKET_STATUS_STATE_RESPONSE = "basketStatusStateResponse";
    ...
    private static final Logger LOG = LoggerFactory.getLogger(BasketStatusResponseTransformer.class);
    ...
    private JsonObject buildResponse(CheckBasketParams trustConfig, BalancePaymentStatus status, String purchaseToken) {
        ...
        if (!trustConfig.isEmptyTerminal()) {
        ...
        String rrn = trustConfig.getRrn();
        if (StringUtils.isNotBlank(rrn)) {
            object.addProperty("rrn", rrn);
        }
        String approvalCode = trustConfig.getApprovalCode();
        if (StringUtils.isNotBlank(approvalCode)) {
            object.addProperty("approval_code", approvalCode);
        }
        String userAccount = trustConfig.getUserAccount();
        if (StringUtils.isNotBlank(userAccount)) {
            object.addProperty("user_account", userAccount);
        }
        return object;
    }
    ...
    public String getName() {
        return BASKET_STATUS_STATE_RESPONSE;
    }
}
