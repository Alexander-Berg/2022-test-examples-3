...
class DsCteIntakeReturnQualityRuleProcessorTest : StartrekProcessorTest() {
    @BeforeEach
    fun setUp() {
        clock.setFixed(Instant.parse("2020-11-02T09:30:00.00Z"), DateTimeUtils.MOSCOW_ZONE)
    }

    @DisplayName("Создание тикета для одного просроченного план-факта приемки ДС - ЦТЭ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/ds_cte_intake_return/create_single.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/ds_cte_intake_return/create_single.xml",
    ...
    fun createIssueTest() {
        ...
        assertSoftly {
            issueValues.getOrThrow("summary") shouldBe "[MQM] 01-11-2020: ЦТЭ Cофьино вовремя не принял заказы обратно от Cофьино-2."
            ...
            (issueValues.getOrThrow("tags") as Array<*>).toList() shouldContainExactlyInAnyOrder
                    listOf("Cофьино:172", "Cофьино-2:123")
        }
    }

    @DisplayName("Создание тикета для группы просроченных план-фактов приемки ДС - ЦТЭ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/ds_cte_intake_return/create_aggregated.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/ds_cte_intake_return/create_aggregated.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
