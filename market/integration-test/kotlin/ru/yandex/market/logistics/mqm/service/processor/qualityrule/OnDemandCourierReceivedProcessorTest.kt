...
@DisplayName("Тесты обработчика забора заказа курьером (On Demand)")
class OnDemandCourierReceivedProcessorTest : StartrekProcessorTest() {
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_courier_received/create_ticket_lavka.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_courier_received/create_ticket.xml",
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_courier_received/create_ticket_pvz.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_courier_received/create_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_courier_received/close_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_courier_received/close_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    fun closeIssueTest() {
        handlePlanFacts()
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_courier_received/reopen_ticket.xml")
    ...
    private fun assertCommonTicketValues(
        ...
        softly.assertThat(values.getOrThrow("summary"))
            .isEqualTo("[MQM] Курьер вовремя не забрал заказ 777 от 01-11-2020")
        ...
        softly.assertThat(values.getOrThrow("components") as LongArray).containsExactly(92425)
