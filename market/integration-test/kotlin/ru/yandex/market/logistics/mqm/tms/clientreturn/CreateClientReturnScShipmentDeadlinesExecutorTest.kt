...
@DisplayName("Тест джобы создания план-фактов для возвратных заказов лежащих в СЦ")
internal class CreateClientReturnScShipmentDeadlinesExecutorTest : AbstractContextualTest() {
    ...
    @Autowired
    private lateinit var executor: CreateClientReturnScShipmentDeadlinesExecutor
    ...
    @Test
    @ExpectedDatabase(
        value = "/tms/clientreturn/createClientReturnScShipmentDeadlinesExecutor/after/return_client_deadlines.xml",
    ...
    fun successPlanFactCreation() {
        doReturn(
            listOf(
                createScReturnEvent(10L, "VOZVRAT_SF_PVZ_11", LocalDateTime.of(2021, 3, 1, 11, 0, 0), 13L, "СЦ13"),
                createScReturnEvent(20L, "VOZVRAT_SF_PVZ_21", LocalDateTime.of(2021, 3, 1, 11, 0, 0), 23L, "СЦ23"),
                createScReturnEvent(30L, "VOZVRAT_SF_PVZ_31", LocalDateTime.of(2021, 3, 1, 12, 0, 0), 13L, "СЦ13"),
                createScReturnEvent(40L, "VOZVRAT_SF_PVZ_41", LocalDateTime.of(2021, 3, 1, 12, 0, 0), 49785L, "49785")
            )
        )
            .whenever(ytService)
        ...
        softly.assertThat(selectString).contains("oh.id >= 0", "oh.id < 1000000")
    }

    @Test
    @DatabaseSetup("/tms/clientreturn/createClientReturnScShipmentDeadlinesExecutor/before/do_not_save_copies.xml")
    @ExpectedDatabase(
        value = "/tms/clientreturn/createClientReturnScShipmentDeadlinesExecutor/after/do_not_save_copies.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    fun doNotSaveCopies() {
        ...
        verify(ytService).selectRowsFromTable<Any>(selectStringCaptor.capture(), anyOrNull(), any())
        val selectString: String = selectStringCaptor.firstValue
        softly.assertThat(selectString).contains("oh.id >= 10", "oh.id < 1000010")
    }
