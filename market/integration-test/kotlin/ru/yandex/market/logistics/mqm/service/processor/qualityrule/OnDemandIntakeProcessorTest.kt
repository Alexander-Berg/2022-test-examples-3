...
@DisplayName("Тесты обработчика приемки из MK OnDemand")
class OnDemandIntakeProcessorTest : StartrekProcessorTest() {

    @Autowired
    var pvzContactInformationCache: PvzContactInformationCache? = null

    @BeforeEach
    fun setUp() {
        clock.setFixed(Instant.parse("2021-03-01T19:00:01.00Z"), DateTimeUtils.MOSCOW_ZONE)
        whenever(pvzContactInformationCache!!.getPvzContactInformationByPvz(any()))
            .thenReturn(PvzContactInformation(1, "email1", "phone1", "phone2"))
    }

    @DisplayName("Создание тикета в Startrek для одного просроченного план-факта приемки")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_intake/create_ticket_with_one_planfact.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_intake/create_ticket_with_one_planfact.xml",
    ...
    fun createSingleIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("summary")).isEqualTo(
            "[MQM] 07-11-2020: Лавка вовремя не принял заказы, отгруженные Маркет курьерка."
        )
        softly.assertThat(values.getOrThrow("description")).isEqualTo(
            listOf(
                ...
                "https://tariff-editor.taxi.yandex-team.ru/dragon-orders/" +
                    "102/info?cluster=platform",
                "Дата создания заказа: 01-11-2020",
                "Дедлайн приемки из МК: 07-11-2020 14:00",
                "Трек MK: 101",
                "Трек СД: 102",
                "Email ПВЗ: email1",
                "Телефон ПВЗ: phone1",
        ...
        softly.assertThat(values.getOrThrow("tags") as Array<String?>)
            .containsExactly("Маркет курьерка:987654321", "Лавка:987654322", "Курьерская доставка")
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_intake/create_ticket_with_some_planfacts.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_intake/create_ticket_with_some_planfacts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @DisplayName("Добавление комментариев в тикете Startrek для просроченных план-фактов приемки")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_intake/comment_with_some_planfacts.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_intake/comment_with_some_planfacts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_intake/close_all_planfacts.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_intake/close_all_planfacts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_intake/reopen_ticket.xml")
    ...
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_intake/create_ticket_with_one_planfact.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_intake/schedule_on_work_hour.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    fun doNotCreateTicketIfCalledBeforeWorkHourTest() {
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_intake/ticket_with_some_planfacts.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_intake/group_with_ticket_rescheduled_15_mins.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    fun rescheduleGroupTo15MinsLaterIfCalledAfterWorkingHours() {
        clock.setFixed(Instant.parse("2021-04-01T21:02:30.00Z"), DateTimeUtils.MOSCOW_ZONE)
