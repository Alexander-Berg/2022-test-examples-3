...
class ScOutToFulfillmentInAggregatedProcessorTest : StartrekProcessorTest() {
    @Captor
    lateinit var issueCreateCaptor: ArgumentCaptor<InputStream>

    @BeforeEach
    fun setUp() {
        clock.setFixed(Instant.parse("2020-11-02T09:31:00.00Z"), DateTimeUtils.MOSCOW_ZONE)
    }

    @DisplayName("Создание тикета для одного просроченного план-факта")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/sc_out_to_ff_in/create_single.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/sc_out_to_ff_in/create_single.xml",
    ...
    fun createIssueTest() {
        ...
        assertSoftly {
            issueValues.getOrThrow("summary") shouldBe "02-11-2020: не принято с магистрали из (NO_SC_NAME_FOUND, 100) в (NO_FF_NAME_FOUND, 172)"
            ...
            read.numberOfColumns shouldBe 9
        }
    }

    @DisplayName("Добавление комментариев в тикета Startrek для просроченных план-фактов")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/sc_out_to_ff_in/comment_with_some_planfacts.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/sc_out_to_ff_in/comment_with_some_planfacts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/sc_out_to_ff_in/close_all_planfacts.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/sc_out_to_ff_in/close_all_planfacts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
