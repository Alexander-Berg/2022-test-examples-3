...
@DisplayName("Тесты обработчика вручения заказа пользователю OnDemand")
class OnDemandRecipientTransmitProcessorTest : StartrekProcessorTest() {

    @BeforeEach
    fun setUp() {
        clock.setFixed(Instant.parse("2021-03-01T18:00:50.00Z"), DateTimeUtils.MOSCOW_ZONE)
    }

    @DisplayName("Создание тикета в Startrek для просроченного план-факта на вручение клиенту из Лавки")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_recipient_transmit/create_ticket_lavka.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_recipient_transmit/create_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    fun createIssueTestLavka() {
    ...
    @DisplayName("Создание тикета в Startrek для просроченного план-факта на вручение клиенту из ПВЗ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_recipient_transmit/create_ticket_pvz.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_recipient_transmit/create_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    fun createIssueTestPvz() {
        whenever(issues.create(any()))
            .thenReturn(Issue(null, null, "MONITORINGSNDBX-1", null, 1, EmptyMap(), null))
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_recipient_transmit/close_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_recipient_transmit/close_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    fun closeIssueTest() {
        handlePlanFacts()
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_recipient_transmit/reopen_ticket.xml")
    ...
    private fun assertCommonTicketValues(
        ...
        softly.assertThat(values.getOrThrow("summary"))
            .isEqualTo("[MQM] Заказ 777 от 01-11-2020 не был вовремя вручен пользователю")
        ...
        softly.assertThat(values.getOrThrow("tags") as Array<String?>)
            .containsExactly("Рога и Копыта:9929", partnerTagName, "Заказ вручен клиенту")
        softly.assertThat(values.getOrThrow("components") as LongArray).containsExactly(1234)
