...
@DisplayName("Тесты на создание тикетов для просрочек по переходу OnDemand заказов в финальный статус")
class OnDemandFinalStatusProcessorTest : StartrekProcessorTest() {

    @BeforeEach
    fun setUp() {
        clock.setFixed(Instant.parse("2021-03-01T19:00:50.00Z"), DateTimeUtils.MOSCOW_ZONE)
    }

    @DisplayName("Создание тикета в Startrek для просроченных план-фактов")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_final_status/create_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_final_status/create_ticket.xml",
    ...
    fun createIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("tags") as Array<String?>)
            .containsExactly("Лавка:987654322", "Яндекс.Go", "Финальный статус")
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_final_status/close_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/on_demand_final_status/close_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    fun closeIssueTest() {
        handlePlanFacts()
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_final_status/reopen_ticket.xml")
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/on_demand_final_status/do_not_create.xml")
