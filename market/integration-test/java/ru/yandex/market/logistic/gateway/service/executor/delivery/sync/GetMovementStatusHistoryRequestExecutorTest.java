...
import static ru.yandex.market.logistic.api.model.common.PartnerMethod.GET_MOVEMENT_STATUS_HISTORY_DS;
...
public class GetMovementStatusHistoryRequestExecutorTest extends AbstractIntegrationTest {
    ...
    @Before
    public void setup() {
        mockServer = createMockServerByRequest(GET_MOVEMENT_STATUS_HISTORY_DS);
    }
    ...
    public void executeSuccess() throws Exception {
        ...
        mockMvc.perform(
            post("/delivery/getMovementStatusHistory")
    ...
    public void executeResponseException() {
        doReturn(null).when(deliveryClient).getMovementStatusHistory(any(), any());

        String requestContent = getFileContent("fixtures/request/common/get_movement_status_history/request.json");

        softAssert.assertThatThrownBy(
            () -> mockMvc.perform(post("/delivery/getMovementStatusHistory")
                .contentType(APPLICATION_JSON)
                .content(requestContent))
        )
    ...
    public void executeWithoutPartnerId() throws Exception {
        ...
        var exception = mockMvc.perform(post("/delivery/getMovementStatusHistory")
            .contentType(APPLICATION_JSON)
            .content(requestContent))
