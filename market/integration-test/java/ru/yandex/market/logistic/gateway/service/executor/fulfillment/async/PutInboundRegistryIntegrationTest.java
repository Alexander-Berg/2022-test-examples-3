...
import static ru.yandex.market.logistic.api.model.common.PartnerMethod.PUT_INBOUND_REGISTRY_FF;
...
public class PutInboundRegistryIntegrationTest extends AbstractIntegrationTest {
    ...
    @Before
    public void setup() {
        mockServer = createMockServerByRequest(PUT_INBOUND_REGISTRY_FF);
    ...
    public void executeSuccess() throws Exception {
        ClientTask task = getClientTask("fixtures/executors/put_inbound_registry/task_message_ff.json");
        when(repository.findTask(eq(TASK_ID))).thenReturn(task);

        prepareMockServerXmlScenario(
            mockServer,
            "https://localhost/put-inbound-registry",
            "fixtures/request/common/put_inbound_registry/request_ff.xml",
