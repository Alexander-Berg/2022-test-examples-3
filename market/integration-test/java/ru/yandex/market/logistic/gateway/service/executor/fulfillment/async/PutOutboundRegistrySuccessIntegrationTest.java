...
public class PutOutboundRegistrySuccessIntegrationTest extends AbstractIntegrationTest {
    ...
    public void executeSuccessTm() {
        ...
        softAssert.assertThat(message.getMessageBody()).isNull();
        verify(transportManagerClient).setPutOutboundRegistrySuccess(
            eq(123456L),
            eq("5927638"),
            eq("39292337"),
            eq(145L)
    ...
    private TaskMessage execute(TaskResultConsumer consumer) {
       ...
       return putOutboundRegistrySuccessExecutor.execute(new ExecutorTaskWrapper(TASK_ID, 0));
    }
    ...
    private ClientTask getClientTask(String filename, TaskResultConsumer consumer) {
        ...
        task.setFlow(RequestFlow.FF_PUT_OUTBOUND_REGISTRY_SUCCESS);
