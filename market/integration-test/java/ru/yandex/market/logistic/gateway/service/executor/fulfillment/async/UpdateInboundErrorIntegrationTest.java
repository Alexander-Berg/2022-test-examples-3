...
/**
 * Интеграционный тест для {@link UpdateInboundErrorExecutor}.
 */
public class UpdateInboundErrorIntegrationTest extends AbstractIntegrationTest {
    ...
    @Autowired
    private UpdateInboundErrorExecutor updateInboundErrorExecutor;
    ...
    public void executeError() throws Throwable {
        ClientTask task =
            getUpdateInboundErrorTask("fixtures/executors/error_task_message.json");
        ...
        prepareMockServerJsonScenario(mockServer, once(),
            ffWorkHost + "/requests/" + YANDEX_ID + "/reject-updating", null);

        updateInboundErrorExecutor.execute(new ExecutorTaskWrapper(INBOUND_ERROR_TASK_ID, 0));

        verify(aspect).executeWithLogging(any(ProceedingJoinPoint.class));
    }

    private ClientTask getUpdateInboundErrorTask(final String filename) throws IOException {
        ClientTask task = new ClientTask();
        task.setId(INBOUND_ERROR_TASK_ID);
        task.setRootId(INBOUND_TASK_ID);
        task.setParentId(INBOUND_TASK_ID);
        task.setStatus(TaskStatus.IN_PROGRESS);
        task.setFlow(RequestFlow.FF_UPDATE_INBOUND_ERROR);
