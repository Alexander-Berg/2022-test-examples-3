...
public class UpdateOrderDeliveryDateErrorIntegrationTest extends AbstractIntegrationTest {
    ...
    @Autowired
    private UpdateOrderDeliveryDateErrorExecutor updateOrderDeliveryDateErrorExecutor;

    @Test
    public void executeWhenBusinessErrorIsThrown() throws Exception {
        ClientTask task = getTask("fixtures/executors/update_order/update_business_error_task_message.json");
        ...
        updateOrderDeliveryDateErrorExecutor.execute(new ExecutorTaskWrapper(TASK_ID, 0));

        verify(mdbClient).setUpdateDeliveryDateError(anyLong(), anyLong(), anyString());
    }

    @Test
    public void executeWhenTechnicalErrorIsThrown() throws Exception {
        ClientTask task = getTask("fixtures/executors/update_order/update_technical_error_task_message.json");
        ClientTask parentTask = getParentTask();
    ...
    private ClientTask getTask(String filename) {
        ...
        task.setFlow(RequestFlow.DS_UPDATE_ORDER_DELIVERY_DATE_ERROR);
    ...
    private ClientTask getParentTask() {
        ...
        task.setMessage(getFileContent("fixtures/executors/update_order/update_order_delivery_date_task_message.json"));
        task.setConsumer(TaskResultConsumer.MDB);
        return task;
    }
}
