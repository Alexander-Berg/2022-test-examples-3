...
/**
 * Интеграционный тест для {@link CreateTransferSuccessExecutor}.
 */
public class CreateTransferSuccessIntegrationTest extends AbstractIntegrationTest {

    private final static long TRANSFERS_SUCCESS_TASK_ID = 564L;
    ...
    @Autowired
    private CreateTransferSuccessExecutor createTransferSuccessExecutor;
    ...
    public void executeSuccess() throws Throwable {
        ClientTask task =
            getCreateTransferSuccessTask("fixtures/executors/create_transfer/create_transfer_executor_response.json");

        when(repository.findTask(eq(TRANSFERS_SUCCESS_TASK_ID))).thenReturn(task);

        prepareMockServerJsonScenario(mockServer, once(),
            ffWorkHost + "/requests/" + YANDEX_ID + "/accept-by-service",
            "fixtures/request/fulfillment/create_transfer/fulfillment_create_transfer_success.json", null);

        createTransferSuccessExecutor.execute(new ExecutorTaskWrapper(TRANSFERS_SUCCESS_TASK_ID, 0));
        verify(aspect).executeWithLogging(any(ProceedingJoinPoint.class));
    }

    private ClientTask getCreateTransferSuccessTask(final String filename) throws IOException {
        ClientTask task = new ClientTask();
        task.setId(TRANSFERS_SUCCESS_TASK_ID);
        task.setStatus(TaskStatus.IN_PROGRESS);
        task.setFlow(RequestFlow.FF_CREATE_TRANSFER_SUCCESS);
