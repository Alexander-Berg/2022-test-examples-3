...
public class GetMovementErrorIntegrationTest extends AbstractIntegrationTest {
    ...
    public void executeError() {
        ...
        ClientTask parentTask =
            getParentClientTask("fixtures/executors/get_movement/get_movement_task_message.json");

        when(repository.findTask(TASK_ID)).thenReturn(task);
        when(repository.findTask(PARENT_TASK_ID)).thenReturn(parentTask);

        TaskMessage message = getMovementErrorExecutor.execute(new ExecutorTaskWrapper(TASK_ID, 0));

        softAssert.assertThat(message.getMessageBody()).isNull();

        verify(tmDeliveryConsumerClient).setGetMovementError(
    ...
    private ClientTask getClientTask(String filename) {
        return new ClientTask()
            ...
            .setFlow(RequestFlow.DS_GET_MOVEMENT_ERROR)
