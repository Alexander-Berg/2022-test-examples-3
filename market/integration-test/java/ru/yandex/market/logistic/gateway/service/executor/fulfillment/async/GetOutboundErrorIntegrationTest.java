...
public class GetOutboundErrorIntegrationTest extends AbstractIntegrationTest {
    ...
    @Autowired
    private GetOutboundErrorExecutor getOutboundErrorExecutor;

    @Test
    public void executeError() {
        ClientTask task = getClientTask("fixtures/executors/get_outbound/error_task_message.json");
        ClientTask parentTask = getParentClientTask("fixtures/executors/get_outbound/task_message.json");

        when(repository.findTask(TASK_ID)).thenReturn(task);
        when(repository.findTask(PARENT_TASK_ID)).thenReturn(parentTask);

        TaskMessage message = getOutboundErrorExecutor.execute(new ExecutorTaskWrapper(TASK_ID, 0));

        softAssert.assertThat(message.getMessageBody()).isNull();

        verify(fulfillmentWorkflowAsyncClientApi).setFfGetOutboundError(
            eq("5927638"),
            eq("39292337"),
            eq(145L),
            eq(PROCESS_ID),
            eq("error while get outbound method")
        );
    }
    ...
    private ClientTask getClientTask(String filename) {
        return new ClientTask()
            ...
            .setFlow(RequestFlow.FF_GET_OUTBOUND_ERROR)
    ...
    private ClientTask getParentClientTask(String filename) {
        return new ClientTask()
            ...
            .setFlow(RequestFlow.FF_GET_OUTBOUND)
            .setMessage(getFileContent(filename));
    }
