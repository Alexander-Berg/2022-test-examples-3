...
/**
 * Интеграционный тест для {@link CancelOutboundErrorExecutor}.
 */
public class CancelOutboundErrorIntegrationTest extends AbstractIntegrationTest {

    private final static long OUTBOUND_ERROR_TASK_ID = 840L;

    private final static long OUTBOUND_TASK_ID = 400L;
    ...
    @Autowired
    private CancelOutboundErrorExecutor cancelOutboundErrorExecutor;
    ...
    public void executeError() throws Exception {
        ClientTask task = getCancelOutboundErrorTask("fixtures/executors/error_task_message.json");

        ClientTask parentTask = getCancelOutboundTask("fixtures/executors/cancel_outbound/cancel_outbound_task_message.json");
        ...
        cancelOutboundErrorExecutor.execute(new ExecutorTaskWrapper(OUTBOUND_ERROR_TASK_ID, 0));
    }

    private ClientTask getCancelOutboundErrorTask(String filename) throws IOException {
        ClientTask task = new ClientTask();
        task.setId(OUTBOUND_ERROR_TASK_ID);
        task.setParentId(OUTBOUND_TASK_ID);
        task.setRootId(OUTBOUND_TASK_ID);
        task.setStatus(TaskStatus.IN_PROGRESS);
        task.setFlow(RequestFlow.FF_CANCEL_OUTBOUND_ERROR);
