...
public class CreateRegisterErrorIntegrationTest extends AbstractIntegrationTest {
    private static final long CREATE_REGISTER_ERROR_TASK_ID = 726L;

    private static final long CREATE_REGISTER_ERROR_PARENT_TASK_ID = 543L;
    ...
    @Before
    public void setUp() {
        doNothing().when(fulfillmentWorkflowConsumerClient).setCreateRegisterError(
            anyString(),
            anyString(),
            anyString()
        );
    ...
    public void successExecutorBaseWorkflow() {
        ...
        ClientTask parentTask =
            getCreateRegisterParentTask("fixtures/executors/fulfillment_create_order/fulfillment_create_register_with_all_parameters.json");
        ...
        TaskMessage message = createRegisterErrorExecutor.execute(new ExecutorTaskWrapper(CREATE_REGISTER_ERROR_TASK_ID, 0));
        softAssert.assertThat(message.getMessageBody()).isNull();

        verify(fulfillmentWorkflowConsumerClient).setCreateRegisterError(
            eq("1"),
            isNull(),
    ...
    private ClientTask getCreateRegisterErrorTask(String filename) {
        ...
        task.setFlow(RequestFlow.FF_CREATE_REGISTER_ERROR);
