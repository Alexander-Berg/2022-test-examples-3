...
import ru.yandex.market.delivery.tracker.client.tracking.lgw.ff.LgwFfTransferClientAdapter;
import ru.yandex.market.logistic.gateway.client.FulfillmentClient;
import ru.yandex.market.logistic.gateway.common.model.fulfillment.TransferStatusType;

class LgwFfTransferRequestResponseParsingTest extends AbstractLgwParsingTest {

    private static final long SERVICE_ID = 145L;
    private static final String HISTORY_URL = "/fulfillment/getTransferHistory";
    private static final String STATUS_URL = "/fulfillment/getTransfersStatus";

    private LgwFfTransferClientAdapter lgwFfTransferClientAdapter;

    @BeforeEach
    void additionalSetup() {
        FulfillmentClient fulfillmentClient = configuration.fulfillmentClient();
        lgwFfTransferClientAdapter = new LgwFfTransferClientAdapter(fulfillmentClient);
    }

    @Test
    void ffGetTransfersStatus() throws ParseException, IOException {
        ...
        List<TrackerHistory> expectedPayload = List.of(
            new TrackerHistory(
                ...
                List.of(
                    TrackerStatus.builder()
                        .code(TransferStatusType.PROCESSING.getCode())
            ...
            new TrackerHistory(
                ...
                List.of(
                    TrackerStatus.builder()
                        .code(TransferStatusType.ACCEPTED.getCode())
        ...
        setupMockServerExpectation(STATUS_URL, "ff_get_transfers_status.json");

        TrackerResponse<List<TrackerHistory>> response = lgwFfTransferClientAdapter.getStatus(
            SERVICE_ID,
            List.of(
                new TrackerEntityId(yandexIds[0], partnerIds[0]),
                new TrackerEntityId(yandexIds[1], partnerIds[1])
    ...
    @Test
    void ffGetTransferHistory() throws ParseException, IOException {
        String yandexId = "400";
        ...
        TrackerHistory expectedPayload = new TrackerHistory(
            ...
            List.of(
                TrackerStatus.builder()
                    .code(TransferStatusType.COMPLETED.getCode())
                    .date(ParsingUtils.parseDate("2018-12-21T11:59:59+03:00"))
                    .build()
            )
        );

        setupMockServerExpectation(HISTORY_URL, "ff_get_transfer_history.json");

        TrackerResponse<TrackerHistory> response = lgwFfTransferClientAdapter.getHistory(
            SERVICE_ID,
            new TrackerEntityId(yandexId, partnerId)
        );
