package ru.yandex.market.delivery.transport_manager.facade.anomaly;
...
class AnomalyLinehaulFacadeTest extends AbstractContextualTest {
    public static final long RETURN_SORTING_CENTER_PARTNER_ID = 555;
    public static final long ANOMALIES_TARGET_LOGISTICS_POINT_ID = -1;

    @Autowired
    private AnomalyLinehaulFacade facade;
    @Autowired
    ...
    @ExpectedDatabase(
        value = "/repository/transportation/after/anomaly_linehaul.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    @Test
    void create() {
        mockProperty(TmPropertyKey.RETURN_SORTING_CENTER_PARTNER_ID, RETURN_SORTING_CENTER_PARTNER_ID);
        mockProperty(TmPropertyKey.ANOMALIES_TARGET_LOGISTICS_POINT_ID, ANOMALIES_TARGET_LOGISTICS_POINT_ID);
        ...
        facade.create(1L, 2L, LocalDateTime.of(2021, 12, 1, 10, 0), 33);

        verify(lmsClient).getLogisticsPoints(Mockito.eq(
