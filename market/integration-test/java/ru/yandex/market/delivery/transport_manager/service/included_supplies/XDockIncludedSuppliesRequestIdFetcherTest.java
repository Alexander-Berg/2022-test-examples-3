...
class XDockIncludedSuppliesRequestIdFetcherTest extends AbstractContextualTest {
    @Autowired
    private XDockIncludedSuppliesRequestIdFetcher fetcher;

    @DisplayName("Получить id реквестов по штрихкодам и большому перемещению")
    @Test
    ...
    void getRequestIds() {
        softly
            .assertThat(fetcher.getRequestIds(
                ...
                new Transportation()
                    .setOutboundUnit(new TransportationUnit().setLogisticPointId(2L))
            ))
