...
@Log4j2
public class MonitorNumPendingJobsTest {
    @Value("${service.url}")
    ...
    @Test
    public void test() {
        String responseMsgErr = target.path("monitor_num_pending_jobs")
            .queryParam("is_forecaster", true).request().get(String.class);
        assertThat(responseMsgErr, is("2;{\"pending: 301\"}"));
        metadataJdbcTemplate.getJdbcOperations().execute("truncate jobs");
        String responseMsgOk = target.path("monitor_num_pending_jobs")
            .queryParam("is_forecaster", true).request().get(String.class);
