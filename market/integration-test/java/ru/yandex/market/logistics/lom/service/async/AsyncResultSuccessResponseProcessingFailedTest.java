...
class AsyncResultSuccessResponseProcessingFailedTest extends AsyncResultServiceTest {
    @ParameterizedTest(name = TUPLE_PARAMETERIZED_DISPLAY_NAME)
    @MethodSource("successPayloadArguments")
    @DisplayName("Ошибка обработки ответа об успехе операции")
    void successResponseProcessingFailed(
        QueueType queueType,
        MockHttpServletRequestBuilder requestBuilder,
        Object payload
    ) throws Exception {
        AsyncResultService<?, ?> service = getService(queueType);
        doThrow(new BadRequestException(MESSAGE)).when(service).processSuccess(any(), any());
        performRequest(requestBuilder, payload)
            .andExpect(status().isBadRequest())
            .andExpect(errorMessage(MESSAGE));
        assertBusinessProcessStateUpdated(
            queueType,
            BusinessProcessStatus.SUCCESS_RESPONSE_PROCESSING_FAILED,
            MESSAGE
        );
    }
}
