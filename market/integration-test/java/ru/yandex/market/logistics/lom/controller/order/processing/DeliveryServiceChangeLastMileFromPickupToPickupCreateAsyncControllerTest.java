...
@DisplayName("Обработка cor CHANGE_LAST_MILE_FROM_PICKUP_TO_PICKUP после получения ds-create-order коллбэка")
@DatabaseSetup(value = {
...
public class DeliveryServiceChangeLastMileFromPickupToPickupCreateAsyncControllerTest extends AbstractContextualTest {

    private static final Long PICKUP_PARTNER_ID = 51L;
    private static final String BARCODE = "1001";
    private static final String EXTERNAL_ID = "new-pickup-external-id";

    @Test
    @DisplayName("Успешный ответ, создана задача PROCESS_CREATE_ORDER_ASYNC_SUCCESS_RESULT")
    @ExpectedDatabase(
        value = "/controller/order/processing/change_last_mile_from_pickup_to_pickup/create/after/success.xml",
    ...
    @ExpectedDatabase(
        value = "/controller/order/processing/change_last_mile_from_pickup_to_pickup/create/after/error.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    @SneakyThrows
    void errorResponse() {
