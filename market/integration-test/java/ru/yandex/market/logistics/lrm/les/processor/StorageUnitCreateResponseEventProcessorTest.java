...
@DisplayName("Обработка ответа о создании грузомест в СЦ")
@DatabaseSetup("/database/tasks/return-segment/create-in-sc/response/before/prepare.xml")
class StorageUnitCreateResponseEventProcessorTest extends AbstractIntegrationTest {
    @Autowired
    private AsyncLesEventProcessor processor;

    @Test
    @DisplayName("Процесс существует, в ответе есть ошибка - статус FAIL, ставим новую задачу на создание грузомест")
    @ExpectedDatabase(
        value = "/database/tasks/return-segment/create-in-sc/response/after/fail_create_new_task.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT
    )
    void processExistResponseWithErrorCreateNewTask() {
        processor.execute(getDbQueuePayload(
            new StorageUnitCreateResponseEvent(
                TEST_REQUEST_ID + "/1",
    ...
    @Test
    @DisplayName("Процесс существует, в ответе есть ошибка - статус FAIL, достигнуто пороговое значение, логгируем")
    @DatabaseSetup(
        value = "/database/tasks/return-segment/create-in-sc/response/before/fail_threshold_reached.xml",
    ...
    void processExistResponseWithErrorThresholdReached() {
        processor.execute(getDbQueuePayload(
            new StorageUnitCreateResponseEvent(
                TEST_REQUEST_ID + "/1",
                new ResultDto(List.of(new StorageUnitResponseErrorDto(
        ...
        softly.assertThat(backLogCaptor.getResults().toString())
            .contains(
                """
                    level=ERROR\t\
                    format=plain\t\
                    code=COUNT_FAILED_BUSINESS_PROCESS_STATES\t\
                    payload=Reached threshold for creating return in SC for segment=2\t\
                    request_id=test-request-id/1\t\
                    tags=STATISTICS\t\
                    extra_keys=segmentId,count,type\t\
                    extra_values=2,3,CREATE_STORAGE_UNITS_IN_SC
                    """
            );
    }
    ...
    @ExpectedDatabase(
        value = "/database/tasks/return-segment/create-in-sc/response/after/success.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT
    )
    @ExpectedDatabase(
        value = "/database/tasks/no_tasks.xml",
    ...
    @MethodSource
    @ExpectedDatabase(
        value = "/database/tasks/return-segment/create-in-sc/response/before/prepare.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT
    )
    ...
    private static Stream<Arguments> invalid() {
        return Stream.of(
            Arguments.of(
                ...
                "reqId100",
                "No business process with requestId reqId100 and type CREATE_STORAGE_UNITS_IN_SC"
            ),
            Arguments.of(
                "По типу нет процесса",
                "test-request-id-3",
                "No business process with requestId test-request-id-3 and type CREATE_STORAGE_UNITS_IN_SC"
            ),
            Arguments.of(
    ...
    @Nonnull
    private StorageUnitCreateResponseEvent successEvent(String requestId) {
        return new StorageUnitCreateResponseEvent(requestId, new ResultDto(List.of()));
    }
}
