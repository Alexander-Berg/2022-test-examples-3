...
@DisplayName("Тесты процессора по созданию тикетов для просрочек приемки на СД для обратной логистики")
public class DsIntakeReturnProcessorTest extends StartrekProcessorTest {
    ...
    @Test
    @DisplayName("Создание тикета для одного просроченного план-факта приемки обратно на СД")
    @DatabaseSetup("/service/processor/qualityrule/before/ds_intake_return/create_single.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/ds_intake_return/create_single.xml",
    ...
    void createSingleIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("summary"))
            .isEqualTo("[MQM] 01-11-2020: СД Какая-то СД вовремя не приняла заказы обратно от СД последней мили.");
        softly.assertThat(values.getOrThrow("description"))
            .isEqualTo(
                String.join(
                    ...
                    "Дедлайн приемки обратно на СД: 01-11-2020 15:00",
                    "Трек СД: 101",
                    "Трек службы последней мили: 102",
                    "Email службы последней мили: email1@mail.com"
                )
            );
        ...
        softly.assertThat(((String[]) values.getOrThrow("tags")))
            .containsExactly("Какая-то СД:123", "СД последней мили:345");
    }

    @Test
    @DisplayName("Создание тикета для группы просроченных план-фактов приемки на ФФ с СД")
    @DatabaseSetup("/service/processor/qualityrule/before/ds_intake_return/create_aggregated.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/ds_intake_return/create_aggregated.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
