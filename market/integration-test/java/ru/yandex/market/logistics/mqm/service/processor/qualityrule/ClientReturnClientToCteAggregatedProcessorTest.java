...
public class ClientReturnClientToCteAggregatedProcessorTest extends StartrekProcessorTest {
    ...
    @DisplayName("Создание тикета в Startrek для группы просроченных план-фактов от клиента до ЦТЭ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/client_cte/create_group_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/client_cte/create_group_ticket.xml",
    ...
    void createAggregatedIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("summary")).isEqualTo(
            "[MQM] 06-11-2020: Клиентские возвраты не доехали до ЦТЭ за 8 дней.");
        ...
        softly.assertThat((long[]) values.getOrThrow("components")).containsExactly(94923L);

        verify(attachments).upload(anyString(), any());
    }

    @DisplayName("Добавление комментариев в тикета Startrek для просроченных план-фактов от клиента до ЦТЭ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/client_cte/add_comment.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/client_cte/add_comment.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    void commentAggregatedIssueTest() {
        ...
        softly.assertThat(commentString).isEqualTo(String.join(
            ...
            "",
            "Удалены неактуальные заказы (1 шт.): 22.",
            "Добавлены новые заказы (1 шт.): 32.",
    ...
    @DisplayName("Закрытие тикета Startrek для просроченных план-фактов от клиента до ЦТЭ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/client_cte/close_issue.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/client_cte/close_issue.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/client_cte/reopen_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/client_cte/reopen_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/client_cte/create_group_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/client_cte/reschedule_ticket_creation.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    void doNotCreateAggregatedIssueInWrongTimeTest() {
        clock.setFixed(Instant.parse("2020-11-07T23:15:00.00Z"), MOSCOW_ZONE);
        Attachment attachment = Mockito.mock(Attachment.class);
