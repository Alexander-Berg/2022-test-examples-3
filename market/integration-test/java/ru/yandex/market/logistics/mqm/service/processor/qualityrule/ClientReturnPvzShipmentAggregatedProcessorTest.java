...
class ClientReturnPvzShipmentAggregatedProcessorTest extends StartrekProcessorTest {
    ...
    @DisplayName("Создание тикета в Startrek для группы просроченных план-фактов на сегменте забора из ПВЗ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/pvz_shipment/create_group_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/pvz_shipment/create_group_ticket.xml",
    ...
    void createAggregatedIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("summary"))
            .isEqualTo("[MQM] 06-11-2020: Клиентские возвраты не забрали из ПВЗ за 2 дня. Белгородская область.");
        ...
        softly.assertThat((long[]) values.getOrThrow("components")).containsExactly(94584L);
        softly.assertThat((String[]) values.getOrThrow("tags")).containsExactlyInAnyOrder("Белгородская область");

        verify(attachments).upload(anyString(), any());
    }

    @DisplayName("Добавление комментариев в тикета Startrek для просроченных план-фактов на сегменте забора из ПВЗ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/pvz_shipment/add_comment.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/pvz_shipment/add_comment.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @DisplayName("Закрытие тикета Startrek для просроченных план-фактов на сегменте забора из ПВЗ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/pvz_shipment/close_issue.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/pvz_shipment/close_issue.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/pvz_shipment/reopen_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/pvz_shipment/reopen_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/pvz_shipment/rescheduling.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/pvz_shipment/rescheduling.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
