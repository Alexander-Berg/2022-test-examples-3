...
class ClientReturnFirstCteIntakeAggregatedProcessorTest extends StartrekProcessorTest {
    @BeforeEach
    public void setUp() {
        clock.setFixed(Instant.parse("2020-11-07T11:00:01.00Z"), MOSCOW_ZONE);
    }

    @DisplayName("Создание тикета в Startrek для группы просроченных план-фактов на сегменте первичной приемки в ЦТЭ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/first_cte_intake/create_group_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/first_cte_intake/create_group_ticket.xml",
    ...
    void createAggregatedIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("summary"))
            .isEqualTo("[MQM] 06-11-2020: Клиентские возвраты от СЦ \"Имя СЦ 1\" "
                + "не были вовремя приняты на первичной приемке ЦТЭ.");
        ...
        softly.assertThat((long[]) values.getOrThrow("components")).containsExactly(94922L);
    ...
    @DisplayName("Добавление комментариев в тикете для просроченных план-фактов на сегменте первичной приемки в ЦТЭ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/first_cte_intake/add_comment.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/first_cte_intake/add_comment.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @DisplayName("Закрытие тикета Startrek для просроченных план-фактов на сегменте первичной приемки в ЦТЭ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/first_cte_intake/close_issue.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/first_cte_intake/close_issue.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/first_cte_intake/reopen_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/first_cte_intake/reopen_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/first_cte_intake/rescheduling.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/first_cte_intake/rescheduling.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
