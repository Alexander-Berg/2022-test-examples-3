...
public class PickupPointToSortingCenterIntakeAggregatedProcessorTest extends StartrekProcessorTest {
    ...
    @DisplayName("Создание тикета в Startrek для группы просроченных план-фактов курьеру до СЦ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/locker_sc/create_group_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/locker_sc/create_group_ticket.xml",
    ...
    void createAggregatedIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("summary")).isEqualTo(
            "[MQM] 06-11-2020: СЦ Some sorting center вовремя не получил заказы.");
        ...
        softly.assertThat((long[]) values.getOrThrow("components")).containsExactly(94920L);
        softly.assertThat((String[]) values.getOrThrow("tags"))
            .containsExactlyInAnyOrder("Some sorting center:10203");

        verify(attachments).upload(anyString(), any());
    }

    @DisplayName("Добавление комментариев в тикета Startrek для просроченных план-фактов отгрузки курьеру до СЦ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/locker_sc/add_comment.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/locker_sc/add_comment.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    void commentAggregatedIssueTest() {
        ...
        softly.assertThat(commentString).isEqualTo(String.join(
            ...
            "",
            "Удалены неактуальные заказы (1 шт.): 20.",
            "Добавлены новые заказы (1 шт.): 30.",
    ...
    @DisplayName("Закрытие тикета Startrek для просроченных план-фактов отгрузки курьеру до СЦ")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/locker_sc/close_issue.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/locker_sc/close_issue.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @DisplayName("Не переоткрывать тикеты, даже если проблемы по нему актуальны")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/locker_sc/reopen_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/clientreturn/locker_sc/reopen_ticket.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    void reopenClosedIssueFlowTest() {
        ...
        handleGroups();
        verify(transitions, times(0)).execute(eq("MONITORINGSNDBX-1"), eq(transition));
    }
    ...
    @DatabaseSetup("/service/processor/qualityrule/before/clientreturn/locker_sc/create_group_ticket.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/before/clientreturn/locker_sc/reschedule_ticket_creation.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    void doNotCreateAggregatedIssueInWrongTimeTest() {
        clock.setFixed(Instant.parse("2020-11-07T11:15:00.00Z"), MOSCOW_ZONE);
        Attachment attachment = Mockito.mock(Attachment.class);
