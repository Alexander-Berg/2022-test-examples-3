...
@DisplayName("Тесты обработчика ФФ-СД для создания и обновления тикетов по просрочки")
class FfDsIntakeQualityRuleProcessorTest extends StartrekProcessorTest {
    ...
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/intake/ff_ds/create_ticket_with_one_planfact.xml")
    @ExpectedDatabase(
        value =
            "/service/processor/qualityrule/after/intake/ff_ds/scheduled_today_when_work_starts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    void doNotCreateTicketIfBeforeWorkingHoursTest() {
        clock.setFixed(
            LocalDateTime.of(
    ...
    @DisplayName("Проверка, что процессор не создает тикет, если был вызван после рабочих часов")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/intake/ff_ds/create_ticket_with_one_planfact.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/intake/ff_ds/scheduled_tomorrow_when_work_starts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    void doNotCreateTicketIfAfterWorkingHoursTest() {
        clock.setFixed(
            LocalDateTime.of(
                    LocalDate.of(2021, 3, 22),
                    LocalTime.of(19, 1)
    ...
    @DisplayName("Создание тикета в Startrek для одного просроченного план-факта отгрузки ФФ-СД")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/intake/ff_ds/create_ticket_with_one_planfact.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/intake/ff_ds/create_ticket_with_one_planfact.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    void createSingleIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("summary")).isEqualTo(
            "[MQM] 07-11-2020: СД Тестовая служба доставки вовремя не приняла заказы со склада Тестовый склад.");
    ...
    @DisplayName("Создание тикета в Startrek для просроченных план-фактов отгрузки ФФ-СД")
    @Test
    @DatabaseSetup(
        "/service/processor/qualityrule/before/intake/ff_ds/create_ticket_with_some_planfacts.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/intake/ff_ds/create_ticket_with_some_planfacts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @DisplayName("Добавление комментариев в тикета Startrek для просроченных план-фактов отгрузки ФФ-СД")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/intake/ff_ds/comment_with_some_planfacts.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/intake/ff_ds/comment_with_some_planfacts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    ...
    @DisplayName("Закрытие тикета Startrek для просроченных план-фактов отгрузки ФФ-СД")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/intake/ff_ds/close_all_planfacts.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/intake/ff_ds/close_all_planfacts.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
    void closeAllAggregatedIssueTest() {
        handleGroups();
        ArgumentCaptor<IssueUpdate> captor = ArgumentCaptor.forClass(IssueUpdate.class);
        ArgumentCaptor<String> transitionCaptor = ArgumentCaptor.forClass(String.class);
        verify(transitions).execute(eq("MONITORINGSNDBX-1"), transitionCaptor.capture(), captor.capture());
        ...
        softly.assertThat(transitionCaptor.getValue()).isEqualTo("close");
    }

    @DisplayName("Переоткрытие тикета, если проблемы по нему актуальны")
    @Test
    @DatabaseSetup("/service/processor/qualityrule/before/intake/ff_ds/reopen_ticket.xml")
