...
@DisplayName("Тесты процессора по созданию тикетов для просрочек отгрузки с СД для обратной логистики")
public class DeliveryServiceShipmentReturnProcessorTest extends StartrekProcessorTest {
    @BeforeEach
    public void setUp() {
    ...
    @Test
    @DisplayName("Создание тикета для одного просроченного план-факта отгрузки с СД")
    @DatabaseSetup("/service/processor/qualityrule/before/ds_shipment_return/create_single.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/ds_shipment_return/create_single.xml",
    ...
    void createSingleIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("summary"))
            .isEqualTo("[MQM] 01-11-2020: СД Какая-то СД вовремя не отгрузила заказы обратно в Cофьино.");
        softly.assertThat(values.getOrThrow("description"))
            .isEqualTo(
                String.join(
                    ...
                    "Дедлайн отгрузки обратно с СД: 01-11-2020 15:00",
                    "Трек СД: 102",
                    "Трек ФФ/СЦ: 101"
                )
            );
        ...
        softly.assertThat(((String[]) values.getOrThrow("tags")))
            .containsExactly("Какая-то СД:123", "Cофьино:172");
    }

    @Test
    @DisplayName("Создание тикета для группы просроченных план-фактов отгрузки с СД")
    @DatabaseSetup("/service/processor/qualityrule/before/ds_shipment_return/create_aggregated.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/ds_shipment_return/create_aggregated.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
