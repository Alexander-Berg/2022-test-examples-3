...
@DisplayName("Тесты процессора по созданию тикетов для просрочек возвращения заказа")
public class ScReturnedReturnProcessorTest extends StartrekProcessorTest {
    ...
    @Test
    @DisplayName("Создание тикета для одного просроченного план-факта возвращения заказа")
    @DatabaseSetup("/service/processor/qualityrule/before/sc_returned_return/create_single.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/sc_returned_return/create_single.xml",
    ...
    void createSingleIssueTest() {
        ...
        softly.assertThat(values.getOrThrow("summary"))
            .isEqualTo("[MQM] 01-11-2020: Возвратный заказ не был вовремя возвращен отправителю (СЦ Какой-то СЦ).");
        softly.assertThat(values.getOrThrow("description"))
            .isEqualTo(
                String.join(
                    ...
                    "Дедлайн возвращения заказа: 01-11-2020 15:00",
                    "Трек СЦ: 101",
    ...
    @Test
    @DisplayName("Создание тикета для группы просроченных план-фактов возвращения заказа")
    @DatabaseSetup("/service/processor/qualityrule/before/sc_returned_return/create_aggregated.xml")
    @ExpectedDatabase(
        value = "/service/processor/qualityrule/after/sc_returned_return/create_aggregated.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT_UNORDERED
    )
