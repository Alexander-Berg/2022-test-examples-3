...
@DatabaseSetup("/data/controller/admin/logisticPointDayOff/prepare_day_offs.xml")
class AdminLogisticsPointDayOffControllerTest extends AbstractContextualAspectValidationTest {

    private static final String GRID_URL = "/admin/lms/logistics-point-day-offs";
    private static final String DETAIL_URL = GRID_URL + "/1";

    private static final Map<String, String> PARENT_COMPONENT_PROPERTIES = Map.of(
        "idFieldName", "id",
        "parentSlug", "logistics-point",
        "parentId", "1"
    );
    ...
    @WithBlackBoxUser(login = "lmsUser", uid = 1, authorities = {LMSPlugin.AUTHORITY_ROLE_LOGISTICS_POINT_DAY_OFFS})
    ...
    private static Stream<Arguments> getGridArguments() {
        return Stream.of(
            Arguments.of(
                "Без фильтрации",
                Map.of(),
                "data/controller/admin/logisticPointDayOff/search_no_filter.json"
            ),
            Arguments.of(
                "По дате",
                Map.of("day", "2020-01-02"),
                "data/controller/admin/logisticPointDayOff/search_by_day.json"
            ),
            Arguments.of(
                "С размером страницы",
                Map.of("size", "2"),
                "data/controller/admin/logisticPointDayOff/search_with_size.json"
            ),
            Arguments.of(
                "С номером страницы",
                Map.of("size", "2", "page", "1"),
                "data/controller/admin/logisticPointDayOff/search_with_page.json"
            ),
            Arguments.of(
                "С сортировкой",
                Map.of("sort", "day,asc"),
                "data/controller/admin/logisticPointDayOff/search_with_sort.json"
            )
        );
    }
    ...
    private static Stream<Arguments> getDetailArguments() {
        return Stream.of(
            ...
            Arguments.of(
                ...
                2,
                status().isOk(),
                TestUtil.jsonContent("data/controller/admin/logisticPointDayOff/detail__found.json")
            )
        );
    }
}
