...
@DisplayName("CRUD выходных дней логистических точек через админку")
@DatabaseSetup("/data/controller/admin/holiday/prepare_calendars.xml")
@ParametersAreNonnullByDefault
class AdminLogisticsPointHolidayControllerTest extends AbstractAdminHolidayControllerTest {
    private static final String METHOD_URL = "/admin/lms/logistics-point-holiday";
    ...
    protected String getReadOnlyDetailResponsePath() {
        return "data/controller/admin/holiday/get_logistics_point_detail_success.json";
    }

    @Nonnull
    @Override
    protected String getReadWriteDetailResponsePath() {
        return "data/controller/admin/holiday/get_logistics_point_detail_success_edit.json";
    }

    @Nonnull
    @Override
    protected String getNewResponsePath() {
        return "data/controller/admin/holiday/get_logistics_point_new.json";
    }

    @Test
    @WithBlackBoxUser(login = "lmsUser", uid = 1, authorities = LMSPlugin.AUTHORITY_ROLE_LOGISTICS_POINT_HOLIDAY_EDIT)
    @ExpectedDatabase(
        value = "/data/controller/admin/holiday/create_holiday_in_new_logistics_point_calendar.xml",
