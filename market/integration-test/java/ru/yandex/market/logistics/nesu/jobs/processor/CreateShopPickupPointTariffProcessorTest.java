...
import ru.yandex.market.logistics.nesu.jobs.consumer.CreateShopPickupPointTariffConsumer;
...
@DatabaseSetup("/jobs/processor/shop_pickup_point_tariff/create/before.xml")
class CreateShopPickupPointTariffProcessorTest extends AbstractContextualTest {

    @Autowired
    private CreateShopPickupPointTariffConsumer createShopPickupPointTariffConsumer;
    ...
    @Test
    @DisplayName("Успешное создание правила доставки ПВЗ магазина в Тариффикаторе")
    @ExpectedDatabase(
        value = "/jobs/processor/shop_pickup_point_tariff/create/after.xml",
        assertionMode = DatabaseAssertionMode.NON_STRICT
    )
    void addNew() {
        when(tarifficatorClient.createPickupPointDeliveryRule(safeRefEq(getCreateRequest())))
            .thenReturn(new PickupPointDeliveryRuleResponse().setId(1L));

        softly.assertThat(createShopPickupPointTariffConsumer.execute(getTask()).getActionType())
            .isEqualTo(FINISH);

        verify(tarifficatorClient).createPickupPointDeliveryRule(safeRefEq(getCreateRequest()));
    }

    @Nonnull
    private PickupPointDeliveryRuleCreateRequest getCreateRequest() {
        return (PickupPointDeliveryRuleCreateRequest) new PickupPointDeliveryRuleCreateRequest()
            .setShopId(2L)
            .setLmsLogisticsPointId(10L)
