...
@DisplayName("Активация Dropship-магазина")
public class DropshipActivationTest extends ShopActivationTest {
    @Test
    @DisplayName("Успех")
    @DatabaseSetup("/service/shop/activation/prepare_dropship.xml")
    void success() {
    ...
    @DatabaseSetup("/service/shop/activation/prepare_dropship_off.xml")
    ...
    @MethodSource
    @DisplayName("Неудачная активация дропшипа")
    ...
    private static Stream<Arguments> activationFail() {
        return Stream.of(
            Arguments.of("Без marketId", validDropship().setMarketId(null)),
            Arguments.of("Без balanceClientId", validDropship().setBalanceClientId(null)),
            Arguments.of("Без склада", validDropship().setId(2L))
        );
    }
