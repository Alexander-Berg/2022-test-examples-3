...
{
    Y_UNIT_TEST(NEW_REGION_RIGHT_FALLBACK)
    {
        World.SetPlanningPeriod(56);
        World.SetStartDate(TDate(2021, 03, 25));
        LoadFromYson("/market/replenishment/algorithms/vicugna/ut/test_new_region_right_fallback/test_data");
        TSimulatedAnnealingAlgorithm simulatedAnnealing(World, 10000, 100, 1);
        simulatedAnnealing.Optimize();

        ui64 countQty = 0;

        for (const auto& recommendation : World.GetRecomendations()) {
