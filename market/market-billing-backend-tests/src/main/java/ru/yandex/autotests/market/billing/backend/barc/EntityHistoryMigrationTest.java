...
import ru.yandex.autotests.market.billing.backend.core.dao.yql.YTEntityHistoryService;
 ...
 * @since 05.04.17
 */
@Aqua.Test(title = "Тест на архивацию entity_history в YT")
@Feature("barc")
@Issue("AUTOTESTMARKET-5928")
@CommonSpringTest(classes = {
        ...
        YTEntityHistoryService.class, MarketBillingConsoleFactory.class})
@ActiveProfiles(profiles = {"test", "migration"})
@ShopsWebDatasource
@ContextConfiguration({
        "classpath*:ru/yandex/autotests/market/billing/backend/barc/EntityHistoryMigrationTestSupport.xml",
...
public class EntityHistoryMigrationTest {
    @ClassRule
    ...
    private LocalDateTime boundaryDT;

    @Before
    public void setUp() {
        boundaryDT = boundaryDefinition.apply(LAUNCH_DATE);
    }

    @Test
    @Title("Джоба migrateEntityHistoryExecutor отработала без ошибок")
    ...
    public void testDataExists() throws IOException {
        LocalDateTime partitionDate = boundaryDT.minus(boundaryDefinition.getPartitionsInterval());

        long count = barcDaoSteps.getYTEntityHistoriesCount(
    ...
    public void testEquivalency() throws IOException {
        archivingSteps.compareBillingAndYTArchivedEntityHistory(
                boundaryDefinition.getPartitionKeyFormatter(),
                boundaryDT.minus(boundaryDefinition.getPartitionsInterval()),
                boundaryDT.minusNanos(1));
    }
    ...
    public void testDeletionPrepared() throws IOException {
        LocalDateTime archivedDate = boundaryDT.minus(boundaryDefinition.getPartitionsInterval());
