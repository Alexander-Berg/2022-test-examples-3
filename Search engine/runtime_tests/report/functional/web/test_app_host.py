...
class TestAppHost(BaseFuncTest):
    ...
    @pytest.mark.skipif(os.environ.get('REPORT_INVERTED') == '1', reason="SERP-67197")
    def test_app_host_schema(self, query, schema_path):
        #SERP-38481 схема INIT контекста
        resp = self.app_host_request(query)

        # TODO: https://st.yandex-team.ru/SERP-70519
        def fix_context(data):
            if data[0]['name'] == 'INIT' and data[1]['name'] == 'INIT':
                data = data[1:]
            if data[0]['name'] == 'INIT':
                init = data[0]
                results = init['results']
                if results[0]['type'] == 'flags' and results[1]['type'] == 'experiments':
                    init['results'] = results[2:]
            sources = [x['name'] for x in data]
            if 'SRC_ASK' not in sources:
                data.insert(2, {'name':  'SRC_ASK', 'results': []})
            return data

        for r in resp.sources['APP_HOST'].requests:
            data = fix_context(r.data)
            self.validate_json_data(data, schema_path)
            #TODO r.validate_schema(schema_path)
