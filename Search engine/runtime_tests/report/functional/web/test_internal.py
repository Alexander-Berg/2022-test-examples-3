...
class TestKukaInSources(BaseFuncTest):
    @pytest.mark.skipif(os.environ.get('AB_MODE') == '2', reason="")
    def test_kuka_inforequest_access(self, query, enable_kuka):
        query.set_url('/search/inforequest')

        query.set_external(True)
        self.request(query, require_status=403)

        query.set_external(False)
        self.request(query, require_status=403)
        ...
        query.set_external(False)
        self.request(query, require_status=200)

    @pytest.mark.skipif(os.environ.get('AB_MODE') == '2', reason="")
    def test_kuka_ajax_wrong_2(self, query, enable_kuka):
        enable_kuka(query)
        query.set_url('/search/inforequest')
        resp = self.request(query, source='INFO_REQUEST')
        assert resp.content.startswith('error:')
        assert not resp.source

    @pytest.mark.parametrize(('myreqid'), [None, '888-myreqid-'])
    @pytest.mark.skipif(os.environ.get('AB_MODE') == '2', reason="")
    def test_kuka_inforequest(self, query, enable_kuka, myreqid):
        enable_kuka(query)
        query.set_method('POST')
        query.set_url(
            '/search/inforequest'
            '?request_type=meta-response'
            '&doc_url=https%3A%2F%2Fwww.1tv.ru%2Flive&user_request=1%3A%3A221'
        )
        post = 'url=http://%s:%d' % (urllib.quote(self.curr_host), GenerateBackendPort('INFO_REQUEST'))
        if myreqid:
            post += '&myreqid=' + myreqid
        query.set_post_params(post)

        resp = self.request(query, source='INFO_REQUEST')
        assert not resp.content.startswith('error:')
        assert resp.source
        assert not myreqid or resp.reqid == myreqid

        eventlog = resp.eventlog()
        assert filter(
            lambda x: x[2] == 'RemoteRequestStart' and x[3] == 'INFO_REQUEST:' + resp.reqid, eventlog['parsed']
        )
...
class TestKukaInTemplates(BaseFuncTest):
    ...
    def test_dbgwzr_tmpl(self, query, dbgwzr, expect):
        ...
        resp = self.json_request(query, source='UPPER')
    ...
    def test_sbh(self, query):
        ...
        data = resp.data
        assert data['eventlog'] == ''
        assert data['url_event_name']
...
class TestInternal(BaseFuncTest):
    ...
    def test_rawdump(self, query):
        query.set_internal()
        query.add_params({'raw_dump': 'reqparam.user_request.0'})
        resp = self.request(query)
        assert resp.content == TEXT.encode('utf-8') + "\n"
    ...
    def test_app_host_timeout(self, query):
        ...
        params = self.json_dump_request(query, "search.app_host.sources.(_.name  eq  'APP_HOST_PARAMS')")
        assert params[0]["results"][0]["timeout"] == 130

    @pytest.mark.skipif(os.environ.get('AB_MODE') == '2', reason="")
    @pytest.mark.ticket('WEBREPORT-72')
    def test_graphrwr_param(self, query):
        value = 'test1:test2:80'
        query.add_params({'graphrwr': value})
        params = self.json_dump_request(query, "search.app_host.sources.(_.name eq 'APP_HOST_PARAMS')")
        assert value in params[0]["results"][0]["report_graphrwr"]
    ...
    def test_graphrwr_flag(self, query):
       # value = 'test1:test2:80'
        ...
        params = self.json_dump_request(query, "search.app_host.sources.(_.name eq 'APP_HOST_PARAMS')")
        assert 'test1' in params[0]["results"][0]["graphrwr"]
    ...
    def test_people_search_docs_isnt_empty(self, query):
        ...
        resp = self.json_request(query, SetXYandexInternalRequestHeader=1)
        data = resp.data
