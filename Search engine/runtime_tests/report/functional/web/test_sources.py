...
class TestSourcesParams(BaseFuncTest):
    ...
    def test_app_host_expboxes(self, query):
        ...
        query.set_params({ "json_dump" : "search.app_host.sources" })
        resp = self.json_request(query)
        expboxes_found = False
        for source in resp.data["search.app_host.sources"]:
            for ctx in source["results"]:
                if "type" in ctx and ctx["type"] == "expboxes":
                    expboxes_found = True
        assert expboxes_found
    ...
    def test_yandexuid_flag(self, query):
        test_uid='111223344556677'
        query.set_flags({ 'yandexuid': test_uid })
        resp = self.json_request(query)
    ...
    def test_report_type_snip_gateway(self, query):
        ...
        assert 'report=gateway' in report

    @pytest.mark.skipif(os.environ.get('AB_MODE') == '1', reason="")
    def test_report_type_global_gateway_wrong(self, query):
        """
        SERP-36530: Пробрасывать тип репорта для ноапача
        Проверяем, что в глобальном контексте есть тип репорта
        """
        ...
        assert 'report=gateway' in ';'.join(noapache_setup['global_ctx'].get('snip', []))

    @pytest.mark.skipif(os.environ.get('AB_MODE') == '1', reason="")
    def test_report_type_global_gateway(self, query):
        """
        SERP-36530: Пробрасывать тип репорта для ноапача
        Проверяем, что в глобальном контексте есть тип репорта
        """
