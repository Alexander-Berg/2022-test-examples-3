...
def static_data_dir(fs_manager, request, static_root_dir):
    old_static = os.path.abspath(os.path.join(static_root_dir, request.node.name))
    return fs_manager.copy(old_static, 'static_data')

@pytest.fixture(scope='class')
def class_static_data_dir(class_fs_manager, request, static_root_dir):
    old_static = os.path.abspath(os.path.join(static_root_dir, request.node.name))
