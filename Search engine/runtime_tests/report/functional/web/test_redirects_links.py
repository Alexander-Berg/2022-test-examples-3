...
class TestRedirects(BaseFuncTest):
    ...
    def test_familyhost(self, query, host):
        """
        SERP-30266
        Check that morda url for turkey family search
        """
        query.set_host(host)
        resp = self.json_request(query)
        assert resp.data['reqdata']['morda_url'] == '//' + host + '/'
    ...
    def test_set_uid_on_redir(self, query, tld):
        ...
        assert 'yandexuid' in cookie

    @pytest.mark.skipif(os.environ.get('AB_MODE') == '1', reason="")
    @pytest.mark.ticket('SERP-38524')
    @pytest.mark.parametrize(("ua", "my", "path", "code", "location"), [
        # sitesearch
        (USER_AGENT_DESKTOP, None,   SEARCH_SITE,       200, None),
        (USER_AGENT_PAD,     None,   SEARCH_SITE,       200, None),
        (USER_AGENT_TOUCH,   None,   SEARCH_SITE,       200, None),
        (USER_AGENT_SMART,   None,   SEARCH_SITE,       200, None),
        # sitesearch_old
        (USER_AGENT_DESKTOP, None,   SEARCH_SITE_OLD,   200, None),
        (USER_AGENT_PAD,     None,   SEARCH_SITE_OLD,   200, None),
        (USER_AGENT_TOUCH,   None,   SEARCH_SITE_OLD,   200, None),
        (USER_AGENT_SMART,   None,   SEARCH_SITE_OLD,   200, None),
        # searchapp:   SERP-52131
        (USER_AGENT_SEARCHAPP_ANDROID, None, SEARCHAPP, 200, None),
        (USER_AGENT_SEARCHAPP_IOS,     None, SEARCHAPP, 200, None),
        # /blogs
        (USER_AGENT_DESKTOP, None,   '/blogs/search',       200, None),
        (USER_AGENT_TOUCH,   None,   '/blogs/search/touch', 200, None),
        (USER_AGENT_PAD,     None,   '/blogs/search/pad',   200, None),
        (USER_AGENT_DESKTOP, None,   '/blogs',              302, '/blogs/search'),
#        (USER_AGENT_DESKTOP, None,   '/blogs/search/rss',   302, '/blogs/search'),
        (USER_AGENT_TOUCH,   None,   '/blogs',              302, '/blogs/search/touch'),
        (USER_AGENT_TOUCH,   None,   '/blogs/search',       302, '/blogs/search/touch')
    ])
    @pytest.mark.parametrize(("tld"), [
        RU,
        #UA,
    ...
    @pytest.mark.parametrize(("host_prefix"), [
        'yandsearch.yandex.',
        'yandex.',
    ])
    def test_path_redirects(self, query, ua, my, path, code, location, tld, host_prefix):
        host = host_prefix + tld
        query.set_flags({'serp3_granny_https': 0})
        query.set_host(host)
        query.set_user_agent(ua)
        query.set_url(path)
        query.replace_params({'lr': REGION_BY_TLD[tld], 'searchid': 2244093})
        if my:
            query.headers.cookie.set_my(COOKIEMY_PREFER_SERP[my])

        resp = self.request(query, require_status=302 if location else 200)

        """
        #no MDA on mobile, except RU
        if tld != RU and ua in MOBILE_UA and path in MOBILE_PATH:
            assert resp.status == 302
            assert re.search(r'^https?://[^/]*yandex.ru/', resp.headers['location'][0]), self.curl(query)
        """

        if location:
            if re.search(r'https?://', location):
                # test absolute redirect still within same domain
                location_wh = location % (host)
                assert resp.headers['location'][0].startswith(location_wh), self.curl(query)
            else:
                assert resp.headers['location'][0].startswith(location), self.curl(query)
