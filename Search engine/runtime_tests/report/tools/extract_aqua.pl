...
open my $fh, 'twr6265_6_ru.txt' or die $!;
...
while (<$fh>) {
    ...
    my ($name, $testpalm, $url) = map { s/^\s+|\s+$//g; s/\s+/ /g; $_ } split /\s*\|\s*/;
    ...
    my $marker = $json;
    if ($testpalm) {
        die $testpalm if $testpalm !~ m{^https://testpalm\.yandex-team\.ru/.};

        die "Duplicated testpalm: $testpalm\n$marker\n$tp_dup{$marker}" if $tp_dup{$testpalm} && $tp_dup{$testpalm} ne $marker;
        $tp_dup{$testpalm} = $marker;

        die "Duplicated case $marker but different testpalm:\n$testpalm\n$tp{$marker}" if $tp{$marker} && $tp{$marker} ne $testpalm;
        $tp{$marker} = $testpalm;
    }
    ...
    qx{$cmd};
    if ($testpalm) {
        qx{echo 21ffb831-6583-4234-9e3a-0362eea4ee00$testpalm >> '$json'};
    }
...
open my $testpalm, '>testpalm.py' or die $!;
print $testpalm "TestPalm = {\n";
for my $k (sort keys %tp) {
    print $testpalm "    '$k': '$tp{$k}',\n";
}
print $testpalm "}\n";
close $testpalm;
