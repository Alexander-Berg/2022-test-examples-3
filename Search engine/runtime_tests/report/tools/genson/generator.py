...
class Schema(object):
    ...
    def __init__(self, merge_arrays=True):
        ...
        * `merge_arrays` (default `True`): Assume all array items share
        ...
        self._type = set()
        self._required = None
        self._properties = defaultdict(lambda: Schema(merge_arrays=merge_arrays))
        self._items = []
        self._other = {}
    ...
    def _add_items_merge(self, items, func):
        if items:
            if not self._items:
                self._items.append(Schema(merge_arrays=self.merge_arrays))
    ...
    def _add_items_sep(self, items, func):
        for item in items:
            subschema = Schema(merge_arrays=self.merge_arrays)
