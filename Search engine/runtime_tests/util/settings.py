...
class Settings(object):
    def __init__(self, request, sandbox, yatest, xdist, root_fs):
...
class Configurator(object):
    __NUM_PROC_OPT = 'numprocesses'
    __SLAVE_ATTR = 'slaveinput'

    def __init__(self, config):
        super(Configurator, self).__init__()
        self._config = config
        self._xdist_master = hasattr(config.option, self.__NUM_PROC_OPT) and config.getoption(self.__NUM_PROC_OPT) is not None
        self._xdist_child = hasattr(config, self.__SLAVE_ATTR)
        self._xdist_run = self._xdist_master or self._xdist_child

    def configure(self):
        if YATEST:
            self.configure_yatest()
        elif not self._xdist_run:
            self.configure_no_xdist()
        elif self._xdist_master:
            self.configure_master()
        elif self._xdist_child:
            self.configure_child()

    def configure_no_xdist(self):
        self.configure_master()
        self.configure_child()
