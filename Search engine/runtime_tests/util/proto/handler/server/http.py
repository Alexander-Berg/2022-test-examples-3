...
from runtime_tests.util.proto.handler.server._common import ConfigServerHandler, HandlerConfigException, Config,\
    RequestInfo, StreamInfo
from runtime_tests.util.stream.io.byte import ByteReader, ByteWriter
from runtime_tests.util.proto.http.stream import HTTPServerStream, HTTPReaderException
    ...
    def handle(self):
        ...
        while not self.__close_connection:
            ...
            if self.sock.has_data():
                ...
                except HTTPReaderException, exc:
...
class StaticResponseHandler(PreparseHandler):
    ...
    def finish_response(self):
        ...
        """
        conn_header = self.config.response.headers.get_one('connection')
        if conn_header is None:
            if self.config.response.response_line.version == 'HTTP/1.0':
                self.force_close()
        else:
