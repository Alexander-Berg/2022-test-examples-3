...
describe('parseHTML', () => {
    ...
    test('startrek urls', () => {
        expect(parseHTML('text https://st.yandex-team.ru/QUEUE-1234')).toEqual(
            'text <a href="https://st.yandex-team.ru/QUEUE-1234" target="_blank">QUEUE-1234</a>',
        );
        expect(
            parseHTML('text https://st.yandex-team.ru/QUEUE-1234#comment1234'),
        ).toEqual(
    ...
    test('sanitize', () => {
        expect(
            parseHTML('<style>oh hi</style> text <script>good day</script>'),
        ).toEqual('<wbr />text');
        expect(
            parseHTML(
