...
IMPORT helpers SYMBOLS $NaT, $timetarg_no_89, $is_correct_date;
...
DEFINE ACTION $run_limits_test($table) AS
    ...
    -- daily_limit should be $inf OR NULL in case of wrong strategy
    DISCARD SELECT
        Ensure(NULL,
              COUNT(*) = 0,
              "Found daily_limit IS NOT NULL and not $inf with wrong strategy in " || CAST(COUNT(*) AS String) || " rows!")
    FROM $table()
    WHERE
    ...
    -- All limits except for daily_limit should be $inf OR NULL for these 3 strategies
    DISCARD SELECT
        Ensure(NULL,
              COUNT(*) = 0,
...
DEFINE ACTION $run_ppcdata_binlog_parsing_test($parser) AS
    $ppcdata_binlog_test_cases = FileContent('direct_ppcdata_binlog_tests.json');
    $type = ParseType(@@
    List<
        Struct<
            args:List<String?>,
    ...
    $tmp = Yson::ConvertTo(Yson::Parse($ppcdata_binlog_test_cases), $type);

    $result = (
        SELECT
            ListMap($tmp, ($dict) -> {
                $ret_val = Yson::From($dict.ret_val.`string`) ?? Yson::From($dict.ret_val.`double`) ??
                           Yson::From($dict.ret_val.`integer`) ?? Yson::From($dict.ret_val.`date`);
                $json = Yson::ParseJson(String::ReplaceAll(String::ReplaceAll(Unwrap($dict.args[0]), "'", '"'), '\"', '"'));
                $res = Yson::From($parser($json, Unwrap($dict.args[1]), 'string')) ??
                       Yson::From($parser($json, Unwrap($dict.args[1]), 'integer')) ??
                       Yson::From($parser($json, Unwrap($dict.args[1]), 'fixed_point')) ??
                       Yson::From($parser($json, Unwrap($dict.args[1]), 'date'));
                return Yson::Equals($res, $ret_val);
            }) AS test_result
    );
    SELECT
        Ensure($result,
              ListSum(CAST(test_result AS List<Int64>)) = ListLength($tmp),
              "One or more time targeting test has been failed!")
    FROM $result;
END DEFINE;

EXPORT $run_time_targeting_tests, $run_empty_row_test, $run_limits_test, $more_rows_than_test, $same_set_of_dates_test, $status_moderate_null_test, $run_ppcdata_binlog_parsing_test;
