...
from crypta.buchhalter.services.main.lib import audience_shadow_dmp_report_calc
...
def test_basic(mock_audience_server, local_yt, local_yt_and_yql_env, config_file, config, input_stats_tables, tvm_api):
    ...
    canonize_excel_data = test_helpers.CanonizeExcelData(audience_shadow_dmp_report_calc.ExcelReportRow)
    diff = tests.Diff()
    ...
    empty_reports = [
        ypath.ypath_join(config.DailyReportDir, "shadow-dmp-login-1/2020-02-01.xlsx"),
        ypath.ypath_join(config.DailyReportDir, "shadow-dmp-login-444/2020-02-01.xlsx"),
    ]
    ...
    output_files = tests.yt_test(
        ...
        args=[
            ...
            "--index", yatest.common.test_source_path("data/shadow_dmp_index.yaml"),
        ],
        ...
        output_tables=[
            ...
            (tables.YsonTable("segment_names.yson", config.ShadowDmpSegmentNamesTable, yson_format="pretty"), [diff]),
            (tables.YsonTable("shadow_dmp_index.yson", config.ShadowDmpIndexTable, yson_format="pretty"), [diff, tests.SchemaEquals(shadow_dmp_index_schema)]),
