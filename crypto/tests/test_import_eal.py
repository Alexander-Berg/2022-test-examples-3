...
from data_imports.import_logs.graph_eal import ImportEalDayTask
...
class TestParseYuidWithEal(object):
    def __prepare_cypress(self, date):
        ...
        mr.mkdir(config.STATBOX_EAL_FOLDER)

    def __load_data(self, date):
        eal_log = config.STATBOX_EAL_FOLDER + date
        yt.create_table(eal_log)
        data = [{'request':
                 "/status.xml?yasoft=punto&clid=41281&ui={8A364D94-B00B-4887-B5D4-3CFDF557C463}&ver=4.2.5.1238&os=winnt&stat=dayuse&launchesAsAdmin=0&osver=win7x64&usesAutocorrection=0&usesDiary=0&yb_installed=0&yu=2136375691466196609&yu_opch=2136375691466196609&fd=24.05.2016",
                 'cookies': "uid=CmuLAldmz+SUHc8ECBLUAg=="}, # punto
    ...
    def __assert_task_result(self, date):
        ...
        for pair in [('D705B5D4-BCE3-BBC5-E058-D57EE2A32E70', '1377513741465469105'),
        ...
        for pair in [('8A364D94-B00B-4887-B5D4-3CFDF557C463', '2136375691466196609'),
        ...
        ext_bro_result_table = config.YT_OUTPUT_FOLDER + date + '/yuid_raw/yuid_with_' + config.ID_SOURCE_TYPE_EXTERNAL_BROWSERS
        ...
        for pair in [('E4388A5E-0AC2-41A9-9989-ECA9C8C24044', '91393041451775493'),
    ...
    def test_import_eal(self):
        ...
        luigi.run(['ImportEalDayTask', '--date', date, '--run-date', date, '--workers', '1', '--local-scheduler', '--no-lock'])
