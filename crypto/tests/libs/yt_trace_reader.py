...
yt_trace_record_pattern = (
    "(.*?) (\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3}) (\w{32})\|\|(\S{3,5})\|\|(.*?)\|\|(.*?)\|\|(.*?)$"
)
record_uid_pattern = re.compile(" (\w{32})\|\|")
record_datetime_pattern = re.compile(" (\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3}) ")
yt_trace_record_not_sync_pattern = "(.*?) (\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3}) (.*?)\|\|(.*?)\|\|(.*?)$"
...
class BaseReader(object):
    ...
    def get_all_log_paths(self, end_datetime, log_prefix):
        yesterday = self.start_datetime - timedelta(days=1)
        log_count = (end_datetime - yesterday).days
        paths = []
        for log_num in range(log_count):
            day = (yesterday + timedelta(days=1)).date()
            paths.append(os.path.join(self.log_folder, log_prefix + str(day) + ".syslog"))
        return paths
