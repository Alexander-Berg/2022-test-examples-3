...
public class TestUpdateBlanks {
    ...
    public void testUpdateBlanks() {
        stubFor(post(anyUrl()).willReturn(aResponse()
                .withBody(TestResources.readResource("UpdateBlanksResponse.json"))));

        var response = client.updateBlanks(1234567);

        verify(postRequestedFor(urlPathEqualTo("/Railway/V1/Reservation/UpdateBlanks")).withRequestBody(equalToJson(
                TestResources.readResource("UpdateBlanksRequest.json")
        )));

        assertThat(response.isModified()).isEqualTo(true);
