 ...
 * Time: 12:53
 */
class RandomWithPrioritySupplierTest extends FlatSpec with Matchers {
  ...
  "RandomWithPrioritySupplier" should "correct select variant by priority" in {
    val s = new RandomWithPrioritySupplier(TreeMap(0 -> requestA, 4 -> requestB), 10, 0)
