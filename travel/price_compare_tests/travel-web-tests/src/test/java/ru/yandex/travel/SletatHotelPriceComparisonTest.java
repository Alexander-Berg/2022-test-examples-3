...
public class SletatHotelPriceComparisonTest {
    ...
    @Parameterized.Parameters(name = "HotelName -> {3}")
    public static Collection<Object[]> getData() throws Exception {
        ...
        TestUtils.readSearches("pricechecking/tours.hotelpage.random.1000.searches.tsv")
                .stream().filter(SletatHotelPriceComparisonTest::isValid).forEach(map -> {
                    SearchParameters sletatParameters = TestUtils.convertSearchParameters(map, true);
                    String url = map.get("searchUrl");
                    collection.add(new Object[]{map, url, sletatParameters, map.get("yaHotelName")});
                });
        Collections.shuffle(collection);
        return collection.stream().limit(config.countForHotels()).collect(toList());
    }


    @Test
    public void priceTest() throws Exception {
        log.info(format(" START hotel price test «%s» (%s)", name, travelSearchUrl));

        TourInformation travelTour = travelSteps.findCheapestHotelTourInformation(travelSearchUrl)
        ...
        sletatSteps.search(sletatSearchParameters, true);
        ...
        log.info(format("hotel price test done (%s) ", travelSearchUrl));
    }


    @AfterClass
    public static void generateReport() throws IOException, TemplateException {
        TestUtils.generateReport(result, "Результаты сравнения цен (отели)");
    }

    private static boolean isValid(Map map) {
        return !map.get("sletatHotelName").equals("not_found") && !map.get("to_country").equals(map.get("to"));
    }
