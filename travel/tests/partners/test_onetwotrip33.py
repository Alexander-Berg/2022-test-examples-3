...
from travel.avia.ticket_daemon.ticket_daemon.partners import onetwotrip33


@mock.patch('requests.get', return_value=get_mocked_response('onetwotrip33.xml'))
@pytest.mark.dbuser
def test_onetwotrip_query(mocked_request):
    expected = expected_variants('onetwotrip33_expected.json')
    ...
    variants = next(onetwotrip33.query(test_query))
