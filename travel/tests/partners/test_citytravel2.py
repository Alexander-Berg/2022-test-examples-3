...
from travel.avia.ticket_daemon.ticket_daemon.partners import citytravel2


@mock.patch('requests.get', return_value=get_mocked_response('citytravel2.xml'))
def test_citytravel_query(mocked_request):
    expected = expected_variants('citytravel2.json')
    test_query = get_query()
    variants = next(citytravel2.query(test_query))
    assert_variants_equal(expected, variants)


@mock.patch('requests.get', return_value=get_mocked_response('citytravel2_fare_codes.xml'))
def test_citytravel_query_fare_codes(mocked_request):
    expected = expected_variants('citytravel2_fare_codes_result.json')
    test_query = get_query()
    variants = next(citytravel2.query(test_query))
    assert_variants_equal(expected, variants)
...
def test_get_baggage(flight_tag, expected_baggage):
    ...
    baggage = citytravel2.get_baggage(f_tag)
