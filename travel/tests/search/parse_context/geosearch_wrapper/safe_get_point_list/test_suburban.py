...
from geosearch.views.point import (
    StopWordError, NoPointsError, InvalidPointKey, TooShortError, SuburbanPointSearch, InvalidSlug
)
...
class TestSafeGetPointsBySuburban(TestCase):
    def setUp(self):
        super(TestSafeGetPointsBySuburban, self).setUp()
        self.point_key = 'key'
    ...
    def test_no_errors(self):
        ...
        with mock.patch.object(SuburbanPointSearch, 'find_point', return_value=expected_point_list) as m_find_point:
        ...
        m_find_point.assert_called_once_with(
            self.point_title, t_type=self.t_type.code, point_key=self.point_key, slug=self.point_slug,
            can_return_hidden=True
        )
    ...
    def _test_error(self, error, expected_error_type):
        with mock.patch.object(SuburbanPointSearch, 'find_point', side_effect=error) as m_find_point:
        ...
        m_find_point.assert_called_once_with(
            self.point_title, t_type=self.t_type.code, point_key=self.point_key, slug=None, can_return_hidden=True
        )
