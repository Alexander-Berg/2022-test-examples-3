...
import {roundDuration} from 'projects/trains/lib/date/duration/roundDuration';
...
describe('roundDuration', () => {
    ...
    test('При 1d 15h 30m 40s с maxUnits = 2 по умолчанию -> 1d 16h', () => {
        const duration = moment.duration({
            days: 1,
            hours: 15,
            minutes: 30,
    ...
    test('При 1d 15h 30m 40s и при переданном opts.maxUnits = 3 -> 1d 15h 31m', () => {
    ...
    test('При 15h 30m 40s и при переданном opts.maxUnits = 3 -> 15h 31m', () => {
        const opts = {maxUnits: 3};
        const duration = moment.duration({
            hours: 15,
            minutes: 30,
    ...
    test('При 30m 40s и при переданном opts.maxUnits = 3 -> 31m', () => {
        const opts = {maxUnits: 3};
        const duration = moment.duration({
            minutes: 30,
            seconds: 40,
