...
tmp_dir=`mktemp -d -t "tap-components.XXXXXXXX"`
...
git clone git@github.com:yandex/tap-components.git $repo_dir
...
rm -rf "$repo_dir/storybook" # В лего есть ссылки на внутренние ресурсы, эти ссылки попадают в Storybook
...
package_file="yandex-tap-components-${version}.tgz"
