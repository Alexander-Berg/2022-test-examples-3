...
describe('express/middlewares/ask-abstract', () => {
    const AskAbstract = require('./ask-abstract');

    /* В AskAbstract геттер name не реализован, поэтому для тестов доопределяем его */
    class AskAbstractTest extends AskAbstract {
        get name() {
            return 'AskAbstractTest';
        }
    }
    ...
    it('Should send required default params to backend', done => {
        fakeBack(
            ...
            (port, hostname, close) => {
                ...
                app.use(AskAbstractTest.create({
                    port,
                    hostname,
                    pathname: '/',
                }));
    ...
    it('Should send json to backend', done => {
        fakeBack(
            ...
            (port, hostname, close) => {
                ...
                class TestAskAbc extends AskAbstractTest {
                    createParams(params) {
    ...
    it('Should not accept != 200 responses', done => {
        fakeBack(
            ...
            (port, hostname, close) => {
                ...
                class TestAskAbc extends AskAbstract {
                    main() {
    ...
    it('Should expose data to res.locals.<name>', done => {
        fakeBack(
            back => {
                back.use((req, res) => {
            ...
            (port, hostname, close) => {
                ...
                app.use((req, res) => {
                    assert.deepEqual(res.locals.path, { content: { foo: 'bar' } });
                    res.sendStatus(555);
                });
    ...
    it('Should add data to res.locals.<name>', done => {
        fakeBack(
            back => {
                back.use((req, res) => {
                    res.status(201).json({ d: 43 });
                });
            },
            (port, hostname, close) => {
                const app = express();
