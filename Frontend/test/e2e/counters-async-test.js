...
// При правке этих тестов нужно также править версию для sync load
describeRum(httpPort => {
    describe('navigate - async load', () => {
        before(async function() {
            const reqId = Math.random();

            const page = this.page = await this.browser.newPage();

            await page.goto(`http://localhost:${httpPort}/?reqid=${reqId}&async_load=1`, {
    ...
    describe('AJAX - async load', () => {
        before(async function() {
            const reqId = Math.random();

            const page = this.page = await this.browser.newPage();

            await page.goto(`http://localhost:${httpPort}/?reqid=${reqId}&async_load`, {
                waitUntil: ['load', 'networkidle0']
            });
    ...
    describe('resource timing - async load', () => {
