specs('Feature one', async function () {
    describe('Describe in specs', async function () {
        ...
        it('The first it case', async function () {
            await this.browser.yaOpenSerp({ foreverdata: '1234', text: 'something', exp_flags: 'font-size=22' }, PO.list());
        });
        // 3b2105f
        it('The second it case', async function () {
            await this.browser.yaOpenSerp({ foreverdata: '1234', text: 'something2', exp_flags: ['hideads=1', 'font-size=22'] }, PO.list());
        });
        // c8cedc7
