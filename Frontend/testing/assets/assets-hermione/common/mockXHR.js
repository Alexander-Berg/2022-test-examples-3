...
window.hermione.injectXhrMock({
    recordData: [
        // Мок должен сработать до того как клиентский js отправит счетчик. Поэтому мокаем Метрику как можно раньше
        // (синхронно, не на load window), т.к. счетчики показа отправляются автоматически при загрузки страницы.
        'https:\/\/mc\.yandex\.ru\/watch\/\\d+.*?vacuum'
    ]
});

window.hermione.injectXhrMock({
    urlRedirectMap: {
        '^https?://favicon.yandex.net/favicon': '/favicon-generator'
    }
});
