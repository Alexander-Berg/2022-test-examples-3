...
import { LcGridBlockImage } from '../LcGridBlockImage';
import { ILcGridBlockImageProps } from '../LcGridBlockImage.types';

describe('<LcGridBlockImage/> component', () => {
    ...
    test('should pass events to link', () => {
        const props: ILcGridBlockImageProps = {
            paddings: {
        ...
        wrapper = mount(<LcGridBlockImage {...props} />);

        expect(wrapper.find('.lc-grid-block-image').childAt(0).prop('events')).toEqual([{
            type: LcEventType.OnClick,
            target: { type: LcSectionType.LcAnalytics },
            data: {},
        }]);
