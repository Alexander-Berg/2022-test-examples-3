...
module.exports = async function yaWaitForVisibleAndAssertErrorMessage(selector, time = 10_000, errorMessage = '', reverse = false) {
    if (reverse) {
        ...
        await assert(false, errorMessage ? errorMessage : 'yaWaitForVisibleAndClick: элемент виден, хотя не должен быть');
    } else {
        try {
            await this.waitForVisible(selector, time);
        } catch (error) {
            await assert(false, errorMessage ? errorMessage : 'yaWaitForVisibleAndClick: элемент не отобразился');
        }
    }
};
