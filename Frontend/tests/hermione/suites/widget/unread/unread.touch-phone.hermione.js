...
specs({
    ...
    ['widget', 'widget_ya'].forEach((item) => {
        it(`Проверка непрочитанных в сборке ${item}`, async function () {
            ...
            // Нет непрочитанных
            await this.browser.assertView('no-unread', '.hermione');

            // Непрочитанные стиль "Точка"
            this.browser.execute(...widgetParamsUpdaterFactory({ badgeCount: 1, theme: 'light', badgeType: 'dot', isMobile: true }));
            await this.browser.waitForVisible('.ya-chat-widget');
            await this.browser.assertView(`unread-light-dot-${item}`, '.hermione');

            this.browser.execute(...widgetParamsUpdaterFactory({ badgeCount: 1, theme: 'dark', badgeType: 'dot', isMobile: true }));
            await this.browser.waitForVisible('.ya-chat-widget');
            await this.browser.assertView(`unread-dark-dot-${item}`, '.hermione');

            this.browser.execute(...widgetParamsUpdaterFactory({ badgeCount: 1, theme: 'legacy', badgeType: 'dot', isMobile: true }));
            ...
            this.browser.execute(...widgetParamsUpdaterFactory({ badgeCount: 1, theme: 'light', badgeType: 'num', isMobile: true }));
            await this.browser.waitForVisible('.ya-chat-widget');
            await this.browser.assertView(`unread-light-num-ltmax-${item}`, '.hermione');

            this.browser.execute(...widgetParamsUpdaterFactory({ badgeCount: 1, theme: 'dark', badgeType: 'num', isMobile: true }));
            await this.browser.waitForVisible('.ya-chat-widget');
            await this.browser.assertView(`unread-dark-num-ltmax-${item}`, '.hermione');

            this.browser.execute(...widgetParamsUpdaterFactory({ badgeCount: 1, theme: 'legacy', badgeType: 'num', isMobile: true }));
            await this.browser.waitForVisible('.ya-messenger');
            ...
            this.browser.execute(...widgetParamsUpdaterFactory({ badgeCount: 100, theme: 'light', badgeType: 'num', isMobile: true }));
            await this.browser.waitForVisible('.ya-chat-widget');
            await this.browser.assertView(`unread-light-num-gtmax-${item}`, '.hermione');

            this.browser.execute(...widgetParamsUpdaterFactory({ badgeCount: 100, theme: 'dark', badgeType: 'num', isMobile: true }));
            await this.browser.waitForVisible('.ya-chat-widget');
            await this.browser.assertView(`unread-dark-num-gtmax-${item}`, '.hermione');

            this.browser.execute(...widgetParamsUpdaterFactory({ badgeCount: 100, theme: 'legacy', badgeType: 'num', isMobile: true }));
            await this.browser.waitForVisible('.ya-messenger');
