...
describe('tap-taxi-435: Центр затрат. Отображение нового центра затрат', function() {
    it('Полный флоу', async function() {
        const bro = this.browser;
        await bro.auth('cost-center-2');
        await indexPage.openWithoutComment(bro);

        await bro.pause(5000);

        await bro.waitForVisible(payment.paymentButton, 30000);
        await bro.click(payment.paymentButton);

        await bro.waitForVisible(payment.modal, 10000);
        ...
        await bro.click(indexPage.tariffRequirementsButton);

        await bro.swipeUp(indexPage.tariff.requirementsModalScroller, 150);
        await bro.click(costCenter.costCenterButton);
        ...
        await bro.waitForEnabled(costCenter.costCenterModalInputDoneButton, 10000);
        ...
        await bro.scroll(costCenter.costCenterDoneButton);
        await bro.click(costCenter.costCenterDoneButton);

        const actualButtonSubtext = await bro.getText(costCenter.costCenterButtonSubtext);
