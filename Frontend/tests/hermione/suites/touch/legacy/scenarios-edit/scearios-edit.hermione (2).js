const { Socket, Light } = require('../../../../devices');
const { Api } = require('../../../../helpers/api');
const { TimetableTrigger, ActionsStep, Scenario } = require('../../../../scenarios');

describe('Страница редактирования сценария', () => {
    it('Пустая страница отображается корректно', async function() { // 29c578c
        const { browser, PO } = this;

        await browser
            .authOnRecord('user')
            .yaOpenPage('iot/scenario/add/summary')
            .waitForVisible(PO.ScenarioEditPage(), 10_000)
            .assertView('plain', 'body');
    });

    it('Открывается шторка с выбором иконки сценария', async function() { // 8a08539
        const { browser, PO } = this;

        await browser
            .authOnRecord('user')
            .yaOpenPage('iot/scenario/add/name')
            .waitForVisible(PO.ScenarioEditPage(), 10_000)
            .click(PO.ScenarioIcon())
            .pause(1_000)
            .assertView('plain', 'body');
    });

    it('Ограничение на количество действий в сценарии', async function() { // ebe7f84
        const { browser, PO } = this;
        ...
        async function addTriggerVoice(name) {
            ...
            await browser
                .waitForVisible(PO.TextInputReal(), 10_000)
                .yaSetValue(PO.TextInputReal(), name)
                .click(PO.ScenarioEditPageSaveButton());
        }

        // Добавить триггер по времени
        async function addTriggerTime() {
            await browser.yaClickToBottomSheetItem('Время');
            await browser.yaWaitForVisible(PO.ScenarioTimeTriggerIntermediary(), 10_000);
            await browser.click(PO.ListItem());
        ...
        await browser.yaAddDevices([new Socket()]);
        ...
        // Скриншот 4 фраз
        await browser
            .waitForVisible(PO.ScenarioSummaryTriggerList(), 10_000)
            .assertView('4-voice', PO.ScenarioSummaryTriggerList());
    ...
    it('Длинное название устройства', async function() { //4ce19f2
        ...
        await browser.waitForVisible('.iot-scenario-edit-action-selector', 10_000);
        await browser.assertView('list', 'body');

        // - Создание сценария (экран подтверждения устройства)
        await browser.yaClickToListItemByText(device.name);
        await browser.assertView('device', 'body');

        // - Создание сценария (главный экран с триггерами и экшенами)
        await browser.yaClickToSelectorByText('button', 'Далее');
        await browser.waitForVisible(PO.ScenarioSummaryActionList(), 10_000);
        await browser.assertView('summary', 'body');
    ...
    it('Тоггл "повторять ежедневно" верно аффектит дни недели', async function() { // a8ead1a
        ...
        await browser.authOnRecord('user');
        await browser.yaOpenPage('iot/scenario/add/trigger/add/scenario.trigger.timetable');
        await browser.waitForVisible(targetView(), 10_000);

        // Нажать на тоггл - включить все дни недели
        await browser.click(targetToggle());
        await browser.assertView('toggle-all-on', targetView(), censorSkillsOptions);

        // Нажать на день недели (воскресенье) - отключить один день
        await browser.click(targetDay());
        await browser.assertView('click-on-a-day', targetView(), censorSkillsOptions);

        // Нажать на тоггл - снова включить все дни недели
        await browser.click(targetToggle());
        await browser.assertView('toggle-all-on-again', targetView(), censorSkillsOptions);

        // Нажать на тоггл - выключить все дни недели
        await browser.click(targetToggle());
        await browser.assertView('toggle-all-off', targetView(), censorSkillsOptions);
    });
    ...
    describe('Отображение дней недели у триггера', () => {
        ...
        it('Выбраны все дни кроме вторника - отображает эти дни двумя буквами', async function() { // 6db9ede
            ...
            await browser.yaOpenPage(`iot/scenario/${scenario_id}`);
            await browser.waitForVisible(targetView, 10_000);
    ...
    describe('После добавления действия в сценарий страница отскролливается вниз', () => {
        ...
        describe('Во время редактирования сценария', () => {
            ...
            it('Без редактирования девайса', async function() { // be73196
                ...
                await browser.yaOpenPage(`iot/scenario/${scenario_id}/summary`);
        ...
        describe('Во время создания нового сценария', () => {
            ...
            it('Без редактирования девайса', async function() { // 9f10010
                ...
                await browser.yaOpenPage('iot/scenario/add/summary');
                await browser.waitForVisible(scenarioEditPage, 10_000);
    ...
    describe('Редактирование триггера сценария', () => {
        ...
        it('При изменении времени срабатывания - отображаемое время срабатывания меняется', async function() {
            ...
            await browser.assertView('initialTrigger', targetView);
            ...
            await browser.waitForVisible(targetView, 10_000);
            await browser.assertView('updatedTrigger', targetView);
        });
    ...
    describe('Кнопка сохранения сценария', () => {
        ...
        it('Кнопка задизейблина при отсутсвии триггеров', async function() {
            ...
            await browser.assertView('inactiveButton', targetButton);
            ...
            await browser.waitForVisible(targetPage, 10_000);
            await browser.assertView('activeButton', targetButton);
