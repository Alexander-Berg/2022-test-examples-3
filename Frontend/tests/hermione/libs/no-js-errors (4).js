...
module.exports = class NoJSErrorsExpectation extends Expectation {
    ...
    validateInBrowserContext() {
        return function() {
            const hermioneErrors = window['hermione-jserrors'];
            const isValid = Array.isArray(hermioneErrors) ? hermioneErrors.length === 0 : true;
            ...
            window['hermione-jserrors'] = hermioneErrors.map(function(error) {
                if (error.message === 'Script error.') {
                    ...
                    const scripts = Array.prototype.slice.call(document.querySelectorAll('script')).filter(function(item) {
                        return item.src;
                    });
                    const withOriginScripts = scripts
                        ...
                        .join('\n');
                    const withoutOriginScripts = scripts
                        .filter(function(item) {
                            return !item.crossOrigin;
                        })
                        .map(function(item) {
