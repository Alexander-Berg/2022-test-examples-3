...
@OperClass(
        apicommand = "get",
        apipath = "/",
        description = "Возвращает указанные настройки из обеих таблиц (user_parameters, mail_profile)"
)
public class Get extends Oper<Get> {

    public static Get get(Obj params) {
        return api(Get.class).params(params).setHost(props().settingsUri().toString());
    }
}
