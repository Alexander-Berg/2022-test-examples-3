package ru.yandex.autotests.smtpgate.tests;
...
@Feature("SEND")
@Aqua.Test
@Title("SendDifferentLettesBodiesTest. Поверка передачи содержимого письма через SEND")
@Description("Проверяем, что тело письма передается правильно")
@RunWith(Parameterized.class)
public class SendDifferentLettesBodiesTest {
    private static final User RECEIVER = new User("smmtpgate-test3@ya.ru", "testqa12345678");
    ...
    public void executeAppend() throws IOException, MessagingException {
        ...
        response = SMTP_SIMPLE_REQUEST.letter(eml).to(RECEIVER.getLogin()).from("Mond'Pond@sfkdsjfs.sdfds").send();
    }


    @Test
    public void shouldSeeLetterBodyDelivery() throws Exception {
        assertThat("Неверный ответ Send-запроса!",
                response, allOf(hasStatusCode(OK_200)
                        // , hasResponseBody(containsPattern(SmtpgateData.RESP_PATTERN))
                ));

        String msgSubject = msg.getSubject().replace("Re: ", "");
        String expectedEmlText = eml.substring(eml.indexOf("\n\n") + 2).replace("\n", "\r\n");

        assertThat("В результате Send-запроса тело письма доставилось некорректно!",
