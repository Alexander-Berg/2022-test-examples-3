...
import static ru.yandex.autotests.smtpgate.tests.utils.SmtpgateData.SAVE_RESP_PATTERN;
import static ru.yandex.autotests.smtpgate.tests.utils.SmtpgateData.SAVE_VIRUS_RESP_PATTERN;
...
@Feature("SAVE")
@Aqua.Test
@Title("SaveDetectVirusTest. Поверка работы ручки save c detect_virus=1")
@Description("Проверяем на вирусных,спамовых и чистых письмах работу save c detect_virus=1")
@RunWith(Parameterized.class)
public class SaveDetectVirusTest {
    private static final User RECEIVER =
            chooseUser(new User("smmtpgate-test@ya.ru", "testqa12345678"),
    ...
    public static Collection<Object[]> data() throws Exception {
        ...
        data.add(new Object[]{"Content-Type: text/plain; charset=\"us-ascii\"\nMIME-Version: 1.0\n" +
                ...
                + VIRUS_TEXT,
                containsPattern(SAVE_VIRUS_RESP_PATTERN), NOT_ACCEPTABLE_406});
        data.add(new Object[]{"\ngood", containsPattern(SAVE_RESP_PATTERN), OK_200});
    ...
    @Test
    public void shouldSeeVirusDetection() throws Exception {
        assertThat("[MPROTO-4256] Неверный ответ save-запроса на сохранение письма с текстом: " + emlText,
                response, allOf(
