...
from .step_types_defs import Shard
...
class TransferAll(FakeRootComponent):
    NAME = 'transfer_all'
    DEPS = [Mdb, ShardDb, HuskyDb, Sharpei, FakeBlackbox, Mulcagate, York, Doberman]
...
def fill_coordinator_context(context):
    start_component = TransferAll
    port_manager = network.PortManager()
    context.coordinator = Coordinator(TestEnv(port_manager, top_comp_cls=start_component), start_component)
