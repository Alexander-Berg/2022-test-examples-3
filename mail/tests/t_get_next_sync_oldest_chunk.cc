...
static const num_t FOLDER_TOP_SIZE = 5;
static const num_t CACHE_SIZE = 20;

struct get_next_sync_oldest_chunk_test : yplatform::log::contains_logger
{
    get_next_sync_oldest_chunk_test()
    {
    ...
    auto run_operation(Args&&... args)
    {
        auto env = make_env(
            ...
            stat,
            handler,
            test_struct_wrapper<get_next_sync_oldest_chunk_test>(this));
    ...
    void operator()(error ec, mb::message_vector_ptr chunk)
    {
        this->ec = ec;
        this->chunk = chunk;
    }
    ...
    error ec;
    mb::message_vector_ptr chunk;
};

TEST_CASE_METHOD(get_next_sync_oldest_chunk_test, "first chunk should contain all messages")
{
    path_t folder_path = { "INBOX", '|' };
    auto folder = mailbox_cache->get_folder(folder_path);

    run_operation<xeno::get_next_sync_oldest_chunk_op>(folder);
    REQUIRE(!ec);
    REQUIRE(chunk->size() == 6);

    imap_id_vector chunk_ids;
    std::transform(
        chunk->begin(), chunk->end(), std::back_inserter(chunk_ids), [](const message& msg) {
            return msg.id;
        });

    REQUIRE(chunk_ids == (imap_id_vector{ 13, 14, 110, 111, 112, 115 }));
}

TEST_CASE_METHOD(get_next_sync_oldest_chunk_test, "second chunk should be empty")
{
    path_t folder_path = { "INBOX", '|' };
    auto folder = mailbox_cache->get_folder(folder_path);
...
TEST_CASE_METHOD(
    get_next_sync_oldest_chunk_test,
    "first chunk should contain messages after downloaded range(including bottom)")
{
    path_t folder_path = { "INBOX", '|' };
    mailbox_cache->update_downloaded_range(folder_path, { 115, 111 });
    auto folder = mailbox_cache->get_folder(folder_path);
    ...
    REQUIRE(chunk_ids == (imap_id_vector{ 13, 14, 110, 111 }));
}

TEST_CASE_METHOD(
    get_next_sync_oldest_chunk_test,
    "second chunk after downloaded range should be empty")
{
    path_t folder_path = { "INBOX", '|' };
    mailbox_cache->update_downloaded_range(folder_path, { 115, 111 });
    auto folder = mailbox_cache->get_folder(folder_path);
