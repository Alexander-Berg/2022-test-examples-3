...
from sendr_interactions.clients.spark.entities import (
    AddressData, LeaderData, MerchantType, OkvedItem, OrganizationData, PhoneData, SparkAddressData, SparkData,
...
def response_mock(mocker, response_headers, response_status, response_json, response_data):
    ...
    mock.text = dummy_async_function(json.dumps(response_json))
    mock.read = dummy_async_function(response_data)
    return mock


@pytest.fixture
async def client_mocker(loop, mocker, dummy_logger, response_mock, pushers_mock):
    def _inner(cls, **kwargs):
        ...
        client = cls(logger=dummy_logger, request_id='test', pushers=pushers_mock, **kwargs)
