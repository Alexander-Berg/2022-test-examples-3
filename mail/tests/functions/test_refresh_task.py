    ...
    with context.reflect_db(user='barbet') as db:
        ...
        assert_that(db.queue.tasks.select(task_id=task_id), contains(
            has_properties(
                task_id=task_id,
                state='in_progress',
                processing_date=greater_than_or_equal_to(current_date))
        ), "should contain this task with refreshed 'processing_date'")
