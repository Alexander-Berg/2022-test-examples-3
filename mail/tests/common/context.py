...
def step_setup(request, context):
    ...
    clear_auditlogdb(context)


def clear_auditlogdb(context):
    purge_tables = [
    ]
    for table in purge_tables:
        context.auditlogdb.execute('DELETE FROM %s' % table)


def start_db(context):
    start_component = AuditlogDb
    ...
    context.auditlogdb = context.coordinator.components[AuditlogDb]
