...
class TestGetUnitSubscription(BaseTestNotFound):  # noqa
    @pytest.fixture
    def action(self, params, mocker):
        return GetUnitSubscriptionAction
    ...
    @pytest.mark.asyncio
    async def test_returned(self, unit_subscriptions, returned):
        assert_that(
            returned,
            contains_inanyorder(*[
                has_properties({
                ...
                for subscription in unit_subscriptions
            ])
        )
