...
#define INTERNAL_CATCH_TESTCASE2(TestName, ...)                                                    \
    ...
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(                                                     \
        autoRegistrar)(&TestName, CATCH_INTERNAL_LINEINFO, Catch::NameAndDesc(__VA_ARGS__));       \
    }                                                                                              \
    static void TestName()
...
#define INTERNAL_CATCH_TESTCASE2(TestName, Name, Desc)                                             \
    ...
    namespace {                                                                                    \
    Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(                                                     \
        autoRegistrar)(&TestName, CATCH_INTERNAL_LINEINFO, Catch::NameAndDesc(Name, Desc));        \
    }                                                                                              \
    static void TestName()
...
#define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName, TestName, Desc)                                 \
    INTERNAL_CATCH_TEST_CASE_METHOD2(                                                              \
        INTERNAL_CATCH_UNIQUE_NAME(____C_A_T_C_H____T_E_S_T____), ClassName, TestName, Desc)
...
namespace Catch { namespace Internal {
...
private:
    void visitChar(char c)
    {
        ...
        if (m_mode == Name)
        {
            ...
            {
                if (subString() == "exclude:") m_exclusion = true;
                else
                    addPattern<TestSpec::NamePattern>();
...
private:
    ...
    {
        if (m_data.outputFilename.empty()) return new CoutStream();
        else if (m_data.outputFilename[0] == '%')
        {
            if (m_data.outputFilename == "%debug") return new DebugOutStream();
            else
...
// CLARA_CONFIG_CPP11_GENERATED_METHODS : The delete and default keywords for compiler generated
// methods CLARA_CONFIG_CPP11_OVERRIDE : is override supported? CLARA_CONFIG_CPP11_UNIQUE_PTR : is
// unique_ptr supported (otherwise use auto_ptr)
...
public:
    ...
    {
        if (inQuotes || std::string("\0", 3).find(c) == std::string::npos) return mode;
...
public:
    ...
    void argSynopsis(std::ostream& os) const
    {
        for (int i = 1; i <= m_highestSpecifiedArgPosition; ++i)
        {
            ...
            if (it != m_positionalArgs.end()) os << "<" << it->second.placeholder << ">";
    ...
    std::vector<Parser::Token> parseInto(std::vector<std::string> const& args, ConfigT& config)
        const
    {
        ...
        ConfigT& config) const
    {
        ...
        for (std::size_t i = 0; i < tokens.size(); ++i)
        {
            ...
            if (it != m_positionalArgs.end()) it->second.boundField.set(config, token.data);
            else
                unusedTokens.push_back(token);
...
inline std::size_t listTests(Config const& config)
{
    ...
    if (config.testSpec().hasFilters()) Catch::cout() << "Matching test cases:\n";
    else
    {
...
{
    TestSpec testSpec = config.testSpec();
    if (config.testSpec().hasFilters()) Catch::cout() << "Tags for matching test cases:\n";
    else
    {
...
IResultCapture& getResultCapture()
{
    if (IResultCapture* capture = getCurrentContext().getResultCapture()) return *capture;
    else
...
IColourImpl* platformColourInstance()
{
    ...
    if (colourMode == UseColour::Auto)
        colourMode =
            (!isDebuggerActive() && isatty(STDOUT_FILENO)) ? UseColour::Yes : UseColour::No;
...
{
    if (isFalseTest(m_info.resultDisposition)) return "!" + m_info.capturedExpression;
    else
        return m_info.capturedExpression;
}
std::string AssertionResult::getExpressionInMacro() const
{
    if (m_info.macroName.empty()) return m_info.capturedExpression;
    else
...
{
    if (startsWith(tag, ".") || tag == "hide" || tag == "!hide") return TestCaseInfo::IsHidden;
    else if (tag == "!throws")
    ...
    for (std::size_t i = 0; i < _descOrTags.size(); ++i)
    {
        ...
        else
        {
            if (c == ']')
            {
                ...
                if (prop == TestCaseInfo::IsHidden) isHidden = true;
...
Section::~Section()
{
    if (m_sectionIncluded)
    {
        ...
        if (std::uncaught_exception()) getResultCapture().sectionEndedEarly(endInfo);
        else
...
AssertionResult ResultBuilder::build() const
{
    ...
    {
        if (data.resultType == ResultWas::Ok) data.resultType = ResultWas::ExpressionFailed;
...
public:
        ...
        SectionNode const& sectionNode)
    {
        ...
        for (SectionNode::ChildSections::const_iterator it = sectionNode.childSections.begin(),
             ...
             it != itEnd;
             ++it)
            if (className.empty()) writeSection(name, "", **it);
            else
