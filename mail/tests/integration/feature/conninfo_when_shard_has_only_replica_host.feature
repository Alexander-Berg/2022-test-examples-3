Feature: Conninfo when shard has only replica host 
  ...
  Scenario: Preparing the database for testing
    Given the first alive host with the role "master" was deleted


  Scenario: Alive replica should be returned when shard has only replica host
  ...
  Scenario: Nothing should be returned when shard has only replica host
    When we register uid "123" in sharddb shard "1"
    And we register uid "123" in mdb shard "1"
    And we request sharpei for conninfo with uid "123" and mode <mode>
