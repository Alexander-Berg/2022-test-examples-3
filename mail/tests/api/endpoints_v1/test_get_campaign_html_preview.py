...
def test_failing_rendering(tvm_api_client, user_id, account, campaign_with_letter):
    letter = campaign_with_letter.default_letter
    letter.html_body = "<html> {% some_unknown_tag_that_cannot_be_rendered %} </html>"
    letter.save()
    response = do_request(tvm_api_client, user_id, account.name, campaign_with_letter.slug)
...
def create_url(user_id, account_slug, campaign_slug):
    query_params = {}
    ...
    return "/api/v1/campaign-html-preview?" + urllib.parse.urlencode(query_params)


def do_request(client, user_id, account_slug, campaign_slug):
    url = create_url(user_id, account_slug, campaign_slug)
