...
from mail.ohio.ohio.tests.utils import dummy_async_context_manager, dummy_async_function
...
def configure_db():
    if not settings.DB_USE_DEVPACK:
        ...
        coordinator = Coordinator(
            TestEnv(port_manager, OhioDB),
            top_comp_cls=OhioDB,
        )
        coordinator.start()
        db = coordinator.components[OhioDB]
        settings.DATABASE = {
            'NAME': db.pg.dbname,
            'USER': None,
            'PASSWORD': None,
...
def raw_db_engine(loop, mock_pg_pinger):
    engine = create_configured_engine(loop)
    yield engine
    engine.close()
