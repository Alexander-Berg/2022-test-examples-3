...
/**
 * Created by mabelpines on 01.12.15.
 */
@Aqua.Test
@Title("Отмена создания пользовательской кнопки")
@Stories(FeaturesConst.CUSTOM_BUTTONS)
@Features(FeaturesConst.MESSAGE_LIST)
@Tag(FeaturesConst.MESSAGE_LIST)
public class CustomButtonCancelTest extends BaseTest {
    ...
    @Title("Отмена создания пользовательской кнопки")
    @TestCaseId("1949")
    public void shouldNotCreateNewCustomBtn() {
        ...
        user.defaultSteps().waitInSeconds(2)
            ...
            .clicksOn(onCustomButtons().overview().archive())
            .clicksOn(onCustomButtons().overview().cancel())
            .shouldNotSee(onCustomButtons().overview(), onMessagePage().toolbar().archiveButton());
    }
}
