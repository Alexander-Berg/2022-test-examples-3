...
public open class ManageFoldersDeleteFolderWithSubfoldersByLongSwipeTest(): RegularYandexMailTestBase("FoldersManager. Удаление папки, в которой есть подпапки, длинным свайпом") {
    open override fun setupSettings(settings: TestSettings): Unit {
...
public open class ManageFoldersValidateViewTest(): RegularYandexMailTestBase("FoldersManager. Внешний вид экрана Управление папками") {
    ...
    open override fun regularScenario(account: UserAccount): TestPlan {
        return this.yandexLogin(account).then(OpenFolderListAction()).then(OpenFolderManagerAction())
    }
...
public open class ManageFoldersValidateFolderLocationViewTest(): RegularYandexMailTestBase("FoldersManager. Внешний вид экрана Расположение папки") {
    ...
    open override fun regularScenario(account: UserAccount): TestPlan {
        return this.yandexLogin(account).then(OpenFolderListAction()).then(OpenFolderManagerAction()).then(OpenCreateFolderScreenAction()).then(OpenFolderLocationScreenAction())
    }
