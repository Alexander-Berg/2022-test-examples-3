...
import static ru.yandex.autotests.innerpochta.beans.tskv.OperTskv.RESET_FRESH;
import static ru.yandex.autotests.innerpochta.beans.tskv.TargetTskv.MAILBOX;
import static ru.yandex.autotests.innerpochta.beans.tskv.WmiTskv.CLIENT_TYPE;
import static ru.yandex.autotests.innerpochta.beans.tskv.WmiTskv.OPERATION;
import static ru.yandex.autotests.innerpochta.beans.tskv.WmiTskv.REQUEST_ID;
import static ru.yandex.autotests.innerpochta.beans.tskv.WmiTskv.TARGET;
...
import ru.yandex.autotests.innerpochta.wmi.core.oper.hound.ResetFreshCounter;
 ...
 */
@Aqua.Test
@Title("[LOGS] TSKV лог для hound2")
@Description("tskv hound лог - " + TskvHound2Test.TSKV_HOUND_LOG_PATH)
...
@IgnoreSshTest
public class TskvHound2Test extends BaseHoundTest {

    @ClassRule
    public static SSHAuthRule sshAuthRule = sshOn(URI.create(props().houndUri()), props().getRobotGerritWebmailTeamSshKey());

    @Rule
    public RetryRule retry = RetryRule.retry().ifException(IndexOutOfBoundsException.class)
            .times(5).every(5, TimeUnit.SECONDS);

    public static final String TSKV_HOUND_LOG_PATH = "/app/log/user_journal.tskv";
    ...
    @Test
    @Issue("DARIA-53895")
    @Title("Пробрасываем X-Yandex-ExpBoxes в /app/log/user_journal.tskv")
    public void shouldWriteExpInHound() {
        String exp = "experiment_" + Util.getRandomString();
        ResetFreshCounter.resetFreshCounter(CountersObject.empty().setUid(uid())).header(Headers.EXP_BOXES, exp)
                .get().via(authClient).shouldBe().ok();
    ...
    public void shouldWriteClientTypeInHound() {
        ...
        ResetFreshCounter.resetFreshCounter(CountersObject.empty().setUid(uid())).header(Headers.CLIENT_TYPE, exp)
                .get().via(authClient).shouldBe().ok();
