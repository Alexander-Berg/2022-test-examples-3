// <<< AUTOGENERATED BY YANDEX.SCRIPT FROM client/requests/foo-request.ts >>>

import Foundation

open class MoveToSpamRequest: BaseNetworkRequest {
  private let mid: ID!
  private let tid: ID!
  private let currentFolder: ID
  public init(_ mid: ID!, _ tid: ID!, _ currentFolder: ID, _ platform: Platform, _ networkExtra: NetworkExtra) {
    self.mid = mid
    self.tid = tid
    self.currentFolder = currentFolder
    super.init(platform, networkExtra)
  }

  @discardableResult
  open override func encoding() -> RequestEncoding {
    return UrlRequestEncoding()
  }

  @discardableResult
  open override func method() -> NetworkMethod {
    return NetworkMethod.post
  }

  @discardableResult
  open override func params() -> MapJSONItem {
    let params = MapJSONItem()
    if mid != nil {
      params.put("mids", StringJSONItem(int64ToString(mid)))
    }
    if tid != nil {
      params.put("tids", StringJSONItem(int64ToString(tid)))
    }
    params.put("current_folder", StringJSONItem(int64ToString(currentFolder)))
    return params
  }

  @discardableResult
  open override func path() -> String {
    return "foo"
  }

  @discardableResult
  open override func version() -> NetworkAPIVersions {
    return NetworkAPIVersions.v1
  }
}
