...
open class MovableToFolderBackend: MovableToFolder {
  ...
  open func moveMessageToFolder(_ order: Int32, _ folderName: String) {
    let tid = mailListDisplayBackend.getThreadMessage(order).tid!
    let fid = clientsHandler.getCurrentClient().getFolderList().filter {
      f in
      f.name == folderName
    }[0].fid
    clientsHandler.getCurrentClient().moveThreadToFolder(tid, fid)
  }
}
